...
const SCAN_SURFACES: &[&str] = &[
    ...
    "/etc/profile.d", "/usr/share/aur/aurto",
    "/etc/xdg/autostart", "/usr/share/polkit-1/rules.d",
];
...
fn has_rust_evidence(prefix: &[u8]) -> bool {
    const SIGS: &[&[u8]] = &[b"rust_begin_unwind", b"__rust_alloc", b"libstd-", b"libcore-"];
    SIGS.iter().any(|s| prefix.windows(s.len()).any(|w| w == *s))
}
...
fn scan_directory(dir: &str) -> Vec<AuditRecord> {
    ...
    entries.filter_map(|e| e.ok()).filter_map(|entry| {
        ...
        if is_app_scope(&path_str) { return None; }
        let Ok(content) = fs::read(&path) else { return None };
        let prefix = &content[..content.len().min(512)];
        let classification = classify_script_bytes(prefix)?;
        let is_blocker = matches!(classification, "PYTHON"|"BASH"|"SHELL"|"PERL"|"RUBY"|"NODE"|"UNKNOWN_SCRIPT");
        Some(AuditRecord { path: path_str.clone(), classification, is_blocker, os_owned: is_os_owned(&path_str), file_size: metadata.len() })
    }).collect()
}

fn run_audit() -> Vec<AuditRecord> {
    SCAN_SURFACES.iter().flat_map(|surface| {
        if !Path::new(surface).exists() { return Vec::new(); }
...
fn generate_report(records: &[AuditRecord]) -> String {
    ...
    for r in records {
        *stats.entry(r.classification).or_insert(0) += 1;
        if r.is_blocker { blocker_count += 1; }
    }
    let mut report = format!("=== AETHERFORGE OS RUST AUDIT v{} ===\nTimestamp: {:?}\nItems: {}\nBlockers: {}\n\n", VERSION, std::time::SystemTime::now(), records.len(), blocker_count);
    report.push_str("--- BREAKDOWN ---\n");
    for (k, v) in &stats { report.push_str(&format!("  {}: {}\n", k, v)); }
    report.push_str("\n--- BLOCKERS ---\n");
    for r in records.iter().filter(|x| x.is_blocker) { report.push_str(&format!("  [{}] {}\n", r.classification, r.path)); }
    report
}
...
fn main() {
    ...
    if let Some(home) = dirs::home_dir() {
        let path = home.join("Downloads").join(format!("aetherforge-audit-{}.txt", VERSION));
...
mod tests {
    use super::*;
    #[test] fn test_is_app_scope() { assert!(is_app_scope("/usr/lib/aetherforge/aetherstream/test")); }
    #[test] fn test_is_os_owned() { assert!(is_os_owned("/usr/bin/garuda-update")); assert!(!is_os_owned("/usr/bin/random")); }
    #[test] fn test_classify() { assert_eq!(classify_script_bytes(b"#!/usr/bin/python3\n"), Some("PYTHON")); }
}
