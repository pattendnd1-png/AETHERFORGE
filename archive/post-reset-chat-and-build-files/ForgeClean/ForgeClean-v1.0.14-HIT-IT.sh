...
EXPECTED_DELTA_SHA="63610f68a345df613aac6dae008a39f4ca3297eaa2c46cbf8637937cbce9fda3"
...
: > "$VERIFY"
emit(){ printf '%s\n' "$*" | tee -a "$VERIFY"; }
die(){ emit "FORGECLEAN_V1_0_14=FAIL:$*"; exit 1; }

on_err(){
  local rc=$? line="$1" cmd="$2"
  trap - ERR
  emit "FORGECLEAN_V1_0_14_UNHANDLED_ERROR=RC:${rc}:LINE:${line}:CMD:${cmd}"
  exit "$rc"
}
...
cleanup(){
  ...
  if (( rc == 0 )); then
    emit "FORGECLEAN_V1_0_14=PASS"
  else
    emit "FORGECLEAN_V1_0_14=FAIL:${rc}"
  fi
...
sed -i '0,/^version = "1\.0\.11"$/s//version = "1.0.14"/' "$RED_SRC/Cargo.toml" || die "TDD_RED_VERSION_PATCH_FAILED"
install -m 0755 "$DELTA_ROOT/payload/tests/regression_v1_0_14_pre_rebase.sh" "$RED_SRC/tests/regression_v1_0_14_pre_rebase.sh" || die "TDD_RED_REGRESSION_INSTALL_FAILED"
set +e
(
  cd "$RED_SRC"
  ./tests/regression_v1_0_14_pre_rebase.sh
) >"$STATE/tdd-red.log" 2>&1
RED_RC=$?
set -e
[[ $RED_RC -ne 0 ]] || die "TDD_RED_UNEXPECTEDLY_PASSED"
emit "FORGECLEAN_V1_0_14_TDD_RED=PASS"

"$DELTA_ROOT/apply_delta.sh" "$BASE_SRC" "$SRC" >>"$VERIFY" 2>&1 || die "DELTA_APPLY_FAILED"
(
  cd "$SRC"
  ./tests/regression_v1_0_14_pre_rebase.sh
  ./tests/regression_v1_0_14_rust_first.sh
  bash -n build-and-verify.sh install-local.sh hit-it-template.sh tests/*.sh
) >>"$VERIFY" 2>&1 || die "TDD_GREEN_STATIC_FAILED"
emit "FORGECLEAN_V1_0_14_TDD_GREEN_STATIC=PASS"
...
grep -Fxq 'FORGECLEAN_V1_0_14_RUST_FIRST=PASS' "$VERIFY" || die "RUST_FIRST_GATE_DID_NOT_PASS"
emit "FORGECLEAN_NONDESTRUCTIVE_GATES=PASS"
emit "FORGECLEAN_V1_0_14_TDD_GREEN=PASS"
...
echo 'FORGECLEAN_V1_0_14_ROLLBACK=PASS'
