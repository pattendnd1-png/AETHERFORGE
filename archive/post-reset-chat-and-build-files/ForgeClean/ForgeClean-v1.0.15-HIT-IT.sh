...
EXPECTED_DELTA_SHA="4d1518f2a48cef9967e868bdd10c57fb9c05fa88ada91872ff18c0a690c3d589"
...
: > "$VERIFY"
emit(){ printf '%s\n' "$*" | tee -a "$VERIFY"; }
die(){ emit "FORGECLEAN_V1_0_15=FAIL:$*"; exit 1; }

on_err(){
  local rc=$? line="$1" cmd="$2"
  trap - ERR
  emit "FORGECLEAN_V1_0_15_UNHANDLED_ERROR=RC:${rc}:LINE:${line}:CMD:${cmd}"
  exit "$rc"
}
...
cleanup(){
  ...
  if (( rc == 0 )); then
    emit "FORGECLEAN_V1_0_15=PASS"
  else
    emit "FORGECLEAN_V1_0_15=FAIL:${rc}"
  fi
...
sed -i '0,/^version = "1\.0\.11"$/s//version = "1.0.15"/' "$RED_SRC/Cargo.toml" || die "TDD_RED_VERSION_PATCH_FAILED"
install -m 0755 "$DELTA_ROOT/payload/tests/regression_v1_0_15_pre_rebase.sh" "$RED_SRC/tests/regression_v1_0_15_pre_rebase.sh" || die "TDD_RED_REGRESSION_INSTALL_FAILED"
if (
  cd "$RED_SRC"
  ./tests/regression_v1_0_15_pre_rebase.sh
...
grep -Fxq 'FORGECLEAN_V1_0_15_RUST_FIRST=PASS' "$VERIFY" || die "RUST_FIRST_GATE_DID_NOT_PASS"
emit "FORGECLEAN_NONDESTRUCTIVE_GATES=PASS"
emit "FORGECLEAN_V1_0_15_TDD_GREEN=PASS"
...
echo 'FORGECLEAN_V1_0_15_ROLLBACK=PASS'
