...
on_exit(){
  local rc=$?
  if (( rc != 0 )); then
    if ! grep -Eq '^AETHER_BROWSER_V2_1_14_VERIFY=(PASS|FAIL)' "$VERIFY_FILE" 2>/dev/null; then
      printf 'AETHER_BROWSER_VERSION=%s\nAETHER_BROWSER_V2_1_14_VERIFY=FAIL_EARLY:%s:exit=%s\nAETHER_BROWSER_FAILURE_STAGE=%s\n' "$VERSION" "$CURRENT_STAGE" "$rc" "$CURRENT_STAGE" >> "$VERIFY_FILE"
    fi
    if ! grep -Eq '^AETHER_BROWSER_V2_1_14_INSTALL_VERIFY=(PASS|FAIL)' "$INSTALL_VERIFY" 2>/dev/null; then
      printf 'AETHER_BROWSER_VERSION=%s\nAETHER_BROWSER_V2_1_14_INSTALL_VERIFY=FAIL_EARLY:%s:exit=%s\nAETHER_BROWSER_FAILURE_STAGE=%s\n' "$VERSION" "$CURRENT_STAGE" "$rc" "$CURRENT_STAGE" >> "$INSTALL_VERIFY"
    fi
...
pkill -TERM -x aether-browser 2>/dev/null || true
for _attempt in $(seq 1 30); do
  pgrep -x aether-browser >/dev/null 2>&1 || break
  sleep 0.1
done
if pgrep -x aether-browser >/dev/null 2>&1; then
  pkill -KILL -x aether-browser 2>/dev/null || true
  sleep 0.2
fi
if pgrep -x aether-browser >/dev/null 2>&1; then
  fail 'AETHER_BROWSER_PREVERIFY_BROWSER_SHUTDOWN=FAIL:process-still-running' 74
fi
