...
shutdown_browser_processes(){
  ...
  if (( ${#zombies[@]} > 0 )); then
    record "AETHER_BROWSER_PREVERIFY_BROWSER_SHUTDOWN_ZOMBIES=IGNORED:${zombies[*]}"
  fi
  mapfile -t pids < <(browser_live_pids)
  if (( ${#pids[@]} > 0 )); then
    record "AETHER_BROWSER_PREVERIFY_BROWSER_SHUTDOWN_LIVE_PIDS=FAIL:${pids[*]}"
    return 1
  fi
  return 0
}
...
on_exit(){
  local rc=$?
  if (( rc != 0 )); then
    if ! grep -Eq '^AETHER_BROWSER_V2_1_55_VERIFY=(PASS|FAIL)' "$VERIFY_FILE" 2>/dev/null; then
      printf 'AETHER_BROWSER_VERSION=%s\nAETHER_BROWSER_V2_1_55_VERIFY=FAIL_EARLY:%s:exit=%s\nAETHER_BROWSER_FAILURE_STAGE=%s\n' "$VERSION" "$CURRENT_STAGE" "$rc" "$CURRENT_STAGE" >> "$VERIFY_FILE"
    fi
    if ! grep -Eq '^AETHER_BROWSER_V2_1_55_INSTALL_VERIFY=(PASS|FAIL)' "$INSTALL_VERIFY" 2>/dev/null; then
      printf 'AETHER_BROWSER_VERSION=%s\nAETHER_BROWSER_V2_1_55_INSTALL_VERIFY=FAIL_EARLY:%s:exit=%s\nAETHER_BROWSER_FAILURE_STAGE=%s\n' "$VERSION" "$CURRENT_STAGE" "$rc" "$CURRENT_STAGE" >> "$INSTALL_VERIFY"
    fi
...
# A minimized old browser is still a live mapped process. Shut it down before any
# runtime verification so probes and the final launch cannot observe stale state.
# Zombies are already dead and cannot be killed; never let an unreaped zombie block install.
CURRENT_STAGE='stale-browser-shutdown'
shutdown_browser_processes || fail 'AETHER_BROWSER_PREVERIFY_BROWSER_SHUTDOWN=FAIL:live-process-still-running' 74
record 'AETHER_BROWSER_PREVERIFY_BROWSER_SHUTDOWN=PASS'
...
CURRENT_STAGE='preinstall-hard-verify'
record 'AETHER_BROWSER_PREINSTALL_HARD_VERIFY=START'
AETHER_BROWSER_VERIFY_PHASE=preinstall AETHER_BROWSER_MEDIA_RENDERER=cpu-bgra AETHER_BROWSER_OUT_DIR="$DOWNLOADS" bash scripts/verify.sh || fail "AETHER_BROWSER_${TAG}_PREINSTALL_HARD_VERIFY=FAIL" 8
record "AETHER_BROWSER_${TAG}_PREINSTALL_HARD_VERIFY=PASS"
record 'AETHER_BROWSER_PREINSTALL_VISUAL_PROBES=DEFERRED'

CURRENT_STAGE='stage-promote-install'
