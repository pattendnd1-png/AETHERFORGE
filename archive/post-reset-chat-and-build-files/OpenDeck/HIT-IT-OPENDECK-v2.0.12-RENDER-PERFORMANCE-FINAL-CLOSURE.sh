...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.12-RENDER-PERFORMANCE-FINAL-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="fa7b65cb7d8933dc3f8b95e66fb5d1a54c08676efe10b92e95c231cf65c4c2a8"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.12-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
INSTALL_ROOT="$HOME/.local/lib/opendeck-v2.0.12"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.12-qualified"
...
fail(){ printf 'OPENDECK_V2_0_12_RENDER_PERFORMANCE=FAIL:%s\nOPENDECK_FAILURE_STAGE=%s\n' "${2:-1}" "$1" >&2; exit "${2:-1}"; }
...
if [[ "${OPENDECK_V212_HUMAN_VISUAL_APPROVED:-0}" == "1" && -f "$STAMP" && -x "$QUALIFIED_ROOT/bin/opendeck-studio" ]]; then
...
PY
  then
    [[ "${OPENDECK_V212_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V212_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V212_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  ...
  stage="$HOME/.local/lib/.opendeck-v2.0.12-stage-$$"
  ...
  if [[ -d "$INSTALL_ROOT" ]]; then mv "$INSTALL_ROOT" "$HOME/.local/lib/opendeck-v2.0.12-previous-$(date +%Y%m%d-%H%M%S)"; fi
  ...
  {
    printf 'OPENDECK_V212_VISUAL_HUMAN_APPROVAL=PASS\n'
    printf 'OPENDECK_V212_INSTALL_ROOT=%s\n' "$INSTALL_ROOT"
    printf 'OPENDECK_V212_ACTIVE_BINARY_SHA256=%s\n' "$active_sha"
    printf 'OPENDECK_V212_BACKGROUND_SERVICES=%s\n' "$service_count"
    printf 'OPENDECK_V212_AUTOSTART_ENTRIES=%s\n' "$autostart_count"
    printf 'OPENDECK_V2_0_12_RENDER_PERFORMANCE_QUALIFY=PASS\n'
    printf 'OPENDECK_V2_0_12_RENDER_PERFORMANCE_ACTIVATE=PASS\n'
    printf 'OPENDECK_V2_0_12_RENDER_PERFORMANCE=PASS\n'
...
for candidate in \
  ...
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v212-node-modules-reuse-$$"
...
OPENDECK_V212_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" \
OPENDECK_V212_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" \
exec bash "$SRC/scripts/qualify-v212-host.sh"
