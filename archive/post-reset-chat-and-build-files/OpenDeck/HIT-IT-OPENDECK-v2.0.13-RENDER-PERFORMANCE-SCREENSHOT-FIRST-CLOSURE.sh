...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.13-RENDER-PERFORMANCE-SCREENSHOT-FIRST-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="c867e4c72698dfd3e58352d671f478ca14b49fc339e530126419354e651a1281"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.13-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
INSTALL_ROOT="$HOME/.local/lib/opendeck-v2.0.13"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.13-qualified"
...
fail(){ printf 'OPENDECK_V2_0_13_RENDER_PERFORMANCE=FAIL:%s\nOPENDECK_FAILURE_STAGE=%s\n' "${2:-1}" "$1" >&2; exit "${2:-1}"; }
...
if [[ "${OPENDECK_V213_HUMAN_VISUAL_APPROVED:-0}" == "1" && -f "$STAMP" && -x "$QUALIFIED_ROOT/bin/opendeck-studio" ]]; then
...
PY
  then
    [[ "${OPENDECK_V213_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V213_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V213_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  ...
  stage="$HOME/.local/lib/.opendeck-v2.0.13-stage-$$"
  ...
  if [[ -d "$INSTALL_ROOT" ]]; then mv "$INSTALL_ROOT" "$HOME/.local/lib/opendeck-v2.0.13-previous-$(date +%Y%m%d-%H%M%S)"; fi
  ...
  {
    printf 'OPENDECK_V213_VISUAL_HUMAN_APPROVAL=PASS\n'
    printf 'OPENDECK_V213_INSTALL_ROOT=%s\n' "$INSTALL_ROOT"
    printf 'OPENDECK_V213_ACTIVE_BINARY_SHA256=%s\n' "$active_sha"
    printf 'OPENDECK_V213_BACKGROUND_SERVICES=%s\n' "$service_count"
    printf 'OPENDECK_V213_AUTOSTART_ENTRIES=%s\n' "$autostart_count"
    printf 'OPENDECK_V2_0_13_RENDER_PERFORMANCE_QUALIFY=PASS\n'
    printf 'OPENDECK_V2_0_13_RENDER_PERFORMANCE_ACTIVATE=PASS\n'
    printf 'OPENDECK_V2_0_13_RENDER_PERFORMANCE=PASS\n'
...
for candidate in \
  ...
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v213-node-modules-reuse-$$"
...
OPENDECK_V213_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" \
OPENDECK_V213_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" \
exec bash "$SRC/scripts/qualify-v213-host.sh"
