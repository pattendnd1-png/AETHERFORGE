...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.14-BOOTSTRAP-READINESS-DIAGNOSTIC-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="457c1d1733518b544903eedb63ea5ac78fc8eee10c5a7b5ff507521bea2c70ca"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.14-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
INSTALL_ROOT="$HOME/.local/lib/opendeck-v2.0.14"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.14-qualified"
...
if [[ "${OPENDECK_V214_HUMAN_VISUAL_APPROVED:-0}" == "1" && -f "$STAMP" && -x "$QUALIFIED_ROOT/bin/opendeck-studio" ]]; then
...
PY
  then
    [[ "${OPENDECK_V214_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V214_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V214_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  ...
  stage="$HOME/.local/lib/.opendeck-v2.0.14-stage-$$"
  ...
  if [[ -d "$INSTALL_ROOT" ]]; then mv "$INSTALL_ROOT" "$HOME/.local/lib/opendeck-v2.0.14-previous-$(date +%Y%m%d-%H%M%S)"; fi
  ...
  {
    printf 'OPENDECK_V214_VISUAL_HUMAN_APPROVAL=PASS\n'
    printf 'OPENDECK_V214_INSTALL_ROOT=%s\n' "$INSTALL_ROOT"
    printf 'OPENDECK_V214_ACTIVE_BINARY_SHA256=%s\n' "$active_sha"
    printf 'OPENDECK_V214_BACKGROUND_SERVICES=%s\n' "$service_count"
    printf 'OPENDECK_V214_AUTOSTART_ENTRIES=%s\n' "$autostart_count"
...
for candidate in \
  ...
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v214-node-modules-reuse-$$"
...
OPENDECK_V214_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" \
OPENDECK_V214_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" \
exec bash "$SRC/scripts/qualify-v214-host.sh"
