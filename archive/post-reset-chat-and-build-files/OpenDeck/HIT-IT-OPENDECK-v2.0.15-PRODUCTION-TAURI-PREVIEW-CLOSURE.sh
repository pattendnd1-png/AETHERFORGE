...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.15-PRODUCTION-TAURI-PREVIEW-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="81f91470a78085497cae6c12c84cd3d4a357d10a16aa4b76fbd1079c8bb81792"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.15-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
INSTALL_ROOT="$HOME/.local/lib/opendeck-v2.0.15"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.15-qualified"
...
fail(){ printf 'OPENDECK_V2_0_15_RENDER_PERFORMANCE=FAIL:%s\nOPENDECK_FAILURE_STAGE=%s\n' "${2:-1}" "$1" >&2; exit "${2:-1}"; }
...
# Fast second phase: after ChatGPT/user visual review, activate the exact already-qualified bytes.
if [[ "${OPENDECK_V215_HUMAN_VISUAL_APPROVED:-0}" == "1" && -f "$STAMP" && -x "$QUALIFIED_ROOT/bin/opendeck-studio" ]]; then
...
PY
  then
    [[ "${OPENDECK_V215_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V215_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V215_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  ...
  mkdir -p "$HOME/.local/lib" "$USER_BIN"
  {
    ...
    printf "RESTORE_COMMAND=ln -sfn '%s' '%s'\n" "$baseline_target" "$USER_BIN/opendeck-studio"
  } > "$ROLLBACK"
  chmod 600 "$ROLLBACK"

  stage="$HOME/.local/lib/.opendeck-v2.0.15-stage-$$"
  ...
  if [[ -d "$INSTALL_ROOT" ]]; then mv "$INSTALL_ROOT" "$HOME/.local/lib/opendeck-v2.0.15-previous-$(date +%Y%m%d-%H%M%S)"; fi
  ...
  {
    printf 'OPENDECK_V215_VISUAL_HUMAN_APPROVAL=PASS\n'
    printf 'OPENDECK_V215_INSTALL_ROOT=%s\n' "$INSTALL_ROOT"
    printf 'OPENDECK_V215_ACTIVE_BINARY_SHA256=%s\n' "$active_sha"
    printf 'OPENDECK_V215_BACKGROUND_SERVICES=%s\n' "$service_count"
    printf 'OPENDECK_V215_AUTOSTART_ENTRIES=%s\n' "$autostart_count"
    printf 'OPENDECK_V2_0_15_RENDER_PERFORMANCE_QUALIFY=PASS\n'
    printf 'OPENDECK_V2_0_15_RENDER_PERFORMANCE_ACTIVATE=PASS\n'
    printf 'OPENDECK_V2_0_15_RENDER_PERFORMANCE=PASS\n'
    printf 'VERIFY_FILE=%s\n' "$VERIFY"
    printf 'ROLLBACK_FILE=%s\n' "$ROLLBACK"
    printf 'RUN_COMMAND=%s\n' "$USER_BIN/opendeck-studio"
  } | tee -a "$VERIFY"
  exit 0
fi
...
for candidate in \
  ...
  "$DL/OpenDeck-v2.0.14-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules" \
  "$DL/OpenDeck-v2.0.13-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules" \
  "$DL/OpenDeck-v2.0.12-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules" \
  "$DL/OpenDeck-v2.0.11-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules" \
  "$DL/OpenDeck-v2.0.10-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules" \
  ...
  "$DL/OpenDeck-v2.0.5-TWITCH-AUTH-COMPLETION-SOURCE/apps/opendeck-studio/node_modules"; do
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v215-node-modules-reuse-$$"
...
# Defer dependency reuse until the extracted source has passed all static/source-hygiene gates.
OPENDECK_V215_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" \
OPENDECK_V215_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" \
exec bash "$SRC/scripts/qualify-v215-host.sh"
