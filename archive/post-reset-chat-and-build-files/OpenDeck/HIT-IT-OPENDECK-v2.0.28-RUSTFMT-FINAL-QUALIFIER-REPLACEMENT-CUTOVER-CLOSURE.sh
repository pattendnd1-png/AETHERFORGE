...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.28-RUSTFMT-FINAL-QUALIFIER-REPLACEMENT-CUTOVER-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="cf0ff8423e1ae9df659992dbcd29bfd4b55f09b2659a97db0bfba748733fbe8b"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.28-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.28-qualified"
...
fail(){ printf 'OPENDECK_V2_0_28_RENDER_PERFORMANCE=FAIL:%s\nOPENDECK_FAILURE_STAGE=%s\n' "${2:-1}" "$1" >&2; exit "${2:-1}"; }
...
# Phase 2: replace the installed version only with the exact already-qualified bytes.
if [[ "${OPENDECK_V228_HUMAN_VISUAL_APPROVED:-0}" == "1" ]]; then
...
PY
  then
    [[ "${OPENDECK_V228_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V228_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V228_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  exec bash "$SRC/scripts/activate-v228-qualified.sh"     "$QUALIFIED_ROOT/bin/opendeck-studio" "$stamp_binary" "$SRC" "$ROLLBACK" "$VERIFY"
fi

# Phase 1: qualify the sealed source. Reuse a prior dependency tree if one exists.
NODE_MODULES_REUSE=""
for candidate in   "$SRC/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.27-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.26-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.25-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.24-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.23-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.22-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.21-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.19-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.18-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.17-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.16-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.15-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.8-DIAL-ENCODER-RUSTFMT-CLOSURE-SOURCE/apps/opendeck-studio/node_modules"; do
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v228-node-modules-reuse-$$"
...
OPENDECK_V228_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" OPENDECK_V228_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" exec bash "$SRC/scripts/qualify-v228-host.sh"
