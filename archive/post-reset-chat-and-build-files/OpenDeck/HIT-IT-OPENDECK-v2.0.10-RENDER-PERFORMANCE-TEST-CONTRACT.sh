...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.10-RENDER-PERFORMANCE-TEST-CONTRACT-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="a81e057cca7abda014608a65155d15a7c72999f86af6af6258f6ba0991fbdfd9"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.10-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
INSTALL_ROOT="$HOME/.local/lib/opendeck-v2.0.10"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.10-qualified"
VERIFY="$DL/OpenDeck-v2.0.10-RENDER-PERFORMANCE-VERIFY.txt"
ROLLBACK="$DL/OpenDeck-v2.0.10-RENDER-PERFORMANCE-ROLLBACK.txt"
STAMP="$DL/OpenDeck-v2.0.10-QUALIFIED-STAMP.txt"
SCREENSHOT="$DL/OpenDeck-v2.0.10-QUALIFICATION.png"
QDIR="$DL/OpenDeck-v2.0.10-qualification"
VISUAL_JSON="$QDIR/OpenDeck-v2.0.10-VISUAL-METRICS.json"
PERF_JSON="$QDIR/OpenDeck-v2.0.10-PERFORMANCE-METRICS.json"
...
if [[ "${OPENDECK_V210_HUMAN_VISUAL_APPROVED:-0}" == "1" && -f "$STAMP" && -x "$QUALIFIED_ROOT/bin/opendeck-studio" ]]; then
...
PY
  then
    [[ "${OPENDECK_V210_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V210_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V210_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  ...
  stage="$HOME/.local/lib/.opendeck-v2.0.10-stage-$$"
  ...
  if [[ -d "$INSTALL_ROOT" ]]; then mv "$INSTALL_ROOT" "$HOME/.local/lib/opendeck-v2.0.10-previous-$(date +%Y%m%d-%H%M%S)"; fi
  ...
  {
    printf 'OPENDECK_V210_VISUAL_HUMAN_APPROVAL=PASS\n'
    printf 'OPENDECK_V210_INSTALL_ROOT=%s\n' "$INSTALL_ROOT"
    printf 'OPENDECK_V210_ACTIVE_BINARY_SHA256=%s\n' "$active_sha"
    printf 'OPENDECK_V210_BACKGROUND_SERVICES=%s\n' "$service_count"
    printf 'OPENDECK_V210_AUTOSTART_ENTRIES=%s\n' "$autostart_count"
...
for candidate in \
  ...
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v210-node-modules-reuse-$$"
...
OPENDECK_V210_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" \
OPENDECK_V210_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" \
exec bash "$SRC/scripts/qualify-v210-host.sh"
