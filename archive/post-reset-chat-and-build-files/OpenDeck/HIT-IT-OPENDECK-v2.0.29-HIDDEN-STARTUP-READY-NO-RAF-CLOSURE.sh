...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.29-HIDDEN-STARTUP-READY-NO-RAF-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="3beee5ad7458942262fc250614963f4991148726274961defe3c6c00ae566234"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.29-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.29-qualified"
...
fail(){ printf 'OPENDECK_V2_0_29_RENDER_PERFORMANCE=FAIL:%s\nOPENDECK_FAILURE_STAGE=%s\n' "${2:-1}" "$1" >&2; exit "${2:-1}"; }
...
if [[ "${OPENDECK_V229_HUMAN_VISUAL_APPROVED:-0}" == "1" ]]; then
...
PY
  then
    [[ "${OPENDECK_V229_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V229_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V229_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  exec bash "$SRC/scripts/activate-v229-qualified.sh" \
...
for candidate in \
  ...
  "$DL/OpenDeck-v2.0.26-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules" \
  "$DL/OpenDeck-v2.0.25-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules" \
  "$DL/OpenDeck-v2.0.24-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules" \
  "$DL/OpenDeck-v2.0.23-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules" \
  ...
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v229-node-modules-reuse-$$"
...
OPENDECK_V229_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" OPENDECK_V229_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" exec bash "$SRC/scripts/qualify-v229-host.sh"
