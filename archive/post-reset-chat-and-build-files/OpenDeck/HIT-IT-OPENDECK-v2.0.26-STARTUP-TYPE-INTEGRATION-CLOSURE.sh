...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.26-STARTUP-TYPE-INTEGRATION-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="b01b47cab59e6ee2ee1ac66662f4c6582a0b7ab7569a853834b98b2e4667da98"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.26-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.26-qualified"
...
fail(){ printf 'OPENDECK_V2_0_26_RENDER_PERFORMANCE=FAIL:%s\nOPENDECK_FAILURE_STAGE=%s\n' "${2:-1}" "$1" >&2; exit "${2:-1}"; }
...
if [[ "${OPENDECK_V226_HUMAN_VISUAL_APPROVED:-0}" == "1" ]]; then
...
PY
  then
    [[ "${OPENDECK_V226_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V226_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V226_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  exec bash "$SRC/scripts/activate-v226-qualified.sh"     "$QUALIFIED_ROOT/bin/opendeck-studio" "$stamp_binary" "$SRC" "$ROLLBACK" "$VERIFY"
fi
...
for candidate in   "$SRC/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.25-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.24-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.23-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.22-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.21-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.19-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.18-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.17-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.16-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.15-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.8-DIAL-ENCODER-RUSTFMT-CLOSURE-SOURCE/apps/opendeck-studio/node_modules"; do
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v226-node-modules-reuse-$$"
...
OPENDECK_V226_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" OPENDECK_V226_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" exec bash "$SRC/scripts/qualify-v226-host.sh"
