...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.11-RENDER-PERFORMANCE-ACCESSIBILITY-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="4bf742ec08c042765a10568acbc689e1ddddd41ef4ead341593fefe24064f035"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.11-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
INSTALL_ROOT="$HOME/.local/lib/opendeck-v2.0.11"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.11-qualified"
VERIFY="$DL/OpenDeck-v2.0.11-RENDER-PERFORMANCE-VERIFY.txt"
ROLLBACK="$DL/OpenDeck-v2.0.11-RENDER-PERFORMANCE-ROLLBACK.txt"
STAMP="$DL/OpenDeck-v2.0.11-QUALIFIED-STAMP.txt"
SCREENSHOT="$DL/OpenDeck-v2.0.11-QUALIFICATION.png"
QDIR="$DL/OpenDeck-v2.0.11-qualification"
VISUAL_JSON="$QDIR/OpenDeck-v2.0.11-VISUAL-METRICS.json"
PERF_JSON="$QDIR/OpenDeck-v2.0.11-PERFORMANCE-METRICS.json"

fail(){ printf 'OPENDECK_V2_0_10_RENDER_PERFORMANCE=FAIL:%s\nOPENDECK_FAILURE_STAGE=%s\n' "${2:-1}" "$1" >&2; exit "${2:-1}"; }
...
if [[ "${OPENDECK_V211_HUMAN_VISUAL_APPROVED:-0}" == "1" && -f "$STAMP" && -x "$QUALIFIED_ROOT/bin/opendeck-studio" ]]; then
...
PY
  then
    [[ "${OPENDECK_V211_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V211_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V211_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  ...
  stage="$HOME/.local/lib/.opendeck-v2.0.11-stage-$$"
  ...
  if [[ -d "$INSTALL_ROOT" ]]; then mv "$INSTALL_ROOT" "$HOME/.local/lib/opendeck-v2.0.11-previous-$(date +%Y%m%d-%H%M%S)"; fi
  ...
  {
    printf 'OPENDECK_V211_VISUAL_HUMAN_APPROVAL=PASS\n'
    printf 'OPENDECK_V211_INSTALL_ROOT=%s\n' "$INSTALL_ROOT"
    printf 'OPENDECK_V211_ACTIVE_BINARY_SHA256=%s\n' "$active_sha"
    printf 'OPENDECK_V211_BACKGROUND_SERVICES=%s\n' "$service_count"
    printf 'OPENDECK_V211_AUTOSTART_ENTRIES=%s\n' "$autostart_count"
    printf 'OPENDECK_V2_0_10_RENDER_PERFORMANCE_QUALIFY=PASS\n'
    printf 'OPENDECK_V2_0_10_RENDER_PERFORMANCE_ACTIVATE=PASS\n'
    printf 'OPENDECK_V2_0_10_RENDER_PERFORMANCE=PASS\n'
...
for candidate in \
  ...
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v211-node-modules-reuse-$$"
...
OPENDECK_V211_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" \
OPENDECK_V211_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" \
exec bash "$SRC/scripts/qualify-v211-host.sh"
