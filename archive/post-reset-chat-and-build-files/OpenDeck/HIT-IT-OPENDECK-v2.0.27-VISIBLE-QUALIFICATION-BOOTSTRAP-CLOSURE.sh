...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.27-VISIBLE-QUALIFICATION-BOOTSTRAP-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="aa5ce8df9424a5c40d5bc8e8a542a59d20c852e25c0c0acde34bc0c91723a662"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.27-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.27-qualified"
...
fail(){ printf 'OPENDECK_V2_0_27_RENDER_PERFORMANCE=FAIL:%s\nOPENDECK_FAILURE_STAGE=%s\n' "${2:-1}" "$1" >&2; exit "${2:-1}"; }
...
if [[ "${OPENDECK_V227_HUMAN_VISUAL_APPROVED:-0}" == "1" ]]; then
...
PY
  then
    [[ "${OPENDECK_V227_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V227_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V227_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  exec bash "$SRC/scripts/activate-v227-qualified.sh"     "$QUALIFIED_ROOT/bin/opendeck-studio" "$stamp_binary" "$SRC" "$ROLLBACK" "$VERIFY"
fi
...
for candidate in   "$SRC/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.26-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.25-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.24-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.23-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.22-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.21-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.19-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.18-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.17-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.16-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.15-RENDER-PERFORMANCE-SOURCE/apps/opendeck-studio/node_modules"   "$DL/OpenDeck-v2.0.8-DIAL-ENCODER-RUSTFMT-CLOSURE-SOURCE/apps/opendeck-studio/node_modules"; do
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v227-node-modules-reuse-$$"
...
OPENDECK_V227_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" OPENDECK_V227_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" exec bash "$SRC/scripts/qualify-v227-host.sh"
