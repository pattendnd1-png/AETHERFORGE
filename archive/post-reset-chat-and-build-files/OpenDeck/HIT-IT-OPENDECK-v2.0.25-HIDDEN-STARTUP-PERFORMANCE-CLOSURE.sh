...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.25-HIDDEN-STARTUP-PERFORMANCE-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="e791a4f4a3bfb2a536d65cf70ba6cdea784aece162ff0a8f6fe5dd1326e62b74"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.25-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.25-qualified"
...
fail(){ printf 'OPENDECK_V2_0_25_RENDER_PERFORMANCE=FAIL:%s\nOPENDECK_FAILURE_STAGE=%s\n' "${2:-1}" "$1" >&2; exit "${2:-1}"; }
...
if [[ "${OPENDECK_V225_HUMAN_VISUAL_APPROVED:-0}" == "1" ]]; then
...
PY
  then
    [[ "${OPENDECK_V225_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V225_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V225_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  exec bash "$SRC/scripts/activate-v225-qualified.sh" \
...
for candidate in \
  ...
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v225-node-modules-reuse-$$"
...
OPENDECK_V225_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" OPENDECK_V225_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" exec bash "$SRC/scripts/qualify-v225-host.sh"
