...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.23-LINT-QUALIFIED-REPLACEMENT-CUTOVER-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="f8a5c3b0ed0bb221733f1bbdbc9c7dfb351b51e1cdbb3ff56fae84a9ae2e0fda"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.23-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.23-qualified"
...
if [[ "${OPENDECK_V223_HUMAN_VISUAL_APPROVED:-0}" == "1" ]]; then
...
PY
  then
    [[ "${OPENDECK_V223_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V223_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V223_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  exec bash "$SRC/scripts/activate-v223-qualified.sh" \
...
for candidate in \
  ...
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v223-node-modules-reuse-$$"
...
OPENDECK_V223_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" OPENDECK_V223_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" exec bash "$SRC/scripts/qualify-v223-host.sh"
