...
DL="$HOME/Downloads"
ARCHIVE="$DL/OpenDeck-v2.0.22-TOUCH-STRIP-ACCESSIBILITY-NAME-CLOSURE-SOURCE.tar.xz"
EXPECTED_ARCHIVE_SHA="636d621fbeca8d6af2a79242f62ad528a2343178f7ca589d2898db751ee3a4cc"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
SRC="$DL/OpenDeck-v2.0.22-RENDER-PERFORMANCE-SOURCE"
STUDIO="$SRC/apps/opendeck-studio"
USER_BIN="$HOME/.local/bin"
INSTALL_ROOT="$HOME/.local/lib/opendeck-v2.0.22"
QUALIFIED_ROOT="$HOME/.local/lib/.opendeck-v2.0.22-qualified"
...
fail(){ printf 'OPENDECK_V2_0_22_RENDER_PERFORMANCE=FAIL:%s\nOPENDECK_FAILURE_STAGE=%s\n' "${2:-1}" "$1" >&2; exit "${2:-1}"; }
...
if [[ "${OPENDECK_V222_HUMAN_VISUAL_APPROVED:-0}" == "1" && -f "$STAMP" && -x "$QUALIFIED_ROOT/bin/opendeck-studio" ]]; then
...
PY2
  then
    [[ "${OPENDECK_V222_MEMORY_APPROVED:-0}" == "1" ]] || { printf 'OPENDECK_V222_MEMORY_TARGET=REQUIRES_USER_APPROVAL\nOPENDECK_V222_RSS_MAX_MIB=%s\n' "$stamp_rss"; exit 0; }
  fi
  ...
  chmod 600 "$ROLLBACK"
  stage="$HOME/.local/lib/.opendeck-v2.0.22-stage-$$"
  ...
  if [[ -d "$INSTALL_ROOT" ]]; then mv "$INSTALL_ROOT" "$HOME/.local/lib/opendeck-v2.0.22-previous-$(date +%Y%m%d-%H%M%S)"; fi
  ...
  {
    printf 'OPENDECK_V222_VISUAL_HUMAN_APPROVAL=PASS\n'
    printf 'OPENDECK_V222_INSTALL_ROOT=%s\n' "$INSTALL_ROOT"
    printf 'OPENDECK_V222_ACTIVE_BINARY_SHA256=%s\n' "$active_sha"
    printf 'OPENDECK_V222_BACKGROUND_SERVICES=%s\n' "$service_count"
    printf 'OPENDECK_V222_AUTOSTART_ENTRIES=%s\n' "$autostart_count"
    printf 'OPENDECK_V2_0_22_RENDER_PERFORMANCE_QUALIFY=PASS\n'
    printf 'OPENDECK_V2_0_22_RENDER_PERFORMANCE_ACTIVATE=PASS\n'
    printf 'OPENDECK_V2_0_22_RENDER_PERFORMANCE=PASS\n'
...
for candidate in \
  ...
  if [[ -d "$candidate" ]]; then
    NODE_MODULES_REUSE="$DL/.opendeck-v222-node-modules-reuse-$$"
...
OPENDECK_V222_ARCHIVE_SHA="$EXPECTED_ARCHIVE_SHA" OPENDECK_V222_NODE_MODULES_REUSE="$NODE_MODULES_REUSE" exec bash "$SRC/scripts/qualify-v222-host.sh"
