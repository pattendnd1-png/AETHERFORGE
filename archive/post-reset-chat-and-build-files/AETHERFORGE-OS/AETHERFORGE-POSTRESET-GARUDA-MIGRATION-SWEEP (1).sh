...
run_sweep() {
  ...
  trap 'if [[ ${had_autosync:-0} == 1 ]]; then systemctl --user start aetherforge-git-autosync.timer >/dev/null 2>&1 || true; fi' EXIT
  ...
  if [[ $had_autosync == 1 ]]; then systemctl --user start aetherforge-git-autosync.timer >/dev/null 2>&1 || true; fi
  trap - EXIT
