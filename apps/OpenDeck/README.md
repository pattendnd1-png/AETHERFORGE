# OpenDeck+ 2.0.41 — Plugin Property Inspector Hook Lint Closure

OpenDeck+ 2.0.41 is the narrow lint-closure successor to the failed 2.0.40 candidate and still builds on the fully qualified system-wide 2.0.38 baseline. It keeps the `/opt/opendeck-plus` installation, staged menu launch, rollback, and canonical system launcher model while expanding the application runtime.
...
The system-wide install remains versioned under `/opt/opendeck-plus/2.0.41`, with `/opt/opendeck-plus/current`, `/usr/local/bin/opendeck-studio`, the canonical `/usr/share/applications/opendeck-studio.desktop`, system icon, Stream Deck udev rule, rollback state, and uninstall helper. The current 2.0.38 versioned `/opt` tree is preserved until 2.0.41 passes the host gates.
...
The packaged source can be statically/model-tested in the build sandbox, but the Rust/Tauri, full frontend dependency tree, physical Stream Deck+, Wine/native third-party plugin behavior, and real KDE/system installation gates must pass on the target host before 2.0.41 is considered fully qualified.

## 2.0.41 closure

The 2.0.40 host run passed all source contracts and the complete frontend test suite, then stopped at FRONTEND_LINT_FIX because `PluginPropertyInspector.tsx` referenced the parsed plugin object inside a React effect while listing only its scalar fields as dependencies. ESLint `react-hooks/exhaustive-deps` therefore emitted one warning, and the zero-warning policy correctly failed the release. 2.0.41 derives stable `pluginUuid` and `actionUuid` scalars before the effect, uses only those scalars inside the effect, and adds a dedicated regression gate. Runtime plugin behavior is otherwise unchanged.
