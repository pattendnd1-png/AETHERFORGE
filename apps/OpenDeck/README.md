# OpenDeck v2.0.1 Customization Editor

OpenDeck is a clean-room Linux Stream Deck controller/editor. v2.0.1 advances the qualified v2.0.0 clean baseline into a compact Windows-style customization editor while preserving the existing OBS, Twitch, and Elgato Marketplace integration boundaries.
...
The old `opendeck-v2.keys` browser-storage key is read only as a one-time v2.0.0 migration input. It is deleted only after the migrated workspace is successfully written by the Rust backend. Browser localStorage is not the v2.0.1 source of truth.
...
OpenDeck v2.0.1 does not install or start a background OpenDeck daemon and does not enable autostart. Hardware/HID runtime remains deliberately separate from this editor qualification step; no public-stream action is triggered by build or install verification.
