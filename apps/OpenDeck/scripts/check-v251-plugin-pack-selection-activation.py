...
checks['ROOT_VERSION']=has('Cargo.toml','version = "2.0.51"')
checks['PACKAGE_VERSION']=json.loads((root/'apps/opendeck-studio/package.json').read_text()).get('version')=='2.0.51'
checks['TAURI_VERSION']=json.loads((root/'apps/opendeck-studio/src-tauri/tauri.conf.json').read_text()).get('version')=='2.0.51'
...
checks['QUALIFIER_GATE']=has('scripts/qualify-v251-host.sh','PLUGIN_PACK_SELECTION_ACTIVATION')
checks['QUALIFIER_UI_GATE']=has('scripts/qualify-v251-host.sh','PLUGIN_PACK_UI_TESTS')
failed=[name for name, ok in checks.items() if not ok]
for name, ok in checks.items(): print(f'{name}={"PASS" if ok else "FAIL"}')
if failed:
    print('OPENDECK_V251_PLUGIN_PACK_SELECTION_ACTIVATION=FAIL:'+','.join(failed))
    sys.exit(1)
print('OPENDECK_V251_PLUGIN_PACK_SELECTION_ACTIVATION=PASS')
