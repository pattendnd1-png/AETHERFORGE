...
add('ROOT_VERSION', has('Cargo.toml','version = "2.0.39"'))
add('PACKAGE_VERSION', json.loads((root/'apps/opendeck-studio/package.json').read_text()).get('version')=='2.0.39')
conf=json.loads((root/'apps/opendeck-studio/src-tauri/tauri.conf.json').read_text())
window=conf['app']['windows'][0]
add('TAURI_VERSION', conf.get('version')=='2.0.39')
...
add('SYSTEM_STAGE', (root/'scripts/stage-v239-system-wide.sh').is_file())
add('SYSTEM_ACTIVATE', (root/'scripts/activate-v239-system-wide.sh').is_file())
add('SYSTEM_ROLLBACK', (root/'scripts/rollback-v239-system-wide.sh').is_file())
add('SYSTEM_UNINSTALL', (root/'scripts/uninstall-v239-system-wide.sh').is_file())
failed=[name for name,ok in checks.items() if not ok]
for name,ok in checks.items(): print(f'{name}={"PASS" if ok else "FAIL"}')
if failed:
    print('OPENDECK_V239_FULL_PARITY_CONTRACT=FAIL:'+','.join(failed))
    sys.exit(1)
print('OPENDECK_V239_FULL_PARITY_CONTRACT=PASS')
