...
checks = {
    ...
    'QUALIFIER_V226_LABEL': 'OPENDECK_V2_0_26_RENDER_PERFORMANCE=START' in qualifier,
    'DIAL_STACKS_V227': 'OPENDECK_DIAL_STACKS=DEFERRED_TO_2.0.27' in qualifier,
}
for name, ok in checks.items():
    print(f'OPENDECK_V226_ADAPTIVE_TOUCH_{name}={"PASS" if ok else "FAIL"}')
if not all(checks.values()):
    raise SystemExit(1)
print('OPENDECK_V226_ADAPTIVE_TOUCH_STRIP=PASS')
