...
qualifier = (root / 'scripts/qualify-v215-host.sh').read_text()
checks = {
    ...
    'QUALIFIER_USES_V215_RELEASE_LABEL': 'OPENDECK_V2_0_15_RENDER_PERFORMANCE' in qualifier,
    ...
    'QUALIFIER_TARGETS_V215_SCREENSHOT': 'OpenDeck-v2.0.15-QUALIFICATION.png' in qualifier,
    'PRODUCTION_TAURI_PREVIEW': 'gate "PREVIEW_TAURI_BUILD"' in qualifier and 'npm run tauri -- build --no-bundle' in qualifier,
}
for key, ok in checks.items(): print(f'OPENDECK_V215_FINAL_CLOSURE_{key}={"PASS" if ok else "FAIL"}')
if not all(checks.values()): sys.exit(1)
print('OPENDECK_V215_FINAL_CLOSURE=PASS')
