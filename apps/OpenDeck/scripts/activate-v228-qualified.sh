...
fail(){ say "OPENDECK_V228_ACTIVATION=FAIL:${1}"; exit "${2:-1}"; }
...
for link_name in opendeck-studio opendeck; do
  tmp_link="$USER_BIN/.${link_name}.v228-$$"
  rm -f -- "$tmp_link"
...
tmp_desktop="$DESKTOP_DIR/.opendeck-studio.desktop.v228-$$"
cat > "$tmp_desktop" <<EOF
[Desktop Entry]
Type=Application
Name=OpenDeck+
...
say "OPENDECK_V228_REPLACEMENT_INSTALL=PASS"
say "OPENDECK_V228_INSTALL_ROOT=$INSTALL_ROOT"
say "OPENDECK_V228_ROLLBACK_ROOT=$ROLLBACK_ROOT"
say "OPENDECK_V228_ACTIVE_BINARY_SHA256=$(sha "$USER_BIN/opendeck-studio")"
say "OPENDECK_V228_DESKTOP_ENTRY=$DESKTOP_FILE"
say "OPENDECK_V228_ICON=$ICON_DEST"
