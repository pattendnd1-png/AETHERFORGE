...
tmp_current="$OPT_ROOT/.current.v254-$$"
rm -f -- "$tmp_current"
...
for pair in \
  ...
  tmp="$LOCAL_BIN_DIR/.$(basename "$dest").v254-$$"
  rm -f -- "$tmp"
  ln -s "$target" "$tmp"
  mv -Tf "$tmp" "$dest"
done
...
say "OPENDECK_V254_INSTALL_ROOT=$INSTALL_ROOT"
say "OPENDECK_V254_CURRENT_LINK=$CURRENT_LINK"
say "OPENDECK_V254_SYSTEM_COMMAND=$SYSTEM_BIN"
say "OPENDECK_V254_SYSTEM_DESKTOP=$SYSTEM_DESKTOP"
say "OPENDECK_V254_ACTIVE_BINARY_SHA256=$(sha "$SYSTEM_BIN")"
say "OPENDECK_V254_ROLLBACK_STATE=$ROLLBACK_DIR"
say "OPENDECK_V254_ROLLBACK_FILE=$ROLLBACK_FILE"
say "OPENDECK_V254_BACKGROUND_SERVICES=NONE"
say "OPENDECK_V254_AUTOSTART=DISABLED"
