...
fail(){ say "OPENDECK_V237_ACTIVATION=FAIL:${1}"; exit "${2:-1}"; }
...
tmp_desktop="$DESKTOP_DIR/.opendeck-studio.v237-$$.desktop"
cat > "$tmp_desktop" <<EOF
[Desktop Entry]
Type=Application
Name=OpenDeck+
...
tmp_legacy="$DESKTOP_DIR/.opendeck.v237-$$.desktop"
cat > "$tmp_legacy" <<EOF
[Desktop Entry]
Type=Application
Name=OpenDeck
...
for link_name in opendeck-studio opendeck; do
  tmp_link="$USER_BIN/.${link_name}.v237-$$"
  rm -f -- "$tmp_link"
...
say "OPENDECK_V237_REPLACEMENT_INSTALL=PASS"
say "OPENDECK_V237_INSTALL_ROOT=$INSTALL_ROOT"
say "OPENDECK_V237_PREVIOUS_ACTIVE_TARGET=$previous_target"
say "OPENDECK_V237_PREVIOUS_ACTIVE_SHA256=$previous_sha"
say "OPENDECK_V237_ACTIVE_BINARY_SHA256=$(sha "$USER_BIN/opendeck-studio")"
say "OPENDECK_V237_ROLLBACK_FILE=$ROLLBACK_FILE"
say "OPENDECK_V237_BACKGROUND_SERVICES=NONE"
say "OPENDECK_V237_AUTOSTART=DISABLED"
