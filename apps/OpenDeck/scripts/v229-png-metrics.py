#!/usr/bin/env python3
"""Broad-structure PNG comparison for the OpenDeck 2.0.29 human-reviewed visual gate.
...
def main():
    if len(sys.argv)!=3:
        print('usage: v229-png-metrics.py CANONICAL.png ACTUAL.png',file=sys.stderr); return 2
    a=block_average(read_png(sys.argv[1])); b=block_average(read_png(sys.argv[2])); cs=color_score(a,b); es=edge_score(a,b)
    print(f'OPENDECK_V229_COLOR_SIMILARITY={cs:.6f}')
    print(f'OPENDECK_V229_EDGE_SIMILARITY={es:.6f}')
    passed=cs>=.82 and es>=.93
    print('OPENDECK_V229_SCREENSHOT_SIMILARITY='+('PASS' if passed else 'FAIL'))
