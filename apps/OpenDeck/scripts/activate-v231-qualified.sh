...
fail(){ say "OPENDECK_V231_ACTIVATION=FAIL:${1}"; exit "${2:-1}"; }
...
for link_name in opendeck-studio opendeck; do
  tmp_link="$USER_BIN/.${link_name}.v231-$$"
  rm -f -- "$tmp_link"
...
tmp_desktop="$DESKTOP_DIR/.opendeck-studio.desktop.v231-$$"
cat > "$tmp_desktop" <<EOF
[Desktop Entry]
Type=Application
Name=OpenDeck+
...
say "OPENDECK_V231_REPLACEMENT_INSTALL=PASS"
say "OPENDECK_V231_INSTALL_ROOT=$INSTALL_ROOT"
say "OPENDECK_V231_PREVIOUS_ACTIVE_TARGET=$previous_target"
say "OPENDECK_V231_PREVIOUS_ACTIVE_SHA256=$previous_sha"
say "OPENDECK_V231_ACTIVE_BINARY_SHA256=$(sha "$USER_BIN/opendeck-studio")"
say "OPENDECK_V231_ROLLBACK_FILE=$ROLLBACK_FILE"
say "OPENDECK_V231_BACKGROUND_SERVICES=NONE"
say "OPENDECK_V231_AUTOSTART=DISABLED"
