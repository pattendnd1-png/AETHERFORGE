...
results.append(check('VERSION_FILE', (root/'VERSION').read_text().strip()=='2.0.53' if (root/'VERSION').is_file() else False))
...
if not all(results):
    raise SystemExit(1)
print(f'OPENDECK_V253_INSTALLED_TREE_{mode.upper()}=PASS')
