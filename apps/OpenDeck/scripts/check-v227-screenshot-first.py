...
runner = Path('scripts/qualify-v227-host.sh')
if not runner.exists():
    raise SystemExit('QUALIFIER_ORDER=FAIL:qualify-v227-host.sh missing')
text = runner.read_text()
required = [
    'gate "PREVIEW_TAURI_BUILD"',
    'say "OPENDECK_V227_PREVIEW_SCREENSHOT=START"',
    'capture-v227-window.sh',
    'say "OPENDECK_V227_PREVIEW_SCREENSHOT=PASS:',
    'gate "CARGO_FMT"',
...
if not (pos['gate "PREVIEW_TAURI_BUILD"'] < pos['say "OPENDECK_V227_PREVIEW_SCREENSHOT=START"'] < pos['capture-v227-window.sh'] < pos['say "OPENDECK_V227_PREVIEW_SCREENSHOT=PASS:'] < pos['gate "CARGO_FMT"'] < pos['gate "CARGO_CLIPPY_STRICT"']):
...
preview_pass = text.index('say \"OPENDECK_V227_PREVIEW_SCREENSHOT=PASS:')
...
print('OPENDECK_V227_SCREENSHOT_FIRST_CONTRACT=PASS')
