...
required_files = [
    ...
    studio / 'src-tauri/src/streamdeck/protocol.rs',
    studio / 'src-tauri/src/streamdeck/render.rs',
    studio / 'src-tauri/src/streamdeck/runtime.rs',
    frontend_root / 'app/hardware-events.ts',
    root / 'packaging/70-opendeck-streamdeck.rules',
    root / 'scripts/check-streamdeck-plus.sh',
...
checks = {
    ...
    'OPENDECK_V207_WINDOWS_SHELL': all(marker in css for marker in ['grid-template-areas:"editor divider actions"', '--action-panel-width', '--inspector-height', 'action-resize-handle', 'inspector-resize-handle', '.device-surface', '.action-mode-tabs', '.configuration-strip', '.editor-toast']) and all(marker in app for marker in ['actionPanelCollapsed', 'inspectorCollapsed', 'UPDATE_PREFERENCES', 'hardwareStatus', 'actionMode', 'editor-toast']),
    'OPENDECK_V207_DIALS_ACTION_FILTER': "supportsControl(item.id, 'dial') || supportsControl(item.id, 'touch')" not in (frontend_root / 'components/ActionLibrary.tsx').read_text() and "supportsControl(item.id, 'dial');" in (frontend_root / 'components/ActionLibrary.tsx').read_text(),
