...
checks = {
    'OPENDECK_V205_VERSION': (
        pkg.get('version') == '2.0.5'
        and tauri.get('version') == '2.0.5'
        and 'version = "2.0.5"' in root_cargo
        and 'version = "2.0.5"' in app_cargo
    ),
    ...
    'OPENDECK_V203_WINDOWS_SHELL': all(marker in css for marker in ['grid-template-areas:"editor divider actions"', '--action-panel-width', '--inspector-height', 'action-resize-handle', 'inspector-resize-handle']) and all(marker in app for marker in ['actionPanelCollapsed', 'inspectorCollapsed', 'UPDATE_PREFERENCES', 'hardwareStatus']),
...
if bad:
    print('OPENDECK_V2_0_5_SOURCE_CONTRACT=FAIL:' + ','.join(bad))
    sys.exit(1)

print('OPENDECK_V2_0_5_SOURCE_CONTRACT=PASS')
