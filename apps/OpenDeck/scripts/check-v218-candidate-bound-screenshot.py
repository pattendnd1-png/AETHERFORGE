...
host = (root/'scripts/qualify-v218-host.sh').read_text() if (root/'scripts/qualify-v218-host.sh').exists() else ''
capture = (root/'scripts/capture-v218-window.sh').read_text() if (root/'scripts/capture-v218-window.sh').exists() else ''
...
required = {
    ...
    'rust focus ack': 'OpenDeck-v2.0.18-FOCUS-ACK.json' in qual and '"release": "2.0.18"' in qual,
    ...
    'host validates visual release': 'validate_visual_release' in host and '2.0.18' in host,
...
if missing:
    raise SystemExit('OPENDECK_V218_CANDIDATE_BOUND_SCREENSHOT=FAIL:' + ','.join(missing))
print('OPENDECK_V218_CANDIDATE_BOUND_SCREENSHOT=PASS')
