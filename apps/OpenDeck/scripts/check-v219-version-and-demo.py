...
if package.get('version')!='2.0.19': errors.append('package version')
cargo=(root/'apps/opendeck-studio/src-tauri/Cargo.toml').read_text()
if not re.search(r'^version = "2\.0\.19"$', cargo, re.M): errors.append('cargo version')
tauri=json.loads((root/'apps/opendeck-studio/src-tauri/tauri.conf.json').read_text())
if tauri.get('version')!='2.0.19': errors.append('tauri version')
...
if errors:
    print('OPENDECK_V219_VERSION_DEMO_CONTRACT=FAIL')
    for e in errors: print('MISSING='+e)
    sys.exit(1)
print('OPENDECK_V219_VERSION_DEMO_CONTRACT=PASS')
