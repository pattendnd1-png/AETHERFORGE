...
q=root/'scripts/qualify-v227-host.sh'
...
for k,v in checks.items(): print(f'OPENDECK_V227_PRODUCTION_TAURI_{k}={"PASS" if v else "FAIL"}')
failed=[k for k,v in checks.items() if not v]
if failed:
 print('OPENDECK_V227_PRODUCTION_TAURI_PREVIEW_CONTRACT=FAIL:'+','.join(failed))
 raise SystemExit(1)
print('OPENDECK_V227_PRODUCTION_TAURI_PREVIEW_CONTRACT=PASS')
