...
checks = {
    ...
    'QUALIFIER_USES_V218_RELEASE_LABEL': 'OPENDECK_V2_0_18_RENDER_PERFORMANCE' in qualifier,
    ...
    'QUALIFIER_TARGETS_V218_SCREENSHOT': 'OpenDeck-v2.0.18-QUALIFICATION.png' in qualifier,
    'PRODUCTION_TAURI_PREVIEW': 'gate "PREVIEW_TAURI_BUILD"' in qualifier and 'npm run tauri -- build --no-bundle' in qualifier,
    'VISUAL_PARITY_CONTRACT': 'check-v218-visual-parity-closure.py' in qualifier,
    'SCREENSHOT_BEFORE_TESTS_CONTRACT': 'check-v218-screenshot-before-tests.py' in qualifier,
    'DIAL_STACKS_DEFERRED_TO_V219': 'OPENDECK_DIAL_STACKS=DEFERRED_TO_2.0.19' in qualifier,
    'CANDIDATE_BOUND_SCREENSHOT_CONTRACT': 'check-v218-candidate-bound-screenshot.py' in qualifier,
}
for key, ok in checks.items(): print(f'OPENDECK_V218_FINAL_CLOSURE_{key}={"PASS" if ok else "FAIL"}')
if not all(checks.values()): sys.exit(1)
print('OPENDECK_V218_FINAL_CLOSURE=PASS')
