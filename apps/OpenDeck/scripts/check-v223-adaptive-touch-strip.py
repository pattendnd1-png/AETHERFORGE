...
checks = {
    ...
    'QUALIFIER_V223_LABEL': 'OPENDECK_V2_0_23_RENDER_PERFORMANCE=START' in qualifier,
    'DIAL_STACKS_V224': 'OPENDECK_DIAL_STACKS=DEFERRED_TO_2.0.24' in qualifier,
}
for name, ok in checks.items():
    print(f'OPENDECK_V223_ADAPTIVE_TOUCH_{name}={"PASS" if ok else "FAIL"}')
if not all(checks.values()):
    raise SystemExit(1)
print('OPENDECK_V223_ADAPTIVE_TOUCH_STRIP=PASS')
