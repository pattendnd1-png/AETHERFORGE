...
checks = {
    ...
    'NO_VERSIONED_QUALIFICATION_ENV_IN_RUNTIME': 'OPENDECK_V244_QUALIFICATION' not in qualification and 'OPENDECK_V245_QUALIFICATION' not in qualification and 'OPENDECK_V255_QUALIFICATION' not in qualification,
    'QUALIFIER_EXPORTS_STABLE_ENV': 'OPENDECK_QUALIFICATION=1 OPENDECK_QUALIFICATION_PHASE=visual' in qualifier,
    'QUALIFIER_DOES_NOT_EXPORT_VERSIONED_ENV': 'OPENDECK_V255_QUALIFICATION=1' not in qualifier,
    'QUALIFICATION_HIDES_NATIVE_STARTUP': 'if crate::qualification::enabled()' in startup and 'return Ok(());' in startup,
    'HARNESS_RELEASE_MATCHES': "release: '2.0.55'" in harness and "focusAck.release !== '2.0.55'" in harness,
    'QUALIFIER_VISUAL_FILE_MATCHES': 'OpenDeck-v2.0.55-VISUAL-METRICS.json' in qualifier,
}
failed = [name for name, ok in checks.items() if not ok]
if failed:
    raise SystemExit('OPENDECK_V255_QUALIFICATION_ENV_CLOSURE=FAIL:' + ','.join(failed))
for name in checks:
    print(f'OPENDECK_V255_{name}=PASS')
print('OPENDECK_V255_QUALIFICATION_ENV_CLOSURE=PASS')
