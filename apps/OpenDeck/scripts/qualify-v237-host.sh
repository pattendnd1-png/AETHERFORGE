...
DL="$HOME/Downloads"
LOGDIR="$DL/OpenDeck-v2.0.37-logs"
VERIFY="$DL/OpenDeck-v2.0.37-VERIFY.txt"
ROLLBACK="$DL/OpenDeck-v2.0.37-ROLLBACK.txt"
QDIR="$DL/OpenDeck-v2.0.37-qualification"
VISUAL_JSON="$QDIR/OpenDeck-v2.0.37-VISUAL-METRICS.json"
FOCUS_ACK="$QDIR/OpenDeck-v2.0.37-FOCUS-ACK.json"
SCREENSHOT="$DL/OpenDeck-v2.0.37-QUALIFICATION.png"
NEW_BIN="$ROOT/target/release/opendeck-studio"
ARCHIVE_SHA="${OPENDECK_V237_ARCHIVE_SHA:-UNSET}"

mkdir -p "$LOGDIR" "$QDIR"
: > "$VERIFY"
say(){ printf '%s\n' "$*" | tee -a "$VERIFY"; }
fail(){ local stage="$1" rc="${2:-1}"; say "OPENDECK_V2_0_37=FAIL:$rc"; say "OPENDECK_FAILURE_STAGE=$stage"; say "VERIFY_FILE=$VERIFY"; exit "$rc"; }
gate(){ local name="$1" log="$2"; shift 2; say "OPENDECK_V237_STAGE=${name}:START"; if "$@" >"$log" 2>&1; then say "OPENDECK_V237_STAGE=${name}:PASS"; else local rc=$?; say "OPENDECK_V237_STAGE=${name}:FAIL:$rc"; tail -700 "$log" | tee -a "$VERIFY"; fail "$name" "$rc"; fi; }

say "OPENDECK_V2_0_37=START"
say "OPENDECK_RELEASE=DESKTOP_ENTRY_ACTIVATION_CLOSURE"
...
gate "DESKTOP_STAGE_EXTENSION" "$LOGDIR/desktop-stage-extension.log" bash -lc "cd '$ROOT' && python3 scripts/check-v237-desktop-stage-extension.py"
gate "EVENT_LOOP_STARTUP_CLOSURE" "$LOGDIR/event-loop-startup-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v237-event-loop-startup-closure.py"
gate "TAURI_WINDOW_PERMISSION_CLOSURE" "$LOGDIR/tauri-window-permission-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v237-tauri-window-permission-closure.py"
gate "VISIBLE_STARTUP_KDE_LAUNCHER_CONTRACT" "$LOGDIR/visible-startup-kde-launcher-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v237-visible-startup-kde-launcher.py"
gate "FRONTEND_TEST_CLOSURE" "$LOGDIR/frontend-test-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v237-frontend-test-closure.py"
gate "ACTION_WHEEL_CONTRACT" "$LOGDIR/action-wheel-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v237-action-wheel-contract.py"
gate "ACTION_WHEEL_MODEL" "$LOGDIR/action-wheel-model.log" bash -lc "cd '$ROOT' && node --experimental-strip-types scripts/test-v237-action-wheel.mjs"
gate "DIAL_STACK_PRESERVATION" "$LOGDIR/dial-stack-preservation.log" bash -lc "cd '$ROOT' && python3 scripts/check-v237-dial-stack-preservation.py"
gate "LAYOUT_CONTRACT" "$LOGDIR/layout-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v237-layout-contract.py"
...
if [[ ! -d "$STUDIO/node_modules" ]]; then
  for candidate in \
    ...
    if [[ -d "$candidate" ]]; then
      say "OPENDECK_V237_NODE_MODULES_REUSE_SOURCE=$candidate"
      if cp -al "$candidate" "$STUDIO/node_modules" 2>"$LOGDIR/node-modules-reuse.log"; then
        say "OPENDECK_V237_NODE_MODULES_REUSE=PASS"
      else
...
if [[ ! -d "$STUDIO/node_modules" ]]; then
  ...
  say "OPENDECK_V237_NPM_INSTALL=REUSED"
fi
...
say "OPENDECK_V237_NEW_BINARY_SHA256=$NEW_SHA"
...
OPENDECK_V237_QUALIFICATION=1 OPENDECK_V237_QUALIFICATION_PHASE=visual OPENDECK_QUALIFICATION_DIR="$QDIR" "$NEW_BIN" >"$LOGDIR/candidate-visual.log" 2>&1 &
pid=$!
...
gate "VISUAL_GEOMETRY" "$LOGDIR/visual-geometry.log" python3 "$ROOT/scripts/check-v237-visual-metrics.py" "$VISUAL_JSON"
if command -v spectacle >/dev/null 2>&1; then
  if "$ROOT/scripts/capture-v229-window.sh" "$SCREENSHOT" "$pid" "2.0.37" "$FOCUS_ACK" >"$LOGDIR/screenshot.log" 2>&1; then
    say "OPENDECK_V237_SCREENSHOT=PASS:$SCREENSHOT"
  else
    say "OPENDECK_V237_SCREENSHOT=SKIP_CAPTURE_FAILED"
    tail -80 "$LOGDIR/screenshot.log" | tee -a "$VERIFY" || true
  fi
else
  say "OPENDECK_V237_SCREENSHOT=SKIP:SPECTACLE_MISSING"
fi
stop_candidate "$pid"

# Prove normal desktop-ID launch produces a visible window before switching the active alias.
gate "MENU_LAUNCH" "$LOGDIR/menu-launch.log" bash "$ROOT/scripts/check-v237-menu-launch.sh" "$NEW_BIN"

# Only now replace the launch symlink. Existing OpenDeck processes are not killed.
gate "ACTIVATION" "$LOGDIR/activation.log" bash "$ROOT/scripts/activate-v237-qualified.sh" "$NEW_BIN" "$NEW_SHA" "$ROOT" "$ROLLBACK" "$VERIFY"
say "OPENDECK_V237_ACTION_WHEEL=PASS"
say "OPENDECK_V237_DIAL_STACKS_PRESERVED=PASS"
say "OPENDECK_V237_FULL_HOST_QUALIFICATION=PASS"
say "OPENDECK_V2_0_37=PASS"
say "VERIFY_FILE=$VERIFY"
say "OPENDECK_V237_NOTE=Launch_OpenDeck_when_ready_to_run_the_new_binary"
