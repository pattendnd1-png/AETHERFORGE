...
for marker in required_app:
    if marker not in app:
        raise SystemExit(f'OPENDECK_V228_CANONICAL_FIT=FAIL:APP_MISSING:{marker}')

for marker in [
    'appMaximize: () => getCurrentWindow().maximize()',
]:
    if marker not in bridge:
        raise SystemExit(f'OPENDECK_V228_CANONICAL_FIT=FAIL:BRIDGE_MISSING:{marker}')

if "if (!qualification.enabled)" not in main or 'bridge.appMaximize()' not in main:
    raise SystemExit('OPENDECK_V228_CANONICAL_FIT=FAIL:NORMAL_MODE_MAXIMIZE_MISSING')
...
for marker in required_css:
    if marker not in css:
        raise SystemExit(f'OPENDECK_V228_CANONICAL_FIT=FAIL:CSS_MISSING:{marker}')

if re.search(r'@media\s*\(max-width:\s*(?:1380|1180)px\)', css):
    raise SystemExit('OPENDECK_V228_CANONICAL_FIT=FAIL:LEGACY_REFLOW_MEDIA_QUERY_PRESENT')

if 'min-width:1100px' in css or 'min-height:720px' in css:
    raise SystemExit('OPENDECK_V228_CANONICAL_FIT=FAIL:ROOT_MINIMUM_BLOCKS_SMALL_VIEWPORT')


tauri = (root/'apps/opendeck-studio/src-tauri/tauri.conf.json').read_text()
if '"minWidth": 1100' in tauri or '"minHeight": 720' in tauri:
    raise SystemExit('OPENDECK_V228_CANONICAL_FIT=FAIL:TAURI_LEGACY_MINIMUM_BLOCKS_SMALL_DISPLAY')
for marker in ['"minWidth": 640', '"minHeight": 480']:
    if marker not in tauri:
        raise SystemExit(f'OPENDECK_V228_CANONICAL_FIT=FAIL:TAURI_WINDOW_FIT_MISSING:{marker}')

# Qualification mode must stay 1:1 for pixel parity.
if 'qualificationMode ? 1 : fitScale' not in app:
    raise SystemExit('OPENDECK_V228_CANONICAL_FIT=FAIL:QUALIFICATION_NOT_LOCKED_1X')

print('OPENDECK_V228_CANONICAL_FIT=PASS')
