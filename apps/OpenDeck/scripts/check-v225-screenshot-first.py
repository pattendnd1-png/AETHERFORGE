...
runner = Path('scripts/qualify-v225-host.sh')
if not runner.exists():
    raise SystemExit('QUALIFIER_ORDER=FAIL:qualify-v225-host.sh missing')
text = runner.read_text()
required = [
    'gate "PREVIEW_TAURI_BUILD"',
    'say "OPENDECK_V225_PREVIEW_SCREENSHOT=START"',
    'capture-v225-window.sh',
    'say "OPENDECK_V225_PREVIEW_SCREENSHOT=PASS:',
    'gate "CARGO_FMT"',
...
if not (pos['gate "PREVIEW_TAURI_BUILD"'] < pos['say "OPENDECK_V225_PREVIEW_SCREENSHOT=START"'] < pos['capture-v225-window.sh'] < pos['say "OPENDECK_V225_PREVIEW_SCREENSHOT=PASS:'] < pos['gate "CARGO_FMT"'] < pos['gate "CARGO_CLIPPY_STRICT"']):
...
preview_pass = text.index('say \"OPENDECK_V225_PREVIEW_SCREENSHOT=PASS:')
...
print('OPENDECK_V225_SCREENSHOT_FIRST_CONTRACT=PASS')
