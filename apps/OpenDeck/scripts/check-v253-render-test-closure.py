...
for item in need:
    if item not in s:
        raise SystemExit(f'OPENDECK_V253_RENDER_TEST_CLOSURE=FAIL:missing:{item}')
for forbidden in ['pixel.0[0] > 150 && pixel.0[1] > 150 && pixel.0[2] > 150', 'bright_upper_pixels']:
    if forbidden in s:
        raise SystemExit(f'OPENDECK_V253_RENDER_TEST_CLOSURE=FAIL:stale:{forbidden}')
print('OPENDECK_V253_RENDER_TEST_CLOSURE=PASS')
