...
for name, ok in checks.items():
    print(f'OPENDECK_V222_VISUAL_PARITY_{name}={"PASS" if ok else "FAIL"}')
if failed:
    print('OPENDECK_V222_VISUAL_PARITY_CLOSURE=FAIL:' + ','.join(failed))
    raise SystemExit(1)
print('OPENDECK_V222_VISUAL_PARITY_CLOSURE=PASS')
