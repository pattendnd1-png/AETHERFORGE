...
add('ROOT_VERSION', has('Cargo.toml','version = "2.0.47"'))
add('PACKAGE_VERSION', json.loads((root/'apps/opendeck-studio/package.json').read_text()).get('version')=='2.0.47')
conf=json.loads((root/'apps/opendeck-studio/src-tauri/tauri.conf.json').read_text())
window=conf['app']['windows'][0]
add('TAURI_VERSION', conf.get('version')=='2.0.47')
...
add('PLUGIN_HOST_RUST_COMPILE_CLOSURE', (root/'scripts/check-v247-plugin-host-rust-compile-closure.py').is_file() and has('scripts/qualify-v247-host.sh','PLUGIN_HOST_RUST_COMPILE_CLOSURE'))
add('PLUGIN_HOST_STRICT_CLIPPY_CLOSURE', (root/'scripts/check-v247-plugin-host-strict-clippy-closure.py').is_file() and has('scripts/qualify-v247-host.sh','gate "PLUGIN_HOST_STRICT_CLIPPY_CLOSURE"'))
add('QUALIFICATION_ENV_CLOSURE', (root/'scripts/check-v247-qualification-env-closure.py').is_file() and has('scripts/qualify-v247-host.sh','gate "QUALIFICATION_ENV_CLOSURE"'))
add('FLUID_WINDOW_FILL_CLOSURE', (root/'scripts/check-v247-fluid-window-fill-closure.py').is_file() and has('scripts/qualify-v247-host.sh','gate "FLUID_WINDOW_FILL_CLOSURE"'))
add('SYSTEM_STAGE', (root/'scripts/stage-v247-system-wide.sh').is_file())
add('SYSTEM_ACTIVATE', (root/'scripts/activate-v247-system-wide.sh').is_file())
add('SYSTEM_ROLLBACK', (root/'scripts/rollback-v247-system-wide.sh').is_file())
add('SYSTEM_UNINSTALL', (root/'scripts/uninstall-v247-system-wide.sh').is_file())
failed=[name for name,ok in checks.items() if not ok]
for name,ok in checks.items(): print(f'{name}={"PASS" if ok else "FAIL"}')
if failed:
    print('OPENDECK_V247_FULL_PARITY_CONTRACT=FAIL:'+','.join(failed))
    sys.exit(1)
print('OPENDECK_V247_FULL_PARITY_CONTRACT=PASS')
