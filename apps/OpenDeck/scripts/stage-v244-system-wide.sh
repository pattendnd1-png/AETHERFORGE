...
ROLLBACK_SOURCE="$SOURCE_ROOT/scripts/rollback-v244-system-wide.sh"
UNINSTALL_SOURCE="$SOURCE_ROOT/scripts/uninstall-v244-system-wide.sh"

say(){ printf '%s\n' "$*"; [[ -n "$VERIFY_FILE" ]] && printf '%s\n' "$*" >> "$VERIFY_FILE" || true; }
fail(){ say "OPENDECK_V244_SYSTEM_STAGE=FAIL:${1}"; exit "${2:-1}"; }
...
say "OPENDECK_V244_SYSTEM_STAGE=PASS"
say "OPENDECK_V244_SYSTEM_INSTALL_ROOT=$INSTALL_ROOT"
say "OPENDECK_V244_SYSTEM_BINARY_SHA256=$(sha "$INSTALL_ROOT/bin/opendeck-studio")"
say "OPENDECK_V244_SYSTEM_MANIFEST=$INSTALL_ROOT/manifest/INSTALL-MANIFEST.txt"
say "OPENDECK_V244_SYSTEM_SHA256SUMS=$INSTALL_ROOT/manifest/SHA256SUMS.txt"
