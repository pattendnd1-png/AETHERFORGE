...
host = (root/'scripts/qualify-v221-host.sh').read_text() if (root/'scripts/qualify-v221-host.sh').exists() else ''
capture = (root/'scripts/capture-v221-window.sh').read_text() if (root/'scripts/capture-v221-window.sh').exists() else ''
...
required = {
    ...
    'rust focus ack': 'OpenDeck-v2.0.21-FOCUS-ACK.json' in qual and '"release": "2.0.21"' in qual,
    ...
    'host validates visual release': 'validate_visual_release' in host and '2.0.21' in host,
...
if missing:
    raise SystemExit('OPENDECK_V221_CANDIDATE_BOUND_SCREENSHOT=FAIL:' + ','.join(missing))
print('OPENDECK_V221_CANDIDATE_BOUND_SCREENSHOT=PASS')
