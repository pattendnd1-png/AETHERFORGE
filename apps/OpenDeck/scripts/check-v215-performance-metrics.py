...
path=Path(sys.argv[1]) if len(sys.argv)>1 else Path.home()/'Downloads/OpenDeck-v2.0.15-PERFORMANCE-METRICS.json'
if not path.exists():
    print(f'OPENDECK_V215_PERFORMANCE_METRICS=FAIL:MISSING:{path}'); raise SystemExit(2)
...
if hid_r and hid_u:
    ...
    print(f'OPENDECK_V215_HID_DECODE_VISIBLE_ESTIMATE_P95_MS={end95:.3f}')
    print(f'OPENDECK_V215_HID_DECODE_VISIBLE_ESTIMATE_P99_MS={end99:.3f}')
...
if total:
    ...
    print(f'OPENDECK_V215_FRAME_WITHIN_16_7_RATIO={r16:.6f}')
    print(f'OPENDECK_V215_FRAME_WITHIN_33_3_RATIO={r33:.6f}')
...
print(f'OPENDECK_V215_COLD_START_MS={cold:.3f}')
print(f'OPENDECK_V215_WARM_START_P95_MS={warm95:.3f}')
...
print(f'OPENDECK_V215_IDLE_CPU_PERCENT={idle_cpu:.3f}')
print(f'OPENDECK_V215_RSS_MAX_MIB={rss:.3f}')
if idle_cpu>1.5: errors.append(f'idleCpu={idle_cpu:.3f}>1.5')
if rss>300: print('OPENDECK_V215_MEMORY_TARGET=REQUIRES_USER_APPROVAL')
else: print('OPENDECK_V215_MEMORY_TARGET=PASS')

if errors:
    print('OPENDECK_V215_PERFORMANCE_METRICS=FAIL')
    for error in errors: print('PERFORMANCE_ERROR='+error)
    raise SystemExit(1)
print('OPENDECK_V215_PERFORMANCE_METRICS=PASS')
