...
DL="$HOME/Downloads"
LOGDIR="$DL/OpenDeck-v2.0.30-logs"
VERIFY="$DL/OpenDeck-v2.0.30-VERIFY.txt"
ROLLBACK="$DL/OpenDeck-v2.0.30-ROLLBACK.txt"
QDIR="$DL/OpenDeck-v2.0.30-qualification"
VISUAL_JSON="$QDIR/OpenDeck-v2.0.30-VISUAL-METRICS.json"
FOCUS_ACK="$QDIR/OpenDeck-v2.0.30-FOCUS-ACK.json"
SCREENSHOT="$DL/OpenDeck-v2.0.30-QUALIFICATION.png"
NEW_BIN="$ROOT/target/release/opendeck-studio"
ARCHIVE_SHA="${OPENDECK_V230_ARCHIVE_SHA:-UNSET}"

mkdir -p "$LOGDIR" "$QDIR"
: > "$VERIFY"
say(){ printf '%s\n' "$*" | tee -a "$VERIFY"; }
fail(){ local stage="$1" rc="${2:-1}"; say "OPENDECK_V2_0_30=FAIL:$rc"; say "OPENDECK_FAILURE_STAGE=$stage"; say "VERIFY_FILE=$VERIFY"; exit "$rc"; }
gate(){ local name="$1" log="$2"; shift 2; say "OPENDECK_V230_STAGE=${name}:START"; if "$@" >"$log" 2>&1; then say "OPENDECK_V230_STAGE=${name}:PASS"; else local rc=$?; say "OPENDECK_V230_STAGE=${name}:FAIL:$rc"; tail -240 "$log" | tee -a "$VERIFY"; fail "$name" "$rc"; fi; }

say "OPENDECK_V2_0_30=START"
say "OPENDECK_RELEASE=DIAL_STACKS"
...
gate "DIAL_STACK_CONTRACT" "$LOGDIR/dial-stack-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v230-dial-stack-contract.py"
gate "LAYOUT_CONTRACT" "$LOGDIR/layout-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v230-layout-contract.py"
...
if [[ ! -d "$STUDIO/node_modules" ]]; then
  for candidate in \
    ...
    if [[ -d "$candidate" ]]; then
      say "OPENDECK_V230_NODE_MODULES_REUSE_SOURCE=$candidate"
      if cp -al "$candidate" "$STUDIO/node_modules" 2>"$LOGDIR/node-modules-reuse.log"; then
        say "OPENDECK_V230_NODE_MODULES_REUSE=PASS"
      else
...
if [[ ! -d "$STUDIO/node_modules" ]]; then
  ...
  say "OPENDECK_V230_NPM_INSTALL=REUSED"
fi
...
say "OPENDECK_V230_NEW_BINARY_SHA256=$NEW_SHA"
...
OPENDECK_V230_QUALIFICATION=1 OPENDECK_V230_QUALIFICATION_PHASE=visual OPENDECK_QUALIFICATION_DIR="$QDIR" "$NEW_BIN" >"$LOGDIR/candidate-visual.log" 2>&1 &
pid=$!
...
gate "VISUAL_GEOMETRY" "$LOGDIR/visual-geometry.log" python3 "$ROOT/scripts/check-v230-visual-metrics.py" "$VISUAL_JSON"
if command -v spectacle >/dev/null 2>&1; then
  if "$ROOT/scripts/capture-v229-window.sh" "$SCREENSHOT" "$pid" "2.0.30" "$FOCUS_ACK" >"$LOGDIR/screenshot.log" 2>&1; then
    say "OPENDECK_V230_SCREENSHOT=PASS:$SCREENSHOT"
  else
    say "OPENDECK_V230_SCREENSHOT=SKIP_CAPTURE_FAILED"
    tail -80 "$LOGDIR/screenshot.log" | tee -a "$VERIFY" || true
  fi
else
  say "OPENDECK_V230_SCREENSHOT=SKIP:SPECTACLE_MISSING"
fi
stop_candidate "$pid"

# Only now replace the launch symlink. The already-running 2.0.29 process is not killed.
gate "ACTIVATION" "$LOGDIR/activation.log" bash "$ROOT/scripts/activate-v230-qualified.sh" "$NEW_BIN" "$NEW_SHA" "$ROOT" "$ROLLBACK" "$VERIFY"
say "OPENDECK_V230_DIAL_STACKS=PASS"
say "OPENDECK_V230_FULL_HOST_QUALIFICATION=PASS"
say "OPENDECK_V2_0_30=PASS"
say "VERIFY_FILE=$VERIFY"
say "OPENDECK_V230_NOTE=Restart_OpenDeck_when_ready_to_run_the_new_binary"
