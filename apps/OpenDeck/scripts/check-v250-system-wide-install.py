...
required = [
    root / 'scripts/stage-v250-system-wide.sh',
    root / 'scripts/activate-v250-system-wide.sh',
    root / 'scripts/rollback-v250-system-wide.sh',
    root / 'scripts/uninstall-v250-system-wide.sh',
    root / 'scripts/check-v250-system-menu-launch.sh',
]
checks = {}
...
checks['WORKSPACE_VERSION_2_0_49'] = 'version = "2.0.50"' in cargo
conf = json.loads((root / 'apps/opendeck-studio/src-tauri/tauri.conf.json').read_text())
checks['TAURI_VERSION_2_0_49'] = conf.get('version') == '2.0.50'

if required[0].exists():
    stage = required[0].read_text()
    checks['STAGE_OPT_VERSION_ROOT'] = '/opt/opendeck-plus' in stage and 'TARGET_VERSION="2.0.50"' in stage and 'INSTALL_ROOT="$OPT_ROOT/$TARGET_VERSION"' in stage
...
if required[4].exists():
    ...
    checks['MENU_SMOKE_INSTALLED_BINARY'] = '/opt/opendeck-plus/2.0.50/bin/opendeck-studio' in smoke
else:
...
if not all(checks.values()):
    raise SystemExit(1)
print('OPENDECK_V250_SYSTEM_WIDE_INSTALL_CONTRACT=PASS')
