...
VERIFY="$DL/OpenDeck-v2.0.15-RENDER-PERFORMANCE-VERIFY.txt"
ROLLBACK="$DL/OpenDeck-v2.0.15-RENDER-PERFORMANCE-ROLLBACK.txt"
LOGDIR="$DL/OpenDeck-v2.0.15-RENDER-PERFORMANCE-logs"
QDIR="$DL/OpenDeck-v2.0.15-qualification"
VISUAL_JSON="$QDIR/OpenDeck-v2.0.15-VISUAL-METRICS.json"
PERF_JSON="$QDIR/OpenDeck-v2.0.15-PERFORMANCE-METRICS.json"
SCREENSHOT="$DL/OpenDeck-v2.0.15-QUALIFICATION.png"
BOOT_DIAGNOSTIC="$DL/OpenDeck-v2.0.15-BOOT-DIAGNOSTIC.png"
STAMP="$DL/OpenDeck-v2.0.15-QUALIFIED-STAMP.txt"
STAGED_QUALIFIED="$HOME/.local/lib/.opendeck-v2.0.15-qualified"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
ARCHIVE_SHA="${OPENDECK_V215_ARCHIVE_SHA:?OPENDECK_V215_ARCHIVE_SHA is required}"

mkdir -p "$LOGDIR" "$QDIR" "$HOME/.local/bin" "$HOME/.local/lib"
: > "$VERIFY"
say(){ printf '%s\n' "$*" | tee -a "$VERIFY"; }
fail(){ local stage="$1" rc="${2:-1}"; say "OPENDECK_V2_0_15_RENDER_PERFORMANCE=FAIL:$rc"; say "OPENDECK_FAILURE_STAGE=$stage"; if [[ -s "$SCREENSHOT" ]]; then say "OPENDECK_V215_PREVIEW_SCREENSHOT_AVAILABLE=$SCREENSHOT"; fi; if [[ -s "$BOOT_DIAGNOSTIC" ]]; then say "OPENDECK_V215_BOOT_DIAGNOSTIC_AVAILABLE=$BOOT_DIAGNOSTIC"; fi; say "VERIFY_FILE=$VERIFY"; exit "$rc"; }
gate(){ local name="$1" log="$2"; shift 2; say "OPENDECK_V215_STAGE=${name}:START"; if "$@" >"$log" 2>&1; then say "OPENDECK_V215_STAGE=${name}:PASS"; else local rc=$?; say "OPENDECK_V215_STAGE=${name}:FAIL:$rc"; tail -260 "$log" | tee -a "$VERIFY"; fail "$name" "$rc"; fi; }

say "OPENDECK_V2_0_15_RENDER_PERFORMANCE=START"
say "OPENDECK_RELEASE=PRODUCTION_TAURI_PREVIEW_CLOSURE"
...
say "OPENDECK_DIAL_STACKS=DEFERRED_TO_2.0.16"
...
say "OPENDECK_PREVIOUS_FAILED_VERSION=2.0.14"
say "OPENDECK_PREVIOUS_FAILURE_STAGE=PREVIEW_DEVURL_CONNECTION_REFUSED"
say "OPENDECK_REPAIR=PRODUCTION_TAURI_PREVIEW_EMBEDDED_FRONTEND"
...
gate "SOURCE_CONTRACT" "$LOGDIR/source-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-clean-baseline.py"
gate "VERSION_DEMO_CONTRACT" "$LOGDIR/version-demo.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-version-and-demo.py"
gate "SHELL_CONTRACT" "$LOGDIR/shell.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-shell-contract.py"
gate "RENDER_CONTRACT" "$LOGDIR/render-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-render-contract.py"
gate "PERFORMANCE_CONTRACT" "$LOGDIR/perf-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-performance-contract.py"
gate "TEST_CONTRACT" "$LOGDIR/test-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-test-contract.py"
gate "ACCESSIBILITY_NAME_CONTRACT" "$LOGDIR/accessibility-name.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-accessibility-name-contract.py"
...
gate "HOST_RUSTFMT_CLOSURE_CONTRACT" "$LOGDIR/host-rustfmt-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-host-rustfmt-closure.py"
...
gate "SCREENSHOT_FIRST_CONTRACT" "$LOGDIR/screenshot-first-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-screenshot-first.py"
gate "BOOTSTRAP_CONTRACT" "$LOGDIR/bootstrap-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-bootstrap-contract.py"
gate "PRODUCTION_TAURI_PREVIEW_CONTRACT" "$LOGDIR/production-tauri-preview-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v215-production-tauri-preview.py"

NODE_MODULES_REUSE="${OPENDECK_V215_NODE_MODULES_REUSE:-}"
if [[ ! -d "$STUDIO/node_modules" && -n "$NODE_MODULES_REUSE" && -d "$NODE_MODULES_REUSE" ]]; then
  mv "$NODE_MODULES_REUSE" "$STUDIO/node_modules"
  say "OPENDECK_V215_DEPENDENCY_REUSE=PASS"
fi
if [[ ! -d "$STUDIO/node_modules" ]]; then gate "NPM_INSTALL" "$LOGDIR/npm-install.log" bash -lc "cd '$STUDIO' && npm install --prefer-offline --no-audit --no-fund"; else say "OPENDECK_V215_NPM_INSTALL=REUSED_PREVIOUS_PASS"; fi
...
NEW_SHA="$(sha256sum "$NEW_BIN" | awk '{print $1}')"; say "OPENDECK_V215_NEW_BINARY_SHA256=$NEW_SHA"
...
launch_candidate(){ local phase="$1"; OPENDECK_V215_QUALIFICATION=1 OPENDECK_V215_QUALIFICATION_PHASE="$phase" OPENDECK_QUALIFICATION_DIR="$QDIR" "$NEW_BIN" >>"$LOGDIR/candidate-$phase.log" 2>&1 & echo $!; }

# Screenshot-first evidence gate. This writes either the qualified preview or a boot diagnostic before later Rust gates.
ERROR_JSON="$QDIR/OpenDeck-v2.0.15-QUALIFICATION-ERROR.json"
rm -f "$VISUAL_JSON" "$SCREENSHOT" "$BOOT_DIAGNOSTIC" "$ERROR_JSON"
say "OPENDECK_V215_PREVIEW_SCREENSHOT=START"
...
if wait_file "$VISUAL_JSON" "$pid" 2000; then
  ...
  ready_rc=$?
  sleep .8
  if "$ROOT/scripts/capture-v215-window.sh" "$BOOT_DIAGNOSTIC" >>"$LOGDIR/boot-diagnostic-capture.log" 2>&1; then
    say "OPENDECK_V215_PREVIEW_BOOT_DIAGNOSTIC=PASS:$BOOT_DIAGNOSTIC"
    say "OPENDECK_V215_BOOT_DIAGNOSTIC_SHA256=$(sha256sum "$BOOT_DIAGNOSTIC" | awk '{print $1}')"
  else
    say "OPENDECK_V215_PREVIEW_BOOT_DIAGNOSTIC=FAIL"
    tail -120 "$LOGDIR/boot-diagnostic-capture.log" 2>/dev/null | tee -a "$VERIFY" || true
  fi
  if [[ -s "$ERROR_JSON" ]]; then
    say "OPENDECK_V215_BOOT_ERROR_JSON=$ERROR_JSON"
    cat "$ERROR_JSON" | tee -a "$VERIFY"
  fi
  if [[ -s "$LOGDIR/candidate-visual.log" ]]; then
    say "OPENDECK_V215_CANDIDATE_LOG=$LOGDIR/candidate-visual.log"
...
sleep .8
if ! "$ROOT/scripts/capture-v215-window.sh" "$SCREENSHOT" >>"$LOGDIR/preview-capture.log" 2>&1; then stop_candidate "$pid"; fail "PREVIEW_SCREENSHOT_CAPTURE" 13; fi
stop_candidate "$pid"
[[ -s "$SCREENSHOT" ]] || fail "PREVIEW_SCREENSHOT_EMPTY" 13
say "OPENDECK_V215_PREVIEW_SCREENSHOT=PASS:$SCREENSHOT"
say "OPENDECK_V215_PREVIEW_SCREENSHOT_SHA256=$(sha256sum "$SCREENSHOT" | awk '{print $1}')"
...
rm -f "$VISUAL_JSON" "$QDIR/OpenDeck-v2.0.15-QUALIFICATION-ERROR.json"
...
PY
)"; stop_candidate "$pid"; say "OPENDECK_V215_COLD_START_MS=$cold_ms"
...
PY
)"; say "OPENDECK_V215_WARM_START_P95_MS=$warm_p95"
...
PY
say "OPENDECK_V215_STARTUP_PERFORMANCE=PASS"

# Final visual run: capture to a temporary path so the early preview survives any later failure.
FINAL_SCREENSHOT="$QDIR/OpenDeck-v2.0.15-QUALIFICATION-FINAL.png"
rm -f "$VISUAL_JSON" "$FINAL_SCREENSHOT" "$QDIR/OpenDeck-v2.0.15-QUALIFICATION-ERROR.json"
pid="$(launch_candidate visual)"; wait_file "$VISUAL_JSON" "$pid" 2000 || { stop_candidate "$pid"; fail "VISUAL_METRICS_GENERATION" 13; }; sleep .8
if ! "$ROOT/scripts/capture-v215-window.sh" "$FINAL_SCREENSHOT" >>"$LOGDIR/capture.log" 2>&1; then stop_candidate "$pid"; fail "SCREENSHOT_CAPTURE" 13; fi
stop_candidate "$pid"
[[ -s "$FINAL_SCREENSHOT" ]] || fail "FINAL_SCREENSHOT_EMPTY" 13
mv -f "$FINAL_SCREENSHOT" "$SCREENSHOT"
gate "VISUAL_GEOMETRY" "$LOGDIR/visual-geometry.log" python3 "$ROOT/scripts/check-v215-visual-metrics.py" "$VISUAL_JSON"
gate "SCREENSHOT_SIMILARITY" "$LOGDIR/screenshot-similarity.log" python3 "$ROOT/scripts/v215-png-metrics.py" "$ROOT/qualification/OpenDeck-v2.0.15-CANONICAL-UI-TARGET.png" "$SCREENSHOT"

# Performance run. The on-screen banner gives a 10-second real-hardware exercise window.
say "OPENDECK_V215_HARDWARE_EXERCISE=WHEN_BANNER_APPEARS_ROTATE_OR_PRESS_ANY_DIAL_CONTINUOUSLY_FOR_10_SECONDS"
rm -f "$PERF_JSON" "$QDIR/OpenDeck-v2.0.15-QUALIFICATION-ERROR.json"
...
say "OPENDECK_V215_IDLE_CPU_PERCENT=$idle_cpu"; say "OPENDECK_V215_RSS_MAX_MIB=$rss_mib"
...
PY
gate "PERFORMANCE_METRICS" "$LOGDIR/performance-metrics.log" python3 "$ROOT/scripts/check-v215-performance-metrics.py" "$PERF_JSON"
say "OPENDECK_V215_WINDOWS_REFERENCE=UNAVAILABLE_ABSOLUTE_GATES_APPLIED"
...
chmod 600 "$STAMP"
say "OPENDECK_V215_AUTOMATED_QUALIFY=PASS"
say "OPENDECK_V215_SCREENSHOT=$SCREENSHOT"
say "OPENDECK_V215_VISUAL_METRICS=$VISUAL_JSON"
say "OPENDECK_V215_PERFORMANCE_METRICS=$PERF_JSON"
say "OPENDECK_V215_QUALIFIED_STAMP=$STAMP"
say "OPENDECK_V215_VISUAL_HUMAN_APPROVAL=PENDING"
say "OPENDECK_V215_ACTIVATE=BLOCKED_PENDING_HUMAN_VISUAL_APPROVAL"
