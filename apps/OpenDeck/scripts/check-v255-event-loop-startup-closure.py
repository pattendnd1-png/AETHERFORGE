...
menu = (root / 'scripts/check-v255-system-menu-launch.sh').read_text() if (root / 'scripts/check-v255-system-menu-launch.sh').exists() else ''
...
if errors:
    for error in errors:
        print(f'OPENDECK_V255_EVENT_LOOP_STARTUP_CLOSURE=FAIL:{error}')
    sys.exit(1)

print('OPENDECK_V255_EVENT_LOOP_STARTUP_CLOSURE=PASS')
