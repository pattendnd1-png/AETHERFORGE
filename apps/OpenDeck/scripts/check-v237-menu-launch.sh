...
[[ -n "$EXPECTED_REAL" && -x "$EXPECTED_REAL" ]] || { echo 'OPENDECK_V237_MENU_LAUNCH=FAIL:EXPECTED_BINARY'; exit 2; }
command -v gtk-launch >/dev/null 2>&1 || { echo 'OPENDECK_V237_MENU_LAUNCH=FAIL:GTK_LAUNCH_MISSING'; exit 3; }

DESKTOP_DIR="$HOME/.local/share/applications"
DESKTOP_ID="opendeck-v237-menu-smoke"
DESKTOP_FILE="$DESKTOP_DIR/$DESKTOP_ID.desktop"
LOG_FILE="/tmp/opendeck-v237-menu-launch.log"
PROBE_FILE="/tmp/opendeck-v237-menu-visible-$$.json"
mkdir -p "$DESKTOP_DIR"
new_pid=""
cleanup(){
  if [[ -n "$new_pid" ]]; then
...
[Desktop Entry]
Type=Application
Name=OpenDeck+ 2.0.37 Menu Smoke
...
if command -v desktop-file-validate >/dev/null 2>&1; then
  desktop-file-validate "$DESKTOP_FILE" || { echo 'OPENDECK_V237_MENU_LAUNCH=FAIL:TEMP_DESKTOP_INVALID'; exit 4; }
fi
command -v update-desktop-database >/dev/null 2>&1 && update-desktop-database "$DESKTOP_DIR" >/dev/null 2>&1 || true
if command -v kbuildsycoca6 >/dev/null 2>&1; then kbuildsycoca6 --noincremental >/dev/null 2>&1 || true
elif command -v kbuildsycoca5 >/dev/null 2>&1; then kbuildsycoca5 --noincremental >/dev/null 2>&1 || true
...
gtk-launch "$DESKTOP_ID" >"$LOG_FILE" 2>&1 || {
  cat "$LOG_FILE" || true
  echo 'OPENDECK_V237_MENU_LAUNCH=FAIL:GTK_LAUNCH'
  exit 5
}
...
[[ -n "$new_pid" ]] || { cat "$LOG_FILE" || true; echo 'OPENDECK_V237_MENU_LAUNCH=FAIL:NO_NEW_PROCESS'; exit 6; }

actual_real="$(readlink -f "/proc/$new_pid/exe" 2>/dev/null || true)"
[[ "$actual_real" == "$EXPECTED_REAL" ]] || {
  echo "OPENDECK_V237_MENU_LAUNCH=FAIL:WRONG_BINARY:$actual_real:$EXPECTED_REAL"
  exit 7
}
...
d=json.load(open(p))
assert d.get('release') == '2.0.37'
assert d.get('pid') == pid
assert d.get('nativeVisible') is True
assert d.get('frontendVisible') is True
PY
  then
    echo "OPENDECK_V237_MENU_LAUNCH=PASS:pid=$new_pid:probe=$PROBE_FILE:binary=$actual_real"
    exit 0
  fi
  kill -0 "$new_pid" >/dev/null 2>&1 || { cat "$LOG_FILE" || true; echo 'OPENDECK_V237_MENU_LAUNCH=FAIL:PROCESS_EXITED'; exit 8; }
  sleep .05
done
cat "$LOG_FILE" || true
echo "OPENDECK_V237_MENU_LAUNCH=FAIL:NO_VISIBLE_STARTUP_ACK:pid=$new_pid"
exit 9
