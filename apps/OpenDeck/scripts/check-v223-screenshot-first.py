...
runner = Path('scripts/qualify-v223-host.sh')
if not runner.exists():
    raise SystemExit('QUALIFIER_ORDER=FAIL:qualify-v223-host.sh missing')
text = runner.read_text()
required = [
    'gate "PREVIEW_TAURI_BUILD"',
    'say "OPENDECK_V223_PREVIEW_SCREENSHOT=START"',
    'capture-v223-window.sh',
    'say "OPENDECK_V223_PREVIEW_SCREENSHOT=PASS:',
    'gate "CARGO_FMT"',
...
if not (pos['gate "PREVIEW_TAURI_BUILD"'] < pos['say "OPENDECK_V223_PREVIEW_SCREENSHOT=START"'] < pos['capture-v223-window.sh'] < pos['say "OPENDECK_V223_PREVIEW_SCREENSHOT=PASS:'] < pos['gate "CARGO_FMT"'] < pos['gate "CARGO_CLIPPY_STRICT"']):
...
for token in [
    'OPENDECK_V223_PREVIEW_SCREENSHOT_AVAILABLE=$SCREENSHOT',
...
preview_pass = text.index('say \"OPENDECK_V223_PREVIEW_SCREENSHOT=PASS:')
...
print('OPENDECK_V223_SCREENSHOT_FIRST_CONTRACT=PASS')
