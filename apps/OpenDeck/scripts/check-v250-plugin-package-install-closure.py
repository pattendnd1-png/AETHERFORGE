...
for name, ok in checks.items():
    print(f'OPENDECK_V250_{name}={"PASS" if ok else "FAIL"}')
if failed:
    print('OPENDECK_V250_PLUGIN_PACKAGE_INSTALL_CLOSURE=FAIL:' + ','.join(failed))
    sys.exit(1)
print('OPENDECK_V250_PLUGIN_PACKAGE_INSTALL_CLOSURE=PASS')
