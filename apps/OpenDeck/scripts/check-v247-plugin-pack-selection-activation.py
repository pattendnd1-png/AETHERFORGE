...
checks['ROOT_VERSION']=has('Cargo.toml','version = "2.0.47"')
checks['PACKAGE_VERSION']=json.loads((root/'apps/opendeck-studio/package.json').read_text()).get('version')=='2.0.47'
checks['TAURI_VERSION']=json.loads((root/'apps/opendeck-studio/src-tauri/tauri.conf.json').read_text()).get('version')=='2.0.47'
checks['PACK_MANAGER_EXISTS']=(root/'apps/opendeck-studio/src-tauri/src/pack_manager.rs').is_file()
checks['PACK_LIST_COMMAND']=has('apps/opendeck-studio/src-tauri/src/pack_manager.rs','pub(crate) fn icon_pack_list')
checks['PACK_INSTALL_COMMAND']=has('apps/opendeck-studio/src-tauri/src/pack_manager.rs','pub(crate) fn icon_pack_install')
checks['PACK_ACTIVATE_COMMAND']=has('apps/opendeck-studio/src-tauri/src/pack_manager.rs','pub(crate) fn icon_pack_set_active')
checks['PACK_REMOVE_COMMAND']=has('apps/opendeck-studio/src-tauri/src/pack_manager.rs','pub(crate) fn icon_pack_remove')
...
checks['PROFILE_IMPORT_ACTIVATE']=has('apps/opendeck-studio/src/app/App.tsx',"normalized.endsWith('.streamdeckprofile')") and has('apps/opendeck-studio/src/app/App.tsx','await importProfile(path, true)')
...
checks['ASSET_REFRESH_AFTER_PACK_CHANGE']=has('apps/opendeck-studio/src/app/App.tsx','Promise.all([refreshIconPacks(), refreshAssets()])')
...
checks['QUALIFIER_GATE']=has('scripts/qualify-v247-host.sh','PLUGIN_PACK_SELECTION_ACTIVATION')
checks['QUALIFIER_UI_GATE']=has('scripts/qualify-v247-host.sh','PLUGIN_PACK_UI_TESTS')
failed=[name for name, ok in checks.items() if not ok]
for name, ok in checks.items(): print(f'{name}={"PASS" if ok else "FAIL"}')
if failed:
    print('OPENDECK_V247_PLUGIN_PACK_SELECTION_ACTIVATION=FAIL:'+','.join(failed))
    sys.exit(1)
print('OPENDECK_V247_PLUGIN_PACK_SELECTION_ACTIVATION=PASS')
