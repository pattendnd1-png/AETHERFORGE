...
add('ROOT_VERSION', has('Cargo.toml','version = "2.0.44"'))
add('PACKAGE_VERSION', json.loads((root/'apps/opendeck-studio/package.json').read_text()).get('version')=='2.0.44')
conf=json.loads((root/'apps/opendeck-studio/src-tauri/tauri.conf.json').read_text())
window=conf['app']['windows'][0]
add('TAURI_VERSION', conf.get('version')=='2.0.44')
...
add('PLUGIN_HOST_RUST_COMPILE_CLOSURE', (root/'scripts/check-v244-plugin-host-rust-compile-closure.py').is_file() and has('scripts/qualify-v244-host.sh','PLUGIN_HOST_RUST_COMPILE_CLOSURE'))
add('PLUGIN_HOST_STRICT_CLIPPY_CLOSURE', (root/'scripts/check-v244-plugin-host-strict-clippy-closure.py').is_file() and has('scripts/qualify-v244-host.sh','gate "PLUGIN_HOST_STRICT_CLIPPY_CLOSURE"'))
add('SYSTEM_STAGE', (root/'scripts/stage-v244-system-wide.sh').is_file())
add('SYSTEM_ACTIVATE', (root/'scripts/activate-v244-system-wide.sh').is_file())
add('SYSTEM_ROLLBACK', (root/'scripts/rollback-v244-system-wide.sh').is_file())
add('SYSTEM_UNINSTALL', (root/'scripts/uninstall-v244-system-wide.sh').is_file())
failed=[name for name,ok in checks.items() if not ok]
for name,ok in checks.items(): print(f'{name}={"PASS" if ok else "FAIL"}')
if failed:
    print('OPENDECK_V244_FULL_PARITY_CONTRACT=FAIL:'+','.join(failed))
    sys.exit(1)
print('OPENDECK_V244_FULL_PARITY_CONTRACT=PASS')
