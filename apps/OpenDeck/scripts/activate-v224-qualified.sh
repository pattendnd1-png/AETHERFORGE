...
fail(){ say "OPENDECK_V224_ACTIVATION=FAIL:${1}"; exit "${2:-1}"; }
...
for link_name in opendeck-studio opendeck; do
  tmp_link="$USER_BIN/.${link_name}.v224-$$"
  rm -f -- "$tmp_link"
...
tmp_desktop="$DESKTOP_DIR/.opendeck-studio.desktop.v224-$$"
cat > "$tmp_desktop" <<EOF
[Desktop Entry]
Type=Application
Name=OpenDeck+
...
say "OPENDECK_V224_REPLACEMENT_INSTALL=PASS"
say "OPENDECK_V224_INSTALL_ROOT=$INSTALL_ROOT"
say "OPENDECK_V224_ROLLBACK_ROOT=$ROLLBACK_ROOT"
say "OPENDECK_V224_ACTIVE_BINARY_SHA256=$(sha "$USER_BIN/opendeck-studio")"
say "OPENDECK_V224_DESKTOP_ENTRY=$DESKTOP_FILE"
say "OPENDECK_V224_ICON=$ICON_DEST"
