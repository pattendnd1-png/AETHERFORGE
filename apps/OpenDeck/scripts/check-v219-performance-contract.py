...
checks={
 ...
 'RUST_METRICS_WRITER': all(x in qual for x in ['OpenDeck-v2.0.19-VISUAL-METRICS.json','OpenDeck-v2.0.19-PERFORMANCE-METRICS.json','atomic_json','record_runtime_sample']),
...
for key,ok in checks.items(): print(f'OPENDECK_V219_{key}={"PASS" if ok else "FAIL"}')
bad=[k for k,v in checks.items() if not v]
if bad:
 print('OPENDECK_V219_PERFORMANCE_CONTRACT=FAIL:'+','.join(bad)); sys.exit(1)
print('OPENDECK_V219_PERFORMANCE_CONTRACT=PASS')
