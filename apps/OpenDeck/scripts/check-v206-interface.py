...
required = {
    ...
    'dials_filter': "supportsControl(item.id, 'dial') || supportsControl(item.id, 'touch')" in actions,
}
...
for key, passed in required.items():
    print(f'OPENDECK_V206_INTERFACE_{key.upper()}={"PASS" if passed else "FAIL"}')
for key, present in forbidden.items():
    print(f'OPENDECK_V206_INTERFACE_FORBID_{key.upper()}={"FAIL" if present else "PASS"}')

bad = [key for key, passed in required.items() if not passed]
bad += [key for key, present in forbidden.items() if present]
if bad:
    print('OPENDECK_V206_INTERFACE_CONTRACT=FAIL:' + ','.join(bad))
    sys.exit(1)
print('OPENDECK_V206_INTERFACE_CONTRACT=PASS')
