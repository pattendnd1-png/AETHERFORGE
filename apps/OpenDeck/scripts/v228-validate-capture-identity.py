...
def main():
    if len(sys.argv)!=5: raise SystemExit('usage: v228-validate-capture-identity.py IMAGE.png FOCUS_ACK.json EXPECTED_PID EXPECTED_RELEASE')
    ...
    if ack.get('release') != release or int(ack.get('pid',-1)) != pid:
        raise SystemExit(f'OPENDECK_V228_CAPTURE_IDENTITY=FAIL:ACK:{ack}')
    w,h,img=decode(image)
    if (w,h)!=(1536,1024): raise SystemExit(f'OPENDECK_V228_CAPTURE_IDENTITY=FAIL:SIZE:{w}x{h}')
    nw=count_region(img,0,0,96,96)
    se=count_region(img,w-96,h-96,w,h)
    if nw < 8 or se < 8:
        raise SystemExit(f'OPENDECK_V228_CAPTURE_IDENTITY=FAIL:TOKEN:nw={nw}:se={se}')
    print(f'OPENDECK_V228_CAPTURE_IDENTITY=PASS:release={release}:pid={pid}:nw={nw}:se={se}')
