...
required = [
    root / 'scripts/stage-v247-system-wide.sh',
    root / 'scripts/activate-v247-system-wide.sh',
    root / 'scripts/rollback-v247-system-wide.sh',
    root / 'scripts/uninstall-v247-system-wide.sh',
    root / 'scripts/check-v247-system-menu-launch.sh',
]
checks = {}
...
checks['WORKSPACE_VERSION_2_0_47'] = 'version = "2.0.47"' in cargo
conf = json.loads((root / 'apps/opendeck-studio/src-tauri/tauri.conf.json').read_text())
checks['TAURI_VERSION_2_0_47'] = conf.get('version') == '2.0.47'

if required[0].exists():
    stage = required[0].read_text()
    checks['STAGE_OPT_VERSION_ROOT'] = '/opt/opendeck-plus' in stage and 'TARGET_VERSION="2.0.47"' in stage and 'INSTALL_ROOT="$OPT_ROOT/$TARGET_VERSION"' in stage
...
if required[4].exists():
    ...
    checks['MENU_SMOKE_INSTALLED_BINARY'] = '/opt/opendeck-plus/2.0.47/bin/opendeck-studio' in smoke
else:
...
if not all(checks.values()):
    raise SystemExit(1)
print('OPENDECK_V247_SYSTEM_WIDE_INSTALL_CONTRACT=PASS')
