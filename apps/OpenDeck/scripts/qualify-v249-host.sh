...
DL="$HOME/Downloads"
LOGDIR="$DL/OpenDeck-v2.0.49-logs"
VERIFY="$DL/OpenDeck-v2.0.49-VERIFY.txt"
ROLLBACK="$DL/OpenDeck-v2.0.49-ROLLBACK.txt"
QDIR="$DL/OpenDeck-v2.0.49-qualification"
VISUAL_JSON="$QDIR/OpenDeck-v2.0.49-VISUAL-METRICS.json"
FOCUS_ACK="$QDIR/OpenDeck-v2.0.49-FOCUS-ACK.json"
SCREENSHOT="$DL/OpenDeck-v2.0.49-QUALIFICATION.png"
NEW_BIN="$ROOT/target/release/opendeck-studio"
SYSTEM_INSTALL_ROOT="/opt/opendeck-plus/2.0.49"
SYSTEM_COMMAND="/usr/local/bin/opendeck-studio"
SYSTEM_ROLLBACK_STATE="/var/lib/opendeck-plus/rollback-v2.0.49"
ARCHIVE_SHA="${OPENDECK_V249_ARCHIVE_SHA:-UNSET}"
...
: > "$VERIFY"
say(){ printf '%s\n' "$*" | tee -a "$VERIFY"; }
fail(){ local stage="$1" rc="${2:-1}"; say "OPENDECK_V2_0_49=FAIL:$rc"; say "OPENDECK_FAILURE_STAGE=$stage"; say "VERIFY_FILE=$VERIFY"; exit "$rc"; }
gate(){ local name="$1" log="$2"; shift 2; say "OPENDECK_V249_STAGE=${name}:START"; if "$@" >"$log" 2>&1; then say "OPENDECK_V249_STAGE=${name}:PASS"; else local rc=$?; say "OPENDECK_V249_STAGE=${name}:FAIL:$rc"; tail -700 "$log" | tee -a "$VERIFY"; fail "$name" "$rc"; fi; }
rollback_on_failure(){
  local rc=$?
  if [[ $rc -ne 0 && "$SYSTEM_ACTIVATED" == "1" ]]; then
    say "OPENDECK_V249_AUTO_ROLLBACK=START"
    if sudo bash "$ROOT/scripts/rollback-v249-system-wide.sh" "$SYSTEM_ROLLBACK_STATE" "$VERIFY" >>"$LOGDIR/auto-rollback.log" 2>&1; then
      say "OPENDECK_V249_AUTO_ROLLBACK=PASS"
    else
      say "OPENDECK_V249_AUTO_ROLLBACK=FAIL"
...
say "OPENDECK_V2_0_49=START"
say "OPENDECK_RELEASE=DEVICE_ICON_PLUGIN_INSTALL_OBS_TOGGLE_CLOSURE"
...
gate "FULL_PARITY_CONTRACT" "$LOGDIR/full-parity-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-full-parity-contract.py"
gate "DEVICE_ICON_PLUGIN_INSTALL_OBS_TOGGLE" "$LOGDIR/device-icon-plugin-install-obs-toggle.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-device-icon-plugin-install-obs-toggle.py"
gate "PLUGIN_PACK_SELECTION_ACTIVATION" "$LOGDIR/plugin-pack-selection-activation.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-plugin-pack-selection-activation.py"
gate "PLUGIN_PACK_SIDEBAR_TEST_CLOSURE" "$LOGDIR/plugin-pack-sidebar-test-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-plugin-pack-sidebar-test-closure.py"
gate "QUALIFICATION_ENV_CLOSURE" "$LOGDIR/qualification-env-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-qualification-env-closure.py"
gate "FLUID_WINDOW_FILL_CLOSURE" "$LOGDIR/fluid-window-fill-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-fluid-window-fill-closure.py"
gate "PLUGIN_MODEL" "$LOGDIR/plugin-model.log" bash -lc "cd '$ROOT' && node --experimental-strip-types scripts/test-v249-plugin-model.mjs"
gate "SYSTEM_WIDE_INSTALL_CONTRACT" "$LOGDIR/system-wide-install-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-system-wide-install.py"
gate "EVENT_LOOP_STARTUP_CLOSURE" "$LOGDIR/event-loop-startup-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-event-loop-startup-closure.py"
gate "TAURI_WINDOW_PERMISSION_CLOSURE" "$LOGDIR/tauri-window-permission-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-tauri-window-permission-closure.py"
gate "TAURI_ASSET_PROTOCOL_FEATURE_CLOSURE" "$LOGDIR/tauri-asset-protocol-feature-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-tauri-asset-protocol-feature-closure.py"
gate "PLUGIN_HOST_RUST_COMPILE_CLOSURE" "$LOGDIR/plugin-host-rust-compile-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-plugin-host-rust-compile-closure.py"
gate "PLUGIN_HOST_STRICT_CLIPPY_CLOSURE" "$LOGDIR/plugin-host-strict-clippy-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-plugin-host-strict-clippy-closure.py"
gate "VISIBLE_STARTUP_KDE_LAUNCHER_CONTRACT" "$LOGDIR/visible-startup-kde-launcher-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-visible-startup-kde-launcher.py"
gate "FRONTEND_TEST_CLOSURE" "$LOGDIR/frontend-test-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-frontend-test-closure.py"
gate "FRONTEND_PLUGIN_BRIDGE_MOCK_CLOSURE" "$LOGDIR/frontend-plugin-bridge-mock-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-frontend-plugin-bridge-mock-closure.py"
gate "PLUGIN_PI_HOOK_LINT_CLOSURE" "$LOGDIR/plugin-pi-hook-lint-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-plugin-pi-hook-lint-closure.py"
gate "ACTION_WHEEL_CONTRACT" "$LOGDIR/action-wheel-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-action-wheel-contract.py"
gate "ACTION_WHEEL_MODEL" "$LOGDIR/action-wheel-model.log" bash -lc "cd '$ROOT' && node --experimental-strip-types scripts/test-v249-action-wheel.mjs"
gate "DIAL_STACK_PRESERVATION" "$LOGDIR/dial-stack-preservation.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-dial-stack-preservation.py"
gate "LAYOUT_CONTRACT" "$LOGDIR/layout-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v249-layout-contract.py"
...
if [[ ! -d "$STUDIO/node_modules" ]]; then
  for candidate in \
    ...
    if [[ -d "$candidate" ]]; then
      say "OPENDECK_V249_NODE_MODULES_REUSE_SOURCE=$candidate"
      if cp -al "$candidate" "$STUDIO/node_modules" 2>"$LOGDIR/node-modules-reuse.log"; then
        say "OPENDECK_V249_NODE_MODULES_REUSE=PASS"
      else
...
if [[ ! -d "$STUDIO/node_modules" ]]; then
  ...
  say "OPENDECK_V249_NPM_INSTALL=REUSED"
fi
...
say "OPENDECK_V249_NEW_BINARY_SHA256=$NEW_SHA"
...
gate "VISUAL_GEOMETRY" "$LOGDIR/visual-geometry.log" python3 "$ROOT/scripts/check-v249-visual-metrics.py" "$VISUAL_JSON"
if command -v spectacle >/dev/null 2>&1; then
  if "$ROOT/scripts/capture-v229-window.sh" "$SCREENSHOT" "$pid" "2.0.49" "$FOCUS_ACK" >"$LOGDIR/screenshot.log" 2>&1; then
    say "OPENDECK_V249_SCREENSHOT=PASS:$SCREENSHOT"
  else
    say "OPENDECK_V249_SCREENSHOT=SKIP_CAPTURE_FAILED"
    tail -80 "$LOGDIR/screenshot.log" | tee -a "$VERIFY" || true
  fi
else
  say "OPENDECK_V249_SCREENSHOT=SKIP:SPECTACLE_MISSING"
fi
stop_candidate "$pid"

# System-wide stage. This is the first privileged mutation and does not change
# the currently active system launch path.
say "OPENDECK_V249_SUDO_AUTH=START"
if sudo -v; then say "OPENDECK_V249_SUDO_AUTH=PASS"; else fail "SUDO_AUTH" 20; fi
gate "SYSTEM_STAGE" "$LOGDIR/system-stage.log" sudo bash "$ROOT/scripts/stage-v249-system-wide.sh" "$NEW_BIN" "$NEW_SHA" "$ROOT" "$VERIFY"
gate "SYSTEM_STAGED_TREE" "$LOGDIR/system-staged-tree.log" python3 "$ROOT/scripts/check-v249-installed-tree.py" staged "$NEW_SHA" "$TARGET_HOME"
say "OPENDECK_V249_STAGED_INSTALL_ROOT=$SYSTEM_INSTALL_ROOT"
...
gate "SYSTEM_MENU_LAUNCH_STAGED" "$LOGDIR/system-menu-staged.log" bash "$ROOT/scripts/check-v249-system-menu-launch.sh" staged "$SYSTEM_INSTALL_ROOT/bin/opendeck-studio"

# Activate the canonical system-wide integration only after staged menu launch passes.
gate "SYSTEM_ACTIVATION" "$LOGDIR/system-activation.log" sudo bash "$ROOT/scripts/activate-v249-system-wide.sh" "$NEW_SHA" "$TARGET_USER" "$TARGET_HOME" "$ROLLBACK" "$VERIFY"
SYSTEM_ACTIVATED=1
...
# to the installed 2.0.49 binary. Any failure after activation triggers rollback.
gate "SYSTEM_ACTIVE_TREE" "$LOGDIR/system-active-tree.log" python3 "$ROOT/scripts/check-v249-installed-tree.py" active "$NEW_SHA" "$TARGET_HOME"
gate "SYSTEM_MENU_LAUNCH_CANONICAL" "$LOGDIR/system-menu-canonical.log" bash "$ROOT/scripts/check-v249-system-menu-launch.sh" canonical "$SYSTEM_COMMAND"
...
SYSTEM_ACTIVATED=0
say "OPENDECK_V249_SYSTEM_FULL_INSTALL=PASS"
say "OPENDECK_V249_INSTALL_ROOT=$SYSTEM_INSTALL_ROOT"
say "OPENDECK_V249_CURRENT_LINK=/opt/opendeck-plus/current"
say "OPENDECK_V249_SYSTEM_COMMAND=$SYSTEM_COMMAND"
say "OPENDECK_V249_SYSTEM_DESKTOP=/usr/share/applications/opendeck-studio.desktop"
say "OPENDECK_V249_SYSTEM_ICON=/usr/share/icons/hicolor/64x64/apps/opendeck-studio.png"
say "OPENDECK_V249_SYSTEM_UDEV=/etc/udev/rules.d/70-opendeck-streamdeck.rules"
say "OPENDECK_V249_UNINSTALL_COMMAND=sudo /usr/local/sbin/opendeck-uninstall"
say "OPENDECK_V249_ROLLBACK_COMMAND=sudo /usr/local/sbin/opendeck-rollback /var/lib/opendeck-plus/rollback-v2.0.49"
say "OPENDECK_V249_PREVIOUS_BASELINE_PRESERVED=$BASELINE_TARGET"
say "OPENDECK_V249_FRAME_RESIZE=PASS"
say "OPENDECK_V249_FLUID_WINDOW_FILL=PASS"
say "OPENDECK_V249_PROFILES=PASS"
say "OPENDECK_V249_PLUGIN_HOST_BUILD_TESTS=PASS"
say "OPENDECK_V249_PLUGIN_PACK_SELECTION_ACTIVATION=PASS"
say "OPENDECK_V249_ICON_PACKS=PASS"
say "OPENDECK_V249_DEVICE_ICONS=PASS"
say "OPENDECK_V249_ICON_PACK_DEVICE_PUSH=PASS"
say "OPENDECK_V249_PLUGIN_INSTALL_DISCOVERY=PASS"
say "OPENDECK_V249_OBS_LAUNCH_CLOSE_ACTION=PASS"
say "OPENDECK_V249_ACTION_WHEEL=PASS"
say "OPENDECK_V249_DIAL_STACKS_PRESERVED=PASS"
say "OPENDECK_V249_FULL_HOST_QUALIFICATION=PASS"
say "OPENDECK_V2_0_49=PASS"
say "VERIFY_FILE=$VERIFY"
say "OPENDECK_V249_NOTE=System_wide_install_ready__Launch_OpenDeck_from_the_application_menu"
