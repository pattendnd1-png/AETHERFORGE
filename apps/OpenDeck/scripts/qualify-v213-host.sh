...
VERIFY="$DL/OpenDeck-v2.0.13-RENDER-PERFORMANCE-VERIFY.txt"
ROLLBACK="$DL/OpenDeck-v2.0.13-RENDER-PERFORMANCE-ROLLBACK.txt"
LOGDIR="$DL/OpenDeck-v2.0.13-RENDER-PERFORMANCE-logs"
QDIR="$DL/OpenDeck-v2.0.13-qualification"
VISUAL_JSON="$QDIR/OpenDeck-v2.0.13-VISUAL-METRICS.json"
PERF_JSON="$QDIR/OpenDeck-v2.0.13-PERFORMANCE-METRICS.json"
SCREENSHOT="$DL/OpenDeck-v2.0.13-QUALIFICATION.png"
STAMP="$DL/OpenDeck-v2.0.13-QUALIFIED-STAMP.txt"
STAGED_QUALIFIED="$HOME/.local/lib/.opendeck-v2.0.13-qualified"
EXPECTED_ACTIVE_SHA="78b5ceb386773d0e58aa322f67766780432c276888df9e3f91537bf687727937"
ARCHIVE_SHA="${OPENDECK_V213_ARCHIVE_SHA:?OPENDECK_V213_ARCHIVE_SHA is required}"

mkdir -p "$LOGDIR" "$QDIR" "$HOME/.local/bin" "$HOME/.local/lib"
: > "$VERIFY"
say(){ printf '%s\n' "$*" | tee -a "$VERIFY"; }
fail(){ local stage="$1" rc="${2:-1}"; say "OPENDECK_V2_0_13_RENDER_PERFORMANCE=FAIL:$rc"; say "OPENDECK_FAILURE_STAGE=$stage"; if [[ -s "$SCREENSHOT" ]]; then say "OPENDECK_V213_PREVIEW_SCREENSHOT_AVAILABLE=$SCREENSHOT"; fi; say "VERIFY_FILE=$VERIFY"; exit "$rc"; }
gate(){ local name="$1" log="$2"; shift 2; say "OPENDECK_V213_STAGE=${name}:START"; if "$@" >"$log" 2>&1; then say "OPENDECK_V213_STAGE=${name}:PASS"; else local rc=$?; say "OPENDECK_V213_STAGE=${name}:FAIL:$rc"; tail -260 "$log" | tee -a "$VERIFY"; fail "$name" "$rc"; fi; }

say "OPENDECK_V2_0_13_RENDER_PERFORMANCE=START"
say "OPENDECK_RELEASE=CANONICAL_RENDER_RUSTFMT_SCREENSHOT_FIRST_CLOSURE"
...
say "OPENDECK_DIAL_STACKS=DEFERRED_TO_2.0.14"
...
say "OPENDECK_PREVIOUS_FAILED_VERSION=2.0.12"
say "OPENDECK_PREVIOUS_FAILURE_STAGE=CARGO_FMT"
say "OPENDECK_REPAIR=RUSTFMT_PLUS_SCREENSHOT_FIRST_QUALIFIER"
...
gate "SOURCE_CONTRACT" "$LOGDIR/source-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v213-clean-baseline.py"
gate "VERSION_DEMO_CONTRACT" "$LOGDIR/version-demo.log" bash -lc "cd '$ROOT' && python3 scripts/check-v213-version-and-demo.py"
gate "SHELL_CONTRACT" "$LOGDIR/shell.log" bash -lc "cd '$ROOT' && python3 scripts/check-v213-shell-contract.py"
gate "RENDER_CONTRACT" "$LOGDIR/render-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v213-render-contract.py"
gate "PERFORMANCE_CONTRACT" "$LOGDIR/perf-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v213-performance-contract.py"
gate "TEST_CONTRACT" "$LOGDIR/test-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v213-test-contract.py"
gate "ACCESSIBILITY_NAME_CONTRACT" "$LOGDIR/accessibility-name.log" bash -lc "cd '$ROOT' && python3 scripts/check-v213-accessibility-name-contract.py"
...
gate "HOST_RUSTFMT_CLOSURE_CONTRACT" "$LOGDIR/host-rustfmt-closure.log" bash -lc "cd '$ROOT' && python3 scripts/check-v213-host-rustfmt-closure.py"
...
gate "SCREENSHOT_FIRST_CONTRACT" "$LOGDIR/screenshot-first-contract.log" bash -lc "cd '$ROOT' && python3 scripts/check-v213-screenshot-first.py"

NODE_MODULES_REUSE="${OPENDECK_V213_NODE_MODULES_REUSE:-}"
if [[ ! -d "$STUDIO/node_modules" && -n "$NODE_MODULES_REUSE" && -d "$NODE_MODULES_REUSE" ]]; then
  mv "$NODE_MODULES_REUSE" "$STUDIO/node_modules"
  say "OPENDECK_V213_DEPENDENCY_REUSE=PASS"
fi
if [[ ! -d "$STUDIO/node_modules" ]]; then gate "NPM_INSTALL" "$LOGDIR/npm-install.log" bash -lc "cd '$STUDIO' && npm install --prefer-offline --no-audit --no-fund"; else say "OPENDECK_V213_NPM_INSTALL=REUSED_PREVIOUS_PASS"; fi
...
NEW_SHA="$(sha256sum "$NEW_BIN" | awk '{print $1}')"; say "OPENDECK_V213_NEW_BINARY_SHA256=$NEW_SHA"
...
launch_candidate(){ local phase="$1"; OPENDECK_V213_QUALIFICATION=1 OPENDECK_V213_QUALIFICATION_PHASE="$phase" OPENDECK_QUALIFICATION_DIR="$QDIR" "$NEW_BIN" >>"$LOGDIR/candidate-$phase.log" 2>&1 & echo $!; }

# Screenshot-first evidence gate. This writes the user-visible PNG before later Rust gates.
rm -f "$VISUAL_JSON" "$SCREENSHOT" "$QDIR/OpenDeck-v2.0.13-QUALIFICATION-ERROR.json"
say "OPENDECK_V213_PREVIEW_SCREENSHOT=START"
pid="$(launch_candidate visual)"
wait_file "$VISUAL_JSON" "$pid" 2000 || { stop_candidate "$pid"; fail "PREVIEW_VISUAL_READY" 13; }
sleep .8
if ! "$ROOT/scripts/capture-v213-window.sh" "$SCREENSHOT" >>"$LOGDIR/preview-capture.log" 2>&1; then stop_candidate "$pid"; fail "PREVIEW_SCREENSHOT_CAPTURE" 13; fi
stop_candidate "$pid"
[[ -s "$SCREENSHOT" ]] || fail "PREVIEW_SCREENSHOT_EMPTY" 13
say "OPENDECK_V213_PREVIEW_SCREENSHOT=PASS:$SCREENSHOT"
say "OPENDECK_V213_PREVIEW_SCREENSHOT_SHA256=$(sha256sum "$SCREENSHOT" | awk '{print $1}')"
...
rm -f "$VISUAL_JSON" "$QDIR/OpenDeck-v2.0.13-QUALIFICATION-ERROR.json"
...
PY
)"; stop_candidate "$pid"; say "OPENDECK_V213_COLD_START_MS=$cold_ms"
...
PY
)"; say "OPENDECK_V213_WARM_START_P95_MS=$warm_p95"
...
PY
say "OPENDECK_V213_STARTUP_PERFORMANCE=PASS"

# Final visual run: capture to a temporary path so the early preview survives any later failure.
FINAL_SCREENSHOT="$QDIR/OpenDeck-v2.0.13-QUALIFICATION-FINAL.png"
rm -f "$VISUAL_JSON" "$FINAL_SCREENSHOT" "$QDIR/OpenDeck-v2.0.13-QUALIFICATION-ERROR.json"
pid="$(launch_candidate visual)"; wait_file "$VISUAL_JSON" "$pid" 2000 || { stop_candidate "$pid"; fail "VISUAL_METRICS_GENERATION" 13; }; sleep .8
if ! "$ROOT/scripts/capture-v213-window.sh" "$FINAL_SCREENSHOT" >>"$LOGDIR/capture.log" 2>&1; then stop_candidate "$pid"; fail "SCREENSHOT_CAPTURE" 13; fi
stop_candidate "$pid"
[[ -s "$FINAL_SCREENSHOT" ]] || fail "FINAL_SCREENSHOT_EMPTY" 13
mv -f "$FINAL_SCREENSHOT" "$SCREENSHOT"
gate "VISUAL_GEOMETRY" "$LOGDIR/visual-geometry.log" python3 "$ROOT/scripts/check-v213-visual-metrics.py" "$VISUAL_JSON"
gate "SCREENSHOT_SIMILARITY" "$LOGDIR/screenshot-similarity.log" python3 "$ROOT/scripts/v213-png-metrics.py" "$ROOT/qualification/OpenDeck-v2.0.13-CANONICAL-UI-TARGET.png" "$SCREENSHOT"

# Performance run. The on-screen banner gives a 10-second real-hardware exercise window.
say "OPENDECK_V213_HARDWARE_EXERCISE=WHEN_BANNER_APPEARS_ROTATE_OR_PRESS_ANY_DIAL_CONTINUOUSLY_FOR_10_SECONDS"
rm -f "$PERF_JSON" "$QDIR/OpenDeck-v2.0.13-QUALIFICATION-ERROR.json"
...
say "OPENDECK_V213_IDLE_CPU_PERCENT=$idle_cpu"; say "OPENDECK_V213_RSS_MAX_MIB=$rss_mib"
...
PY
gate "PERFORMANCE_METRICS" "$LOGDIR/performance-metrics.log" python3 "$ROOT/scripts/check-v213-performance-metrics.py" "$PERF_JSON"
say "OPENDECK_V213_WINDOWS_REFERENCE=UNAVAILABLE_ABSOLUTE_GATES_APPLIED"
...
chmod 600 "$STAMP"
say "OPENDECK_V213_AUTOMATED_QUALIFY=PASS"
say "OPENDECK_V213_SCREENSHOT=$SCREENSHOT"
say "OPENDECK_V213_VISUAL_METRICS=$VISUAL_JSON"
say "OPENDECK_V213_PERFORMANCE_METRICS=$PERF_JSON"
say "OPENDECK_V213_QUALIFIED_STAMP=$STAMP"
say "OPENDECK_V213_VISUAL_HUMAN_APPROVAL=PENDING"
say "OPENDECK_V213_ACTIVATE=BLOCKED_PENDING_HUMAN_VISUAL_APPROVAL"
