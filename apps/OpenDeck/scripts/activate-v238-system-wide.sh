...
# Preserve every path that the full install will mutate, including old per-user
# launchers. The 2.0.37 install tree itself is never removed.
...
tmp_current="$OPT_ROOT/.current.v238-$$"
rm -f -- "$tmp_current"
...
for pair in \
  ...
  tmp="$LOCAL_BIN_DIR/.$(basename "$dest").v238-$$"
  rm -f -- "$tmp"
  ln -s "$target" "$tmp"
  mv -Tf "$tmp" "$dest"
done
...
PREVIOUS_USER_INSTALL_PRESERVED=$TARGET_HOME/.local/lib/opendeck-v2.0.37
EOF
...
say "OPENDECK_V238_INSTALL_ROOT=$INSTALL_ROOT"
say "OPENDECK_V238_CURRENT_LINK=$CURRENT_LINK"
say "OPENDECK_V238_SYSTEM_COMMAND=$SYSTEM_BIN"
say "OPENDECK_V238_SYSTEM_DESKTOP=$SYSTEM_DESKTOP"
say "OPENDECK_V238_ACTIVE_BINARY_SHA256=$(sha "$SYSTEM_BIN")"
say "OPENDECK_V238_ROLLBACK_STATE=$ROLLBACK_DIR"
say "OPENDECK_V238_ROLLBACK_FILE=$ROLLBACK_FILE"
say "OPENDECK_V238_BACKGROUND_SERVICES=NONE"
say "OPENDECK_V238_AUTOSTART=DISABLED"
