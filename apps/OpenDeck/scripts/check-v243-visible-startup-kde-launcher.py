...
activator_path = root / 'scripts/activate-v243-system-wide.sh'
qualifier_path = root / 'scripts/qualify-v243-host.sh'
menu_path = root / 'scripts/check-v243-system-menu-launch.sh'
activator = activator_path.read_text() if activator_path.exists() else ''
stage = (root / 'scripts/stage-v243-system-wide.sh').read_text()
...
if not all(checks.values()):
    raise SystemExit(1)
print('OPENDECK_V243_VISIBLE_STARTUP_KDE_LAUNCHER_CONTRACT=PASS')
