...
markers = {
    ...
    'PREVIEW_SCREENSHOT_PASS': 'OPENDECK_V223_PREVIEW_SCREENSHOT=PASS:',
...
for key, ok in checks.items():
    print(f'OPENDECK_V223_{key}={"PASS" if ok else "FAIL"}')

bad = [key for key, ok in checks.items() if not ok]
if bad:
    print('OPENDECK_V223_SCREENSHOT_BEFORE_TESTS_CONTRACT=FAIL:' + ','.join(bad))
    sys.exit(1)
print('OPENDECK_V223_SCREENSHOT_BEFORE_TESTS_CONTRACT=PASS')
