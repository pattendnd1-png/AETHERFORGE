...
tmp_current="$OPT_ROOT/.current.v253-$$"
rm -f -- "$tmp_current"
...
for pair in \
  ...
  tmp="$LOCAL_BIN_DIR/.$(basename "$dest").v253-$$"
  rm -f -- "$tmp"
  ln -s "$target" "$tmp"
  mv -Tf "$tmp" "$dest"
done
...
say "OPENDECK_V253_INSTALL_ROOT=$INSTALL_ROOT"
say "OPENDECK_V253_CURRENT_LINK=$CURRENT_LINK"
say "OPENDECK_V253_SYSTEM_COMMAND=$SYSTEM_BIN"
say "OPENDECK_V253_SYSTEM_DESKTOP=$SYSTEM_DESKTOP"
say "OPENDECK_V253_ACTIVE_BINARY_SHA256=$(sha "$SYSTEM_BIN")"
say "OPENDECK_V253_ROLLBACK_STATE=$ROLLBACK_DIR"
say "OPENDECK_V253_ROLLBACK_FILE=$ROLLBACK_FILE"
say "OPENDECK_V253_BACKGROUND_SERVICES=NONE"
say "OPENDECK_V253_AUTOSTART=DISABLED"
