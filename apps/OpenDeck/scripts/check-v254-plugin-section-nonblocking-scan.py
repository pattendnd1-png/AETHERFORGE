...
visual = (root/'scripts/check-v254-visual-metrics.py').read_text()
checks = {
...
for name, ok in checks.items():
    print(f'OPENDECK_V254_{name}={"PASS" if ok else "FAIL"}')
failed=[name for name,ok in checks.items() if not ok]
if failed:
    print('OPENDECK_V254_PLUGIN_SECTION_NONBLOCKING_SCAN=FAIL:'+','.join(failed))
    raise SystemExit(1)
print('OPENDECK_V254_PLUGIN_SECTION_NONBLOCKING_SCAN=PASS')
