...
EXPECTED_BIN="${2:-/opt/opendeck-plus/2.0.50/bin/opendeck-studio}"
...
[[ "$MODE" == "staged" || "$MODE" == "canonical" ]] || { echo "OPENDECK_V250_SYSTEM_MENU_LAUNCH=FAIL:MODE:$MODE"; exit 2; }
[[ -n "$EXPECTED_REAL" && -x "$EXPECTED_REAL" ]] || { echo 'OPENDECK_V250_SYSTEM_MENU_LAUNCH=FAIL:EXPECTED_BINARY'; exit 2; }
command -v gtk-launch >/dev/null 2>&1 || { echo 'OPENDECK_V250_SYSTEM_MENU_LAUNCH=FAIL:GTK_LAUNCH_MISSING'; exit 3; }
command -v desktop-file-validate >/dev/null 2>&1 || { echo 'OPENDECK_V250_SYSTEM_MENU_LAUNCH=FAIL:DESKTOP_VALIDATE_MISSING'; exit 3; }

DESKTOP_ID="opendeck-studio"
TEMP_SYSTEM_FILE=""
LOCAL_TEMP=""
if [[ "$MODE" == "staged" ]]; then
  DESKTOP_ID="opendeck-v250-system-smoke"
  TEMP_SYSTEM_FILE="$SYSTEM_APP_DIR/$DESKTOP_ID.desktop"
  LOCAL_TEMP="/tmp/$DESKTOP_ID-$$.desktop"
  cat > "$LOCAL_TEMP" <<EOF
[Desktop Entry]
Type=Application
Name=OpenDeck+ 2.0.50 System Install Smoke
Exec=$EXPECTED_REAL
...
StartupNotify=true
EOF
  desktop-file-validate "$LOCAL_TEMP" || { echo 'OPENDECK_V250_SYSTEM_MENU_LAUNCH=FAIL:TEMP_DESKTOP_INVALID'; exit 4; }
...
LOG_FILE="/tmp/opendeck-v250-system-menu-$MODE.log"
PROBE_FILE="/tmp/opendeck-v250-system-menu-$MODE-$$.json"
new_pid=""
cleanup(){
  if [[ -n "$new_pid" ]]; then
...
OPENDECK_STARTUP_PROBE_FILE="$PROBE_FILE" gtk-launch "$DESKTOP_ID" >"$LOG_FILE" 2>&1 || {
  cat "$LOG_FILE" || true
  echo "OPENDECK_V250_SYSTEM_MENU_LAUNCH=FAIL:GTK_LAUNCH:$MODE"
  exit 5
}
...
[[ -n "$new_pid" ]] || { cat "$LOG_FILE" || true; echo "OPENDECK_V250_SYSTEM_MENU_LAUNCH=FAIL:NO_NEW_PROCESS:$MODE"; exit 6; }

actual_real="$(readlink -f "/proc/$new_pid/exe" 2>/dev/null || true)"
[[ "$actual_real" == "$EXPECTED_REAL" ]] || {
  echo "OPENDECK_V250_SYSTEM_MENU_LAUNCH=FAIL:WRONG_BINARY:$actual_real:$EXPECTED_REAL:$MODE"
  exit 7
}
...
d=json.load(open(p))
assert d.get('release') == '2.0.50'
assert d.get('pid') == pid
assert d.get('nativeVisible') is True
assert d.get('frontendVisible') is True
PY
  then
    echo "OPENDECK_V250_SYSTEM_MENU_LAUNCH=PASS:mode=$MODE:pid=$new_pid:binary=$actual_real"
    exit 0
  fi
  kill -0 "$new_pid" >/dev/null 2>&1 || { cat "$LOG_FILE" || true; echo "OPENDECK_V250_SYSTEM_MENU_LAUNCH=FAIL:PROCESS_EXITED:$MODE"; exit 8; }
  sleep .05
done
cat "$LOG_FILE" || true
echo "OPENDECK_V250_SYSTEM_MENU_LAUNCH=FAIL:NO_VISIBLE_STARTUP_ACK:mode=$MODE:pid=$new_pid"
exit 9
