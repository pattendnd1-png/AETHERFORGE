...
tmp_current="$OPT_ROOT/.current.v248-$$"
rm -f -- "$tmp_current"
...
for pair in \
  ...
  tmp="$LOCAL_BIN_DIR/.$(basename "$dest").v248-$$"
  rm -f -- "$tmp"
  ln -s "$target" "$tmp"
  mv -Tf "$tmp" "$dest"
done
...
say "OPENDECK_V248_INSTALL_ROOT=$INSTALL_ROOT"
say "OPENDECK_V248_CURRENT_LINK=$CURRENT_LINK"
say "OPENDECK_V248_SYSTEM_COMMAND=$SYSTEM_BIN"
say "OPENDECK_V248_SYSTEM_DESKTOP=$SYSTEM_DESKTOP"
say "OPENDECK_V248_ACTIVE_BINARY_SHA256=$(sha "$SYSTEM_BIN")"
say "OPENDECK_V248_ROLLBACK_STATE=$ROLLBACK_DIR"
say "OPENDECK_V248_ROLLBACK_FILE=$ROLLBACK_FILE"
say "OPENDECK_V248_BACKGROUND_SERVICES=NONE"
say "OPENDECK_V248_AUTOSTART=DISABLED"
