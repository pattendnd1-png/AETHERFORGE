...
menu = (root / 'scripts/check-v238-system-menu-launch.sh').read_text() if (root / 'scripts/check-v238-system-menu-launch.sh').exists() else ''
...
if 'window.maximize()' not in prepare:
    errors.append('native_maximize_request_missing')
...
if errors:
    for error in errors:
        print(f'OPENDECK_V238_EVENT_LOOP_STARTUP_CLOSURE=FAIL:{error}')
    sys.exit(1)

print('OPENDECK_V238_EVENT_LOOP_STARTUP_CLOSURE=PASS')
