...
qualifier = (root/'scripts/qualify-v219-host.sh').read_text() if (root/'scripts/qualify-v219-host.sh').exists() else ''
checks = {
    ...
    'DIAGNOSTIC_PNG_ON_READY_FAILURE': 'OpenDeck-v2.0.19-BOOT-DIAGNOSTIC.png' in qualifier and 'PREVIEW_BOOT_DIAGNOSTIC=PASS' in qualifier,
    'V219_VISUAL_READY': 'OPENDECK_V219_PREVIEW_SCREENSHOT=PASS' in qualifier,
}
failed = [name for name, ok in checks.items() if not ok]
for name, ok in checks.items():
    print(f'OPENDECK_V219_BOOTSTRAP_{name}={"PASS" if ok else "FAIL"}')
if failed:
    print('OPENDECK_V219_BOOTSTRAP_CONTRACT=FAIL:' + ','.join(failed))
    sys.exit(1)
print('OPENDECK_V219_BOOTSTRAP_CONTRACT=PASS')
