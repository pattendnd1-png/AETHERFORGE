...
export const DialControl = memo(function DialControl({ slot, appearance, selected, iconUrl, backgroundUrl, onSelect, onDragStart, onDragOver, onDrop }: DialControlProps) {
  const title = appearance.title || `Dial ${slot.position + 1}`;
  const knobStyle = {
  ...
  return <button aria-label={title} draggable data-testid="dial" data-qualify-element="dial" className={`dial-control${selected ? ' selected' : ''}`} onClick={onSelect} onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop}>
    <span className="dial-ring" style={knobStyle}><span className="dial-cap"><span className="dial-highlight" />{iconUrl ? <img src={iconUrl} alt="" style={{ opacity: appearance.iconOpacity }} /> : <Glyph name={glyphForLabel(title)} />}</span></span>
