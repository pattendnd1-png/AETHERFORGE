...
import type { ActionInstance, Appearance, AppearanceOverride, ControlSlot, Interaction, Page, Profile, TouchStripConfig, TouchStripMode, TouchStripPresentation, TouchStripRule } from '../model/workspace';
...
import { KeyLogicEditor } from './KeyLogicEditor';
  ...
  return <section className="configuration-strip" data-layout-region="configuration">
    ...
    {!slot ? <div className="configuration-empty"><p>Select a key, dial, or touch region to configure it.</p></div> : <div className="configuration-workspace">
      ...
      <div className="configuration-content">
        ...
        {tab==='action' ? (binding?.definitionId === 'editor.multiAction' ? <><MultiActionEditor steps={Array.isArray(binding.config.steps) ? binding.config.steps as ActionInstance[] : []} controlKind={slot.kind} onChange={(steps) => onConfig(visibleInteraction, { steps })} /><div className="inspector-actions"><button onClick={()=>onTest(visibleInteraction)}>Test Multi Action</button><button className="danger subtle" onClick={()=>onClear(visibleInteraction)}>Clear Binding</button></div></> : binding?.definitionId === 'editor.keyLogic' ? <><KeyLogicEditor press={(binding.config.press as ActionInstance | null) ?? null} doublePress={(binding.config.doublePress as ActionInstance | null) ?? null} longPress={(binding.config.longPress as ActionInstance | null) ?? null} onChange={(trigger, action) => onConfig(visibleInteraction, { [trigger]: action })} /><div className="inspector-actions"><button onClick={()=>onTest(visibleInteraction)}>Test Press Branch</button><button className="danger subtle" onClick={()=>onClear(visibleInteraction)}>Clear Binding</button></div></> : binding && isPluginDefinitionId(binding.definitionId) ? <><div className="binding-title"><strong>{getActionDefinition(binding.definitionId).group}</strong><span>{getActionDefinition(binding.definitionId).label}</span></div><PluginPropertyInspector action={binding} /><div className="inspector-actions"><button onClick={()=>onTest(visibleInteraction)}>Test Action</button><button className="danger subtle" onClick={()=>onClear(visibleInteraction)}>Clear Binding</button></div></> : <ActionInspector slot={slot} interaction={visibleInteraction} pages={pages} profiles={profiles} onInteraction={onInteraction} onConfig={(patch)=>onConfig(visibleInteraction,patch)} onClear={()=>onClear(visibleInteraction)} onTest={()=>onTest(visibleInteraction)} />) : tab==='appearance' ? <AppearanceInspector slot={slot} onUpdate={onAppearance} onOpenAssets={(role)=>onOpenAssets(role,'base')} /> : <StatesInspector slot={slot} previewState={previewState} onPreviewState={onPreviewState} onUpdate={onState} onReset={onResetState} onCopyDefault={onCopyState} onOpenAssets={(role)=>onOpenAssets(role,'active')} />}
      </div>
    </div>}
  </section>;
});
