...
export function PropertyInspector({ slot, pages, profiles, collapsed, onToggleCollapsed, onConfig, onClear, onTest, onAppearance, onState, onResetState, onCopyState, previewState, onPreviewState, onOpenAssets }: Props) {
  ...
  if (collapsed) {
    return <section className="property-inspector collapsed" data-layout-region="inspector"><div className="inspector-header collapsed-header"><div><strong>Property Inspector</strong><span>{slot ? `${slot.kind} ${slot.position + 1}` : 'No selection'}</span></div><button aria-label="Expand property inspector" title="Expand Property Inspector" onClick={onToggleCollapsed}>⌃</button></div></section>;
  }

  return <section className="property-inspector" data-layout-region="inspector">
