...
interface ActionLibraryProps { mode: ActionLibraryMode; collapsed: boolean; onModeChange: (mode: ActionLibraryMode) => void; onChoose: (id: string) => void; onToggleCollapsed: () => void; externalActions?: ActionDefinition[]; qualificationCatalogSize?: number; }
...
export const ActionLibrary = memo(function ActionLibrary({ mode, collapsed, onModeChange, onChoose, onToggleCollapsed, externalActions = [], qualificationCatalogSize = 0 }: ActionLibraryProps) {
  const [query, setQuery] = useState('');
  const deferredQuery = useDeferredValue(query);
  const [collapsedGroups, setCollapsedGroups] = useState<Set<string>>(() => new Set(EXTRA_GROUPS));
  const rows = useMemo<Row[]>(() => {
    const needle = deferredQuery.trim().toLowerCase();
    const baseCatalog = [...ACTION_DEFINITIONS, ...externalActions];
    const catalog: ActionDefinition[] = qualificationCatalogSize > baseCatalog.length
      ? Array.from({ length: qualificationCatalogSize }, (_, index) => {
          const base = baseCatalog[index % baseCatalog.length];
    ...
    return result;
  }, [mode, deferredQuery, collapsedGroups, externalActions, qualificationCatalogSize]);
  ...
  return <aside className="action-library" data-layout-region="actions" aria-label="Actions">
    ...
    <div className="action-scroll"><VirtualActionList items={rows} rowHeight={42} viewportHeight={740} renderRow={(row) => row.kind === 'group' ? <button className="action-group-heading" aria-expanded={!row.collapsed} onClick={() => toggleGroup(row.group)}><span className="group-icon"><Glyph name={groupGlyph(row.group)} /></span><strong>{row.group}</strong><span className="group-chevron">{row.collapsed ? '⌄' : '⌃'}</span></button> : <button key={row.action.id} draggable className="action-entry" title={row.action.label} onDragStart={(event) => { event.dataTransfer.effectAllowed='copy'; event.dataTransfer.setData(ACTION_MIME,row.action.id); }} onClick={() => onChoose(row.action.id)}><span className="action-entry-icon"><Glyph name={glyphForLabel(row.action.label)} /></span><span>{row.action.label}</span></button>} /></div>
  </aside>;
});
