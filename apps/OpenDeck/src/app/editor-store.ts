...
export type EditorAction =
  ...
  | { type: 'IMPORT_PROFILE'; profile: Profile; activate?: boolean }
  ...
  switch (action.type) {
    ...
    case 'RENAME_PROFILE':
      return commit(state, (workspace) => {
        ...
        if (profile && !profile.pluginReadonly && action.name.trim()) profile.name = action.name.trim();
      });
    ...
    case 'DELETE_PROFILE': {
      ...
      if (state.workspace.profiles.find((profile) => profile.id === action.profileId)?.pluginReadonly) return state;
      return commit(state, (workspace) => {
    ...
    case 'IMPORT_PROFILE':
      return commit(state, (workspace) => {
        const source = structuredClone(action.profile);
        const profile = createProfile(source.name || `Imported Profile ${workspace.profiles.length + 1}`, source.device_id || workspace.active_device_id);
        const pageIds = new Map<string, string>();
        profile.app_match = Array.isArray(source.app_match) ? structuredClone(source.app_match) : [];
        profile.pluginOwnerUuid = source.pluginOwnerUuid ?? null;
        profile.pluginReadonly = source.pluginReadonly === true;
        profile.pages = source.pages.map((sourcePage) => {
          const page = createPage(sourcePage.name);
          pageIds.set(sourcePage.id, page.id);
          page.parentFolderId = sourcePage.parentFolderId;
        ...
        remapFolderPageReferences(profile, pageIds);
        profile.active_page_id = pageIds.get(source.active_page_id) ?? profile.pages[0]?.id ?? createPage().id;
        const existingIndex = profile.pluginOwnerUuid
          ? workspace.profiles.findIndex((candidate) => candidate.pluginOwnerUuid === profile.pluginOwnerUuid && candidate.name === profile.name)
          : -1;
        const wasActive = existingIndex >= 0 && workspace.active_profile_id === workspace.profiles[existingIndex].id;
        if (existingIndex >= 0) {
          profile.id = workspace.profiles[existingIndex].id;
          workspace.profiles[existingIndex] = profile;
        } else {
          workspace.profiles.push(profile);
        }
        if (action.activate !== false || wasActive) {
          workspace.active_profile_id = profile.id;
          workspace.active_device_id = profile.device_id;
        }
      });
    case 'MOVE_CONTROL':
      return commit(state, (workspace) => {
