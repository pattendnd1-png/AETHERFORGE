import { createPage, createProfile, cloneWorkspace, findSlot, getActivePage, getActiveProfile, type ActionInstance, type Appearance, type AppearanceOverride, type AssetRecord, type ControlSelection, type Interaction, type Workspace } from '../model/workspace';
...
export function editorReducer(state: EditorState, action: EditorAction): EditorState {
  switch (action.type) {
    ...
    case 'REMOVE_ACTION':
      return commit(state, (workspace) => {
        const slot = state.selection ? slotFor(workspace, state.selection) : undefined;
        if (slot) delete slot.bindings[action.interaction];
      });
    case 'UPDATE_ACTION_CONFIG':
      return commit(state, (workspace) => {
        const slot = state.selection ? slotFor(workspace, state.selection) : undefined;
        const binding = slot?.bindings[action.interaction];
        if (binding) binding.config = { ...binding.config, ...action.patch };
      });
    ...
    case 'DELETE_PAGE':
      return commit(state, (workspace) => {
        ...
        profile.pages = profile.pages.filter((page) => page.id !== profile.active_page_id);
    ...
    case 'DUPLICATE_PROFILE':
      return commit(state, (workspace) => {
        ...
        const replacement = createProfile(`${source.name} Copy`, source.device_id);
        profile.id = replacement.id;
        profile.name = replacement.name;
        profile.pages = source.pages.map((sourcePage, pageIndex) => {
          ...
          if (pageIndex === 0) profile.active_page_id = page.id;
          return page;
        });
        workspace.profiles.push(profile);
        workspace.active_profile_id = profile.id;
    ...
    case 'DELETE_PROFILE':
      return commit(state, (workspace) => {
        if (workspace.profiles.length <= 1) return;
