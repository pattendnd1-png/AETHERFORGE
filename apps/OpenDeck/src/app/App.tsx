...
export default function EditorApp({ qualification = DEFAULT_QUALIFICATION }: EditorAppProps) {
  ...
  async function installPlugin(path: string, activate = true) {
    ...
    if (activate) await bridge.pluginSetActive(plugin.uuid, true);
  ...
  async function installIconPack(path: string) {
    ...
    setStatus(`Installed and activated icon pack ${pack.name} (${pack.itemCount} icons).`);
  }

  async function installPackage(path: string) {
    ...
    if (normalized.endsWith('.streamdeckprofile')) {
      await importProfile(path, true);
      return;
    }
    await installPlugin(path, true);
  }
  ...
  return <div className="opendeck-viewport">
    ...
    <TopBar
      ...
      onMarketplace={() => { setSection('plugins'); setPanel('none'); void refreshPlugins(); void refreshIconPacks(); void scanMarketplace(); }}
    ...
    <div className={`app-body workspace-grid${section === 'profiles' || section === 'plugins' ? ' management-view' : ''}`}>
      <AppSidebar section={section} onSectionChange={(next) => {
        ...
        if (next === 'plugins') { setPanel('none'); void refreshPlugins(); void refreshIconPacks(); void scanMarketplace(); }
