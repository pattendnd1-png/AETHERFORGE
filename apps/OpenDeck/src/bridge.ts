...
import type { ActiveApplicationContext, IconPackDescriptor, MarketplaceItem, StreamDeckStatus, TwitchDeviceCode, TwitchIdentity } from './types';
...
export const bridge = {
  ...
  iconPackList: () => invoke<IconPackDescriptor[]>('icon_pack_list'),
  iconPackInstall: (path: string) => invoke<IconPackDescriptor>('icon_pack_install', { path }),
  iconPackSetActive: (packId: string, active: boolean) => invoke<void>('icon_pack_set_active', { packId, active }),
  iconPackRemove: (packId: string) => invoke<void>('icon_pack_remove', { packId }),
