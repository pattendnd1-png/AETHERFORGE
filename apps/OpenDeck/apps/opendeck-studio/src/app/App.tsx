...
import type { IconPackDescriptor, MarketplaceItem, StreamDeckInputEvent, StreamDeckStatus, TwitchDeviceCode, TwitchIdentity } from '../types';
...
export default function EditorApp({ qualification = DEFAULT_QUALIFICATION }: EditorAppProps) {
  ...
  const marketplaceScanBusyRef = useRef(false);
  const [plugins, setPlugins] = useState<PluginDescriptor[]>([]);
  const [iconPacks, setIconPacks] = useState<IconPackDescriptor[]>([]);
  ...
  const [hardwareStatus, setHardwareStatus] = useState<StreamDeckStatus>(qualificationMode
    ? { state: 'connected', model: 'Stream Deck +', serial: 'QUALIFY-V247', message: 'Connected' }
  ...
  const refreshIconPacks = useCallback(async () => {
    setIconPacks(await bridge.iconPackList());
  }, []);

  const refreshAssets = useCallback(async () => {
    dispatch({ type: 'SET_ASSETS', assets: await bridge.editorListAssets() });
  }, []);
    ...
    const run = async (current: ActionInstance, nestedMultiAction: boolean, depth: number): Promise<void> => {
      ...
      switch (execution.kind) {
        ...
        case 'obs.toggleMute':
          ...
          return;
        case 'obs.toggleApp': {
          const result = await bridge.obsToggleApp();
          setStatus(result === 'launched' ? 'OBS launched.' : 'OBS closed.');
          return;
        }
  ...
  useEffect(() => {
    ...
    void refreshIconPacks().catch((error) => { if (alive) setStatus(`Icon-pack warning: ${String(error)}`); });
  ...
  }, [qualificationMode, refreshIconPacks, refreshPlugins]);
  ...
  async function scanMarketplace() {
    if (marketplaceScanBusyRef.current) return;
    marketplaceScanBusyRef.current = true;
    setStatus('Scanning Downloads for Stream Deck packages…');
    try {
      const items = await bridge.scanMarketplace();
      setMarketItems(items);
      setStatus(`Downloads scan complete: ${items.length} compatible package${items.length === 1 ? '' : 's'} found.`);
    } catch (error) {
      setStatus(`Downloads scan failed: ${String(error)}`);
    } finally {
      marketplaceScanBusyRef.current = false;
    }
  }
  ...
  async function importProfile(path: string, activate = false) {
    const imported = await bridge.editorImportProfile(path);
    dispatch({ type: 'IMPORT_PROFILE', profile: imported, activate });
    setStatus(`${activate ? 'Imported and activated' : 'Imported'} profile ${imported.name}.`);
  }
  ...
  async function installPlugin(path: string, activate = true) {
    ...
    if (activate) {
      try {
        await bridge.pluginSetActive(plugin.uuid, true);
      } catch (error) {
        await refreshPlugins();
        setStatus(`Installed ${plugin.name}, but activation failed: ${String(error)}`);
        return;
      }
    }
    await refreshPlugins();
    setStatus(`${activate ? 'Installed and activated' : 'Installed'} ${plugin.name}${importedCount ? ` with ${importedCount} bundled profile${importedCount === 1 ? '' : 's'}` : ''}.`);
  }
  ...
  async function installPackage(path: string, activate = true) {
    ...
    if (normalized.endsWith('.streamdeckiconpack')) {
      await installIconPack(path);
      return;
    }
    if (normalized.endsWith('.streamdeckprofile')) {
      await importProfile(path, activate);
      return;
    }
    await installPlugin(path, activate);
  }

  async function setPluginEnabled(uuid: string, enabled: boolean) {
    await bridge.pluginSetEnabled(uuid, enabled);
    await refreshPlugins();
    setStatus(`${enabled ? 'Enabled' : 'Disabled'} plugin.`);
  }

  async function setPluginActive(uuid: string, active: boolean) {
    await bridge.pluginSetActive(uuid, active);
    await refreshPlugins();
    setStatus(`${active ? 'Activated' : 'Deactivated'} plugin.`);
  }
  ...
  return <div className="opendeck-viewport">
    ...
    <TopBar
      ...
      onMarketplace={() => { setSection('plugins'); setPanel('none'); void refreshPlugins(); void refreshIconPacks(); }}
    ...
    <div className={`app-body workspace-grid${section === 'profiles' || section === 'plugins' ? ' management-view' : ''}`}>
      <AppSidebar section={section} onSectionChange={(next) => {
        ...
        if (next === 'plugins') { setPanel('none'); void refreshPlugins(); void refreshIconPacks(); }
      ...
      <main className="main-editor" data-layout-region="main">
        ...
        /> : section === 'plugins' ? <PluginManager
          ...
          onSetIconPackActive={setIconPackActive}
          onRemoveIconPack={removeIconPack}
