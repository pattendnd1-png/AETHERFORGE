...
vi.mock('./bridge', () => ({ bridge: {
  ...
  iconPackList: vi.fn(), iconPackInstall: vi.fn(), iconPackSetActive: vi.fn(), iconPackRemove: vi.fn(),
...
beforeEach(() => {
  ...
  vi.mocked(bridge.pluginHostStatus).mockResolvedValue({ protocolVersion: '2.0.55', streamDeckCompatibilityTarget: '7.6', websocketHost: '127.0.0.1', installed: 0, active: 0, enabled: 0 });
  ...
  vi.mocked(bridge.iconPackList).mockResolvedValue([]);
  vi.mocked(bridge.iconPackInstall).mockRejectedValue(new Error('not used'));
  vi.mocked(bridge.iconPackSetActive).mockResolvedValue(undefined);
  vi.mocked(bridge.iconPackRemove).mockResolvedValue(undefined);
  ...
  vi.mocked(bridge.qualificationFocusWindow).mockResolvedValue({ release: '2.0.55', pid: 1234, title: 'OpenDeck+ 2.0.55 Qualification [1234]' });
...
describe('OpenDeck 2.0.55 editor', () => {
  ...
  it('opens Plugins & Packs without automatically scanning Downloads', async () => {
    await renderEditor();
    fireEvent.click(screen.getByRole('button', { name: /Plugins & Packs/i }));
    expect(await screen.findByLabelText('Plugins and Packs')).toBeInTheDocument();
    expect(bridge.scanMarketplace).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole('button', { name: 'Scan Downloads' }));
    await waitFor(() => expect(bridge.scanMarketplace).toHaveBeenCalledTimes(1));
  });
