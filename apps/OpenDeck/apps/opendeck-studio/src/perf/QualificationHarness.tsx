...
async function benchmarkUi(workspace: Workspace): Promise<Record<string, unknown>> {
  ...
  return { schemaVersion: 1, release: '2.0.55', interaction: metricSnapshot(), framePacing: await framePacingSample() };
}
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (focusAck.release !== '2.0.55') throw new Error(`Qualification focus release mismatch: ${focusAck.release}`);
