...
function parseSelection(raw: string): ControlSelection | null {
  try {
    ...
    if ((value.kind === 'key' || value.kind === 'dial' || value.kind === 'touch') && typeof value.slotId === 'string') {
      return { kind: value.kind, slotId: value.slotId };
    }
  } catch { /* ignore invalid drag payloads */ }
  return null;
}

export function DeviceEditor({ page, selection, assetPreviews, previewState, onSelect, onDropControl, onDropAction }: DeviceEditorProps) {
  ...
  function dragHandlers(destination: ControlSelection) {
    return {
      ...
      onDrop: (event: DragEvent<HTMLButtonElement>) => {
        ...
        if (actionId) {
          onDropAction(actionId, destination);
          return;
        }
