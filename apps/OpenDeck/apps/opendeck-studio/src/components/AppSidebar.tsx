...
const ITEMS: Array<{ id: AppSection; label: string; sublabel: string; icon: GlyphName }> = [
  ...
  { id: 'plugins', label: 'Plugins & Packs', sublabel: 'Select & activate', icon: 'plugins' },
...
export const AppSidebar = memo(function AppSidebar({ section, onSectionChange }: AppSidebarProps) {
  return <aside className="app-sidebar" data-layout-region="sidebar">
    ...
    <footer className="sidebar-footer"><span className="sidebar-heart">♥</span><div><strong>OpenDeck+ 2.0.50</strong><small>Open Source. More possibilities.</small></div></footer>
  </aside>;
});
