...
export const KeyControl = memo(function KeyControl({ slot, appearance, selected, iconUrl, backgroundUrl, onSelect, onDragStart, onDragOver, onDrop }: KeyControlProps) {
  ...
  return <button aria-label={`Key ${slot.position + 1}: ${title}`} draggable data-testid="deck-key" data-qualify-element="key" className={`control key-control${selected ? ' selected' : ''}`} style={style} onClick={onSelect} onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop}>
