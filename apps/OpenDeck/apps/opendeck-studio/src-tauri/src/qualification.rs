...
pub fn enabled() -> bool {
    std::env::var("OPENDECK_V242_QUALIFICATION").is_ok_and(|value| value == "1")
}

pub fn phase() -> String {
    std::env::var("OPENDECK_V242_QUALIFICATION_PHASE")
        .ok()
...
pub fn workspace_benchmark_path() -> Result<PathBuf, String> {
    Ok(output_dir()?.join(".OpenDeck-v2.0.42-workspace-benchmark.json"))
}
...
pub fn qualification_focus_window(app: tauri::AppHandle) -> Result<Value, String> {
    if !enabled() {
        return Err("OpenDeck v2.0.42 qualification mode is not enabled".into());
    }
    let pid = std::process::id();
    let title = format!("OpenDeck+ 2.0.42 Qualification [{pid}]");
    let window = app
    ...
    atomic_json(&output_dir()?.join("OpenDeck-v2.0.42-FOCUS-ACK.json"), &ack)?;
    Ok(ack)
}
...
pub fn qualification_record_ui_metrics(payload: Value) -> Result<(), String> {
    ...
    let name = match kind {
        "visual" => "OpenDeck-v2.0.42-VISUAL-METRICS.json",
        "performance" => "OpenDeck-v2.0.42-PERFORMANCE-METRICS.json",
        "startup" => "OpenDeck-v2.0.42-STARTUP-METRICS.json",
        _ => "OpenDeck-v2.0.42-QUALIFICATION-ERROR.json",
    };
