...
const HOST_PROTOCOL_VERSION: &str = "2.0.45";
...
impl Service {
    ...
    fn app(&self) -> Result<AppHandle, String> {
        self.state
            ...
            .app
            .clone()
            .ok_or_else(|| "plugin host is not initialized".to_string())
    }
    ...
    fn plugin(&self, uuid: &str) -> Result<PluginDescriptor, String> {
        self.state
            ...
            .plugins
            .iter()
            .find(|plugin| plugin.uuid == uuid)
            .cloned()
            .ok_or_else(|| format!("plugin {uuid} is not installed"))
    }
    ...
    async fn start_plugin(&self, uuid: &str) -> Result<(), String> {
        ...
        if plugin.compatibility == "unsupported" {
            return Err(plugin
                .last_error
                .unwrap_or_else(|| "plugin runtime is unsupported".into()));
        }

        self.set_process_state(uuid, "starting", None);
        let listener = TcpListener::bind("127.0.0.1:0")
            .await
            .map_err(|error| format!("plugin websocket bind: {error}"))?;
        let port = listener
            .local_addr()
...

        {
            ...
            if let Some(stored) = state
                ...
                .plugins
                .iter_mut()
                .find(|stored| stored.uuid == plugin.uuid)
            {
                stored.active = true;
    ...
    fn outbound(&self, uuid: &str) -> Result<broadcast::Sender<String>, String> {
        ...
        state
            ...
            .ok_or_else(|| format!("plugin {uuid} is not active"))
    }
...
fn config_root() -> Result<PathBuf, String> {
    if let Some(root) = std::env::var_os("XDG_CONFIG_HOME") {
        return Ok(PathBuf::from(root).join("opendeck-v2/plugins"));
    }
...
fn cache_root() -> Result<PathBuf, String> {
    if let Some(root) = std::env::var_os("XDG_CACHE_HOME") {
        return Ok(PathBuf::from(root).join("opendeck-v2/plugin-feedback"));
    }
...
fn contexts_path() -> Result<PathBuf, String> {
    Ok(config_root()?.join("contexts.json"))
}
...
fn write_json<T: Serialize>(path: &Path, value: &T) -> Result<(), String> {
    let parent = path
        .parent()
        .ok_or_else(|| "invalid plugin persistence path".to_string())?;
...
fn write_context_store(value: &ContextStore) -> Result<(), String> {
    write_json(&contexts_path()?, value)
}
...
fn supported_kinds(controllers: &[String]) -> Vec<String> {
    ...
    if controllers
        .iter()
        .any(|controller| controller == "Keypad" || controller == "Neo")
    {
        result.push("key".into());
    }
    if controllers.iter().any(|controller| controller == "Encoder") {
        result.push("dial".into());
        result.push("touch".into());
    }
    result
}
...
fn descriptor_from_elgato(root: &Path, manifest: ElgatoManifest) -> PluginDescriptor {
    ...
    let runtime_kind = if manifest.nodejs.is_some()
        || lower.ends_with(".js")
        || lower.ends_with(".mjs")
        || lower.ends_with(".cjs")
    {
    ...
    let runtime_error = match runtime_kind.as_deref() {
        Some("wine") if compatibility == "unsupported" => {
            Some("Windows plugin requires Wine or Wine64".into())
        }
    ...
    PluginDescriptor {
        ...
        profiles: manifest
            ...
            .unwrap_or_default()
            .into_iter()
            .map(|profile| PluginProfileDescriptor {
                name: profile.name,
                device_type: profile.device_type,
                readonly: profile.readonly.unwrap_or(false),
                auto_install: profile.auto_install.unwrap_or(true),
                dont_auto_switch_when_installed: profile
                    .dont_auto_switch_when_installed
                    .unwrap_or(false),
            })
            .collect(),
        ...
        secrets_path: root
            .join("secrets.json")
            .is_file()
            .then(|| root.join("secrets.json").display().to_string()),
    }
}
...
fn descriptor_from_opendeck(root: &Path, manifest: OpenDeckManifest) -> PluginDescriptor {
    ...
    let compatibility = if manifest.runtime == "node" && command_exists("node") {
        "node"
    } else if manifest.runtime == "native" && entry.exists() {
        "native"
    } else {
        "unsupported"
    }
...
fn runtime_command(plugin: &PluginDescriptor, entry: &Path) -> Result<Command, String> {
    match plugin.runtime_kind.as_deref() {
        ...
        Some("wine") => {
            let wine = if command_exists("wine64") {
                "wine64"
            } else {
                "wine"
            };
...
async fn client_loop(
    ...
    let websocket = accept_async(stream)
        .await
        .map_err(|error| format!("plugin websocket accept: {error}"))?;
    ...
    match event {
        "getSettings" => {
            ...
            let response_payload =
                context_response_payload(service, &context, settings, json!({}))?;
        ...
        "setSettings" => {
            let payload = message.get("payload").cloned().unwrap_or_else(|| json!({}));
            {
                ...
                state
                    .contexts
                    .settings
                    .insert(context.clone(), payload.clone());
        ...
        "setGlobalSettings" => {
            let payload = message.get("payload").cloned().unwrap_or_else(|| json!({}));
            {
                ...
                state
                    .contexts
                    .global_settings
                    .insert(plugin.uuid.clone(), payload.clone());
                Service::save_locked(&state)?;
            }
            let _ = outbound.send(
                json!({"event":"didReceiveGlobalSettings","payload":{"settings":payload}})
                    .to_string(),
            );
        }
        "getResources" => {
            ...
            let response_payload = context_response_payload(
                service,
                &context,
                json!({}),
                serde_json::to_value(resources).map_err(|error| error.to_string())?,
            )?;
        ...
        "setResources" => {
            let resources: HashMap<String, String> = serde_json::from_value(
                message.get("payload").cloned().unwrap_or_else(|| json!({})),
            )
            .unwrap_or_default();
            {
                let mut state = service
                ...
                state
                    .contexts
                    .resources
                    .insert(context.clone(), resources.clone());
                Service::save_locked(&state)?;
            }
            let response_payload = context_response_payload(
                service,
        ...
        "getSecrets" => {
            let secrets = plugin
                ...
                .unwrap_or_else(|| json!({}));
            let _ = outbound.send(
                json!({"event":"didReceiveSecrets","payload":{"secrets":secrets}}).to_string(),
            );
        }
        ...
        "logMessage" => {
            if let Some(line) = message.pointer("/payload/message").and_then(Value::as_str) {
                write_plugin_log(plugin, line);
            }
        }
        _ => {
...
fn emit_feedback(
    ...
    match event {
        ...
        "setFeedbackLayout" => {
            feedback.feedback_layout = payload
                .get("layout")
                .and_then(Value::as_str)
                .map(str::to_owned)
        }
    ...
    app.emit(PLUGIN_FEEDBACK_EVENT, feedback)
        .map_err(|error| error.to_string())
}

fn materialize_feedback_image(
    plugin: &PluginDescriptor,
    image: &str,
    ...
    let (bytes, extension, mime, data_url) = if let Some((header, encoded)) = image.split_once(',')
    {
        if header.starts_with("data:image/") && header.contains(";base64") {
            let bytes = STANDARD
                .decode(encoded)
                .map_err(|error| format!("plugin image base64: {error}"))?;
            let (ext, mime) = if header.contains("svg+xml") {
                ("svg", "image/svg+xml")
        ...
        let root = PathBuf::from(&plugin.root)
            .canonicalize()
            .map_err(|error| format!("plugin root: {error}"))?;
        ...
        let candidate = candidate
            .canonicalize()
            .map_err(|error| format!("plugin image {}: {error}", image))?;
        if !candidate.starts_with(&root) {
            return Err("plugin image path escaped the plugin directory".into());
        }
        let bytes = fs::read(&candidate)
            .map_err(|error| format!("read plugin image {}: {error}", candidate.display()))?;
        let extension = candidate
            .extension()
            .and_then(|value| value.to_str())
            .unwrap_or("png")
            .to_ascii_lowercase();
        let (extension, mime) = match extension.as_str() {
...
fn extract_package(source: &Path, destination: &Path) -> Result<(), String> {
    ...
    let mut archive = ZipArchive::new(file).map_err(|error| {
        format!(
            "{} is not a readable Stream Deck package: {error}",
            source.display()
        )
    })?;
...
fn controller_for_request(request: &PluginDispatchRequest) -> &'static str {
    if request.control_kind == "dial" || request.control_kind == "touch" {
        "Encoder"
    } else {
        "Keypad"
    }
...
fn remember_context(service: &Service, request: &PluginDispatchRequest) -> Result<(), String> {
    let metadata = ContextMetadata {
        ...
        device: if request.device.is_empty() {
            DEVICE_ID.into()
        } else {
    ...
    state
        .contexts
        .metadata
        .insert(request.context.clone(), metadata);
    Service::save_locked(&state)
}
...
fn maybe_toggle_automatic_state(
    service: &Service,
    plugin: &PluginDescriptor,
    ...
    let action = plugin
        .actions
        .iter()
        .find(|candidate| candidate.uuid == request.action_uuid);
    let current = current_context_state(service, &request.context)?;
    let should_toggle = event == "keyDown"
        && !request.is_in_multi_action
        && action
            .is_some_and(|action| action.states.len() == 2 && !action.disable_automatic_states);
    if should_toggle {
...
fn dispatch_messages(
    ...
    let build = |event: &str,
     ...
     -> Result<Value, String> {
        ...
        Ok(registration_payload(
            &plugin, request, event, extra, settings, resources,
        ))
    };
    if let Some(raw) = request.hardware_event.as_ref() {
        ...
        let message = match kind {
            ...
            "dialRotate" => build(
                "dialRotate",
                json!({"ticks":raw.get("ticks").cloned().unwrap_or(json!(0)),"pressed":raw.get("pressed").cloned().unwrap_or(json!(false))}),
                settings,
                resources,
            )?,
            "touchTap" | "touchPress" => build(
                "touchTap",
                json!({"hold":kind=="touchPress","tapPos":[raw.get("x").cloned().unwrap_or(json!(0)),raw.get("y").cloned().unwrap_or(json!(0))]}),
                settings,
                resources,
            )?,
            _ => return Ok(Vec::new()),
    ...
    let messages = match (request.control_kind.as_str(), interaction) {
        ...
        ("dial", "press") => vec![
            build("dialDown", json!({}), settings.clone(), resources.clone())?,
            build("dialUp", json!({}), settings, resources)?,
        ],
        ("dial", "rotateLeft" | "pressRotateLeft") => vec![build(
            "dialRotate",
            json!({"ticks":-1,"pressed":interaction.starts_with("press")}),
            settings,
            resources,
        )?],
        ("dial", "rotateRight" | "pressRotateRight") => vec![build(
            "dialRotate",
            json!({"ticks":1,"pressed":interaction.starts_with("press")}),
            settings,
            resources,
        ...
        ("touch", _) => vec![build(
            "touchTap",
            json!({"hold":interaction=="longPress","tapPos":[0,0]}),
            settings,
            resources,
        )?],
        _ => Vec::new(),
...
pub(crate) async fn plugin_install(
        ...
        .map_err(|error| error.to_string())??;
    {
        ...
        state
            .registry
            .plugins
            .retain(|plugin| plugin.uuid != descriptor.uuid);
...
pub(crate) async fn plugin_remove(
    ...
    let root = {
        ...
        state
            .registry
            .plugins
            .retain(|plugin| plugin.uuid != plugin_uuid);
        Service::save_locked(&state)?;
        root
    };
...
pub(crate) async fn plugin_set_enabled(
    ...
    }
    {
        ...
        let plugin = state
            ...
            .iter_mut()
            .find(|plugin| plugin.uuid == plugin_uuid)
            .ok_or_else(|| format!("plugin {plugin_uuid} not found"))?;
...
#[tauri::command]
pub(crate) async fn plugin_set_active(
    plugin_uuid: String,
    active: bool,
    service: State<'_, Service>,
) -> Result<(), String> {
...
#[tauri::command]
pub(crate) async fn plugin_restart(
    plugin_uuid: String,
    service: State<'_, Service>,
) -> Result<(), String> {
    let cloned = service.inner().clone();
...
#[tauri::command]
pub(crate) async fn plugin_dispatch_action(
    request: PluginDispatchRequest,
    service: State<'_, Service>,
) -> Result<(), String> {
    let cloned = service.inner().clone();
...
#[tauri::command]
pub(crate) async fn plugin_dispatch_lifecycle(
    ...
    let payload = registration_payload(
        ...
        &action_request,
        &request.event,
        json!({"state": action_state}),
        settings,
        resources,
    );
    let _ = cloned
        .outbound(&request.plugin_uuid)?
...
#[tauri::command]
pub(crate) fn plugin_dispatch_host_event(
    request: PluginHostEventRequest,
    service: State<'_, Service>,
) -> Result<(), String> {
    let mut message = json!({"event": request.event, "payload": request.payload});
    if let Some(device) = request.device {
        message["device"] = Value::String(device);
    }
    if let Some(application) = request.application {
        message["application"] = Value::String(application);
    }
    service.broadcast_to_active(&message)
}

#[tauri::command]
pub(crate) fn plugin_import_bundled_profile(
    plugin_uuid: String,
    profile_name: String,
    service: State<'_, Service>,
) -> Result<editor::Profile, String> {
    ...
    let profile = plugin
        ...
        .ok_or_else(|| {
            format!("bundled profile {profile_name} is not declared by {plugin_uuid}")
        })?;
    if profile.device_type != DEVICE_TYPE_STREAM_DECK_PLUS {
        return Err(format!(
            "bundled profile targets Stream Deck device type {}, not Stream Deck + ({})",
            profile.device_type, DEVICE_TYPE_STREAM_DECK_PLUS
        ));
    }
    let relative = format!("{}.streamDeckProfile", profile.name);
    let path = PathBuf::from(&plugin.root).join(relative);
    if !path.is_file() {
        return Err(format!("bundled profile is missing: {}", path.display()));
    }
...
#[tauri::command]
pub(crate) async fn plugin_property_inspector_session(
    request: PropertyInspectorRequest,
    service: State<'_, Service>,
) -> Result<PropertyInspectorSession, String> {
    let cloned = service.inner().clone();
    if cloned.outbound(&request.plugin_uuid).is_err() {
        cloned.start_plugin(&request.plugin_uuid).await?;
    }
    ...
    let action = plugin
        .actions
        .iter()
        .find(|action| action.uuid == request.action_uuid)
        .ok_or_else(|| "plugin action not found".to_string())?;
    let relative = action
        .property_inspector_path
        .as_deref()
        .or(plugin.property_inspector_path.as_deref())
        .ok_or_else(|| "plugin action has no Property Inspector".to_string())?;
    let source = PathBuf::from(&plugin.root).join(relative);
    let pi_root = cache_root()?
        .join("property-inspectors")
        .join(&plugin.uuid)
        .join("root");
    if pi_root.exists() {
        fs::remove_dir_all(&pi_root).map_err(|error| error.to_string())?;
    }
    copy_tree(Path::new(&plugin.root), &pi_root)?;
    let relative_source = source
        .strip_prefix(Path::new(&plugin.root))
        .map_err(|error| error.to_string())?;
    let output = pi_root.join(relative_source);
    let mut html = fs::read_to_string(&output)
        .map_err(|error| format!("read Property Inspector {}: {error}", output.display()))?;
    let port = cloned.runtime_port(&request.plugin_uuid)?;
    let (pi_settings, pi_resources) = context_values(&cloned, &request.context)?;
    let pi_payload =
        context_response_payload(&cloned, &request.context, pi_settings, pi_resources)?;
    let action_info = json!({"action":request.action_uuid,"context":request.context,"device":DEVICE_ID,"payload":pi_payload});
    let bootstrap = format!(
        r#"<script>(()=>{{const connect=()=>{{if(typeof window.connectElgatoStreamDeckSocket==='function'){{window.connectElgatoStreamDeckSocket({port}, {uuid:?}, 'registerPropertyInspector', {info:?}, {action_info:?});}}else{{setTimeout(connect,25);}}}};connect();}})();</script>"#,
        uuid = plugin.uuid,
        info = registration_info(&plugin).to_string(),
        action_info = action_info.to_string()
    );
    if let Some(index) = html.rfind("</body>") {
        html.insert_str(index, &bootstrap);
    } else {
        html.push_str(&bootstrap);
    }
    fs::write(&output, html).map_err(|error| error.to_string())?;
    Ok(PropertyInspectorSession {
        plugin_uuid: request.plugin_uuid,
        action_uuid: request.action_uuid,
        context: request.context,
...
mod tests {
    ...
    fn registration_info_reports_stream_deck_plus() {
        let plugin = PluginDescriptor {
            ...
            active: false,
            process_state: "inactive".into(),
            compatibility: "node".into(),
            runtime_kind: Some("node".into()),
            last_error: None,
            sdk_version: 3,
            minimum_software_version: Some("7.6".into()),
            property_inspector_path: None,
            actions: vec![],
            profiles: vec![],
            entry_path: "/tmp/test/index.js".into(),
