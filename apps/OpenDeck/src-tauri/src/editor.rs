    ...
    for (position, slot) in slots.iter().enumerate() {
        ...
        if let Some(wheel) = &slot.action_wheel {
            ...
            if slot.dial_stack.is_some() {
                return Err("dial control may not contain both a dial stack and action wheel".into());
            }
...
fn streamdeck_folder_id(uuid: &str) -> Result<String, String> {
    ...
    let encoded: String = encoded
        .to_ascii_uppercase()
        .chars()
        .map(|ch| match ch { 'U' => 'V', 'V' => 'W', other => other })
        .collect();
    Ok(format!("{encoded}Z"))
}

fn read_zip_json(archive: &mut ZipArchive<File>, name: &str) -> Result<serde_json::Value, String> {
    let mut file = archive.by_name(name).map_err(|error| format!("read {name}: {error}"))?;
...
fn profile_action_instance(action: &serde_json::Value, bundled_plugin_uuid: Option<&str>) -> Option<serde_json::Value> {
    ...
    let definition_id = match action_uuid {
        ...
        _ => {
            let action_plugin_uuid = action.pointer("/Plugin/UUID").and_then(serde_json::Value::as_str)
    ...
    let state = action.get("State").and_then(serde_json::Value::as_u64).unwrap_or(0);
    let settings = action.get("Settings").cloned().unwrap_or_else(|| serde_json::json!({}));
...
fn apply_profile_action(slot: &mut ControlSlot, action: &serde_json::Value, binding: serde_json::Value) {
    slot.bindings.insert("press".into(), binding);
    let state_index = action.get("State").and_then(serde_json::Value::as_u64).unwrap_or(0) as usize;
    if let Some(states) = action.get("States").and_then(serde_json::Value::as_array) {
        ...
        if let Some(state) = state {
            if let Some(title) = state.get("Title").and_then(serde_json::Value::as_str) { slot.appearance.title = title.into(); }
            if let Some(show) = state.get("ShowTitle").and_then(serde_json::Value::as_bool) { slot.appearance.title_visible = show; }
            if let Some(color) = state.get("TitleColor").and_then(serde_json::Value::as_str).filter(|value| !value.is_empty()) { slot.appearance.text_color = color.into(); }
            if let Some(font) = state.get("FontFamily").and_then(serde_json::Value::as_str) { slot.appearance.font_family = font.into(); }
            if let Some(size) = state.get("FontSize").and_then(serde_json::Value::as_u64) { slot.appearance.font_size = size.clamp(6, 96) as u16; }
            if let Some(align) = state.get("TitleAlignment").and_then(serde_json::Value::as_str) {
                slot.appearance.vertical_align = match align { "top" => "top", "middle" => "center", _ => "bottom" }.into();
            }
        }
    }
}

fn page_from_streamdeck_manifest(value: &serde_json::Value, name: String, bundled_plugin_uuid: Option<&str>) -> Result<Page, String> {
    let mut page = Page {
        id: id("page"),
        name,
        slots: PageSlots { keys: slots("key", 8), dials: slots("dial", 4), touch_regions: slots("touch", 4) },
        touch_strip: default_touch_strip_config(),
        parent_folder_id: None,
    };
    let controllers = value.get("Controllers").and_then(serde_json::Value::as_array).cloned().unwrap_or_default();
    for controller in controllers {
        let controller_type = controller.get("Type").and_then(serde_json::Value::as_str).unwrap_or("Keypad");
        let Some(actions) = controller.get("Actions").and_then(serde_json::Value::as_object) else { continue; };
        for (coordinate, action) in actions {
            let mut pieces = coordinate.split(',');
            let col = pieces.next().and_then(|value| value.parse::<usize>().ok()).unwrap_or(0);
            let row = pieces.next().and_then(|value| value.parse::<usize>().ok()).unwrap_or(0);
            let Some(binding) = profile_action_instance(action, bundled_plugin_uuid) else { continue; };
            if controller_type == "Encoder" {
                if col >= page.slots.dials.len() { continue; }
                let mut dial_binding = binding.clone();
                if let Some(config) = dial_binding.get_mut("config").and_then(serde_json::Value::as_object_mut) {
                    let shared_context = format!("plugin-context-{}", Uuid::new_v4());
                    config.insert("context".into(), serde_json::Value::String(shared_context.clone()));
                    for interaction in ["press", "rotateLeft", "rotateRight", "pressRotateLeft", "pressRotateRight"] {
                        let mut clone = dial_binding.clone();
                        if let Some(clone_config) = clone.get_mut("config").and_then(serde_json::Value::as_object_mut) { clone_config.insert("context".into(), serde_json::Value::String(shared_context.clone())); }
                        page.slots.dials[col].bindings.insert(interaction.into(), clone);
                    }
                    if col < page.slots.touch_regions.len() {
                        let mut touch = dial_binding.clone();
                        if let Some(touch_config) = touch.get_mut("config").and_then(serde_json::Value::as_object_mut) { touch_config.insert("context".into(), serde_json::Value::String(shared_context)); }
                        page.slots.touch_regions[col].bindings.insert("touch".into(), touch);
                    }
                }
                apply_profile_action(&mut page.slots.dials[col], action, binding);
            } else {
                let position = row.saturating_mul(4).saturating_add(col);
                if position >= page.slots.keys.len() { continue; }
...
pub(crate) fn import_streamdeck_profile(path: &Path, bundled_plugin_uuid: Option<&str>) -> Result<Profile, String> {
    let file = File::open(path).map_err(|error| format!("open {}: {error}", path.display()))?;
    let mut archive = ZipArchive::new(file).map_err(|error| format!("{} is not a readable .streamDeckProfile: {error}", path.display()))?;
    let names = (0..archive.len()).filter_map(|index| archive.by_index(index).ok().map(|file| file.name().to_string())).collect::<Vec<_>>();
    let root_manifest_name = names.iter().find(|name| name.ends_with(".sdProfile/manifest.json") && !name.contains("/Profiles/")).cloned().ok_or_else(|| "Stream Deck profile root manifest not found".to_string())?;
    let root_prefix = root_manifest_name.strip_suffix("manifest.json").unwrap_or_default().to_string();
    let root = read_zip_json(&mut archive, &root_manifest_name)?;
    let profile_name = root.get("Name").and_then(serde_json::Value::as_str).unwrap_or("Imported Stream Deck Profile").to_string();
    let mut pages = Vec::new();
    let mut source_page_ids = Vec::new();
    if let Some(page_ids) = root.pointer("/Pages/Pages").and_then(serde_json::Value::as_array) {
        for page_uuid in page_ids.iter().filter_map(serde_json::Value::as_str) {
            ...
            if names.iter().any(|name| name == &manifest_name) {
                ...
                let page_name = page_value.get("Name").and_then(serde_json::Value::as_str).unwrap_or("Page").to_string();
                pages.push(page_from_streamdeck_manifest(&page_value, page_name, bundled_plugin_uuid)?);
                source_page_ids.push(page_uuid.to_string());
            }
        }
    }
    if pages.is_empty() {
        let page_manifests = names.iter().filter(|name| name.starts_with(&format!("{root_prefix}Profiles/")) && name.ends_with("/manifest.json")).cloned().collect::<Vec<_>>();
        for (index, manifest_name) in page_manifests.iter().enumerate() {
            let page_value = read_zip_json(&mut archive, manifest_name)?;
            let page_name = page_value.get("Name").and_then(serde_json::Value::as_str).map(str::to_owned).unwrap_or_else(|| format!("Page {}", index + 1));
            pages.push(page_from_streamdeck_manifest(&page_value, page_name, bundled_plugin_uuid)?);
        }
    }
    if pages.is_empty() && root.get("Actions").is_some() {
        ...
        pages.push(page_from_streamdeck_manifest(&synthetic, "Page 1".into(), bundled_plugin_uuid)?);
    }
    if pages.is_empty() { return Err("Stream Deck profile contains no importable pages".into()); }
    let current_source = root.pointer("/Pages/Current").and_then(serde_json::Value::as_str);
    let current_index = current_source.and_then(|current| source_page_ids.iter().position(|candidate| candidate == current)).unwrap_or(0).min(pages.len() - 1);
    let active_page_id = pages[current_index].id.clone();
    Ok(Profile { id: id("profile"), name: profile_name, device_id: "stream-deck-plus".into(), pages, active_page_id, app_match: Vec::new(), plugin_owner_uuid: None, plugin_readonly: false })
}
...
pub(crate) fn editor_import_profile(path: String) -> Result<Profile, String> {
    ...
    if path.extension().is_some_and(|extension| extension.eq_ignore_ascii_case("streamDeckProfile")) {
...
mod tests {
    ...
    fn dial_stack_round_trip_preserves_active_entry_and_bindings() {
        ...
        let stack = parsed.profiles[0].pages[0].slots.dials[0].dial_stack.as_ref().unwrap();
    ...
    fn rejects_dial_stack_on_non_dial_control() {
        ...
        assert!(validate_workspace(&workspace).unwrap_err().contains("dial stack"));
    }
    ...
    fn action_wheel_round_trip_preserves_selection_and_press_binding() {
        ...
        let json = serde_json::to_string(&workspace).unwrap();
        let parsed: Workspace = serde_json::from_str(&json).unwrap();
        let wheel = parsed.profiles[0].pages[0].slots.dials[1].action_wheel.as_ref().unwrap();
    ...
    fn rejects_action_wheel_on_non_dial_control() {
        ...
        assert!(validate_workspace(&workspace).unwrap_err().contains("action wheel"));
    }
