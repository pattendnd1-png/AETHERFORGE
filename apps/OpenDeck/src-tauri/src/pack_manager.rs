...
fn validate_pack_id(pack_id: &str) -> Result<(), String> {
    if pack_id.is_empty()
        ...
        || pack_id.contains('/')
        || pack_id.contains('\\')
        || pack_id.chars().any(|ch| !(ch.is_ascii_alphanumeric() || matches!(ch, '.' | '_' | '-')))
    {
...
fn extract_package(source: &Path, destination: &Path) -> Result<(), String> {
    ...
    let mut archive = ZipArchive::new(file)
        .map_err(|error| format!("{} is not a readable .streamDeckIconPack: {error}", source.display()))?;
    fs::create_dir_all(destination).map_err(|error| error.to_string())?;
...
fn manifest_path(root: &Path) -> Result<PathBuf, String> {
    ...
    WalkDir::new(root)
        ...
        .find(|entry| entry.file_type().is_file() && entry.file_name().to_string_lossy().eq_ignore_ascii_case("manifest.json"))
...
fn parse_manifest(root: &Path, fallback_hash: &str) -> Result<StoredPackMeta, String> {
    ...
    let value: Value = serde_json::from_slice(&fs::read(&manifest).map_err(|error| error.to_string())?)
        .map_err(|error| format!("parse {}: {error}", manifest.display()))?;
    let name = value.get("Name").and_then(Value::as_str).unwrap_or("Stream Deck Icon Pack").trim().to_string();
    let version = value.get("Version").and_then(Value::as_str).unwrap_or("0.0.0").trim().to_string();
    let author = value.get("Author").and_then(Value::as_str).unwrap_or("Unknown").trim().to_string();
    let description = value.get("Description").and_then(Value::as_str).unwrap_or("").trim().to_string();
    ...
    let id = declared_id
        ...
        .unwrap_or_else(|| format!("icon-pack-{}", &fallback_hash[..12.min(fallback_hash.len())]));
    validate_pack_id(&id)?;
    Ok(StoredPackMeta { id, name, version, author, description })
}

fn is_icon_file(path: &Path) -> bool {
    path.extension()
        .and_then(|extension| extension.to_str())
        .map(|extension| matches!(extension.to_ascii_lowercase().as_str(), "svg" | "png" | "jpg" | "jpeg" | "gif" | "webp"))
        .unwrap_or(false)
}
...
fn icon_count(root: &Path) -> usize {
    WalkDir::new(root)
        ...
        .filter(|entry| entry.path().components().any(|component| component.as_os_str().to_string_lossy().eq_ignore_ascii_case("icons")))
        .count()
}
...
pub(crate) fn icon_pack_install(path: String) -> Result<IconPackDescriptor, String> {
    ...
    if !source.exists() {
        return Err(format!("icon-pack source does not exist: {}", source.display()));
    }
    if source.is_file()
        && !source
            .extension()
    ...
    {
        return Err("icon-pack installer requires a .streamDeckIconPack package or extracted directory".into());
    }
    ...
    fs::create_dir_all(&active).map_err(|error| error.to_string())?;
    fs::create_dir_all(&inactive).map_err(|error| error.to_string())?;
    let staging = home_dir()?.join(format!(".local/share/opendeck/.icon-pack-stage-{}", Uuid::new_v4()));
    ...
    let hash = if source.is_file() { sha256_file(&source)? } else { format!("{:x}", Sha256::digest(source.display().to_string().as_bytes())) };
    ...
    fs::rename(&staging, &active_path).map_err(|error| format!("activate icon pack {}: {error}", meta.name))?;
...
pub(crate) fn icon_pack_set_active(pack_id: String, active: bool) -> Result<(), String> {
    ...
    let (source, destination) = if active { (&inactive_path, &active_path) } else { (&active_path, &inactive_path) };
