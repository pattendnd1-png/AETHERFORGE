...
printf 'FORGEHX_BASELINE_VERSION=10.0.24\n'
printf 'FORGEHX_UPDATE=DSP_DEVICEPAGE_CLIPPY_CLEANUP_HOTFIX\n'
...
run SOURCE python3 scripts/check-v10.0.25-source.py
run VERIFY_CLEANUP python3 scripts/test-10.0.25-verification-cleanup.py
run CLIPPY_CLEANUP_10_0_23 python3 scripts/test-10.0.23-clippy-cleanup.py
run GUI_CLIPPY_CLEANUP_10_0_24 python3 scripts/test-10.0.24-gui-clippy-cleanup.py
run DSP_DEVICEPAGE_CLIPPY_CLEANUP python3 scripts/test-10.0.25-dsp-devicepage-clippy-cleanup.py
run PACKAGING_VERSION python3 scripts/test-10.0.25-packaging-version.py
run BACKGROUND_REJECTION python3 scripts/test-10.0.21-background-rejection.py
...
run MIC_AETHERSTREAM_BRIDGE python3 scripts/test-10.0.25-aetherstream-bridge.py
...
if command -v wpctl >/dev/null 2>&1 && wpctl status 2>/dev/null | grep -Fq 'HyperX SoloCast 2 Analog Stereo'; then printf 'FORGEHX_SOLOCAST_ENDPOINT_VISIBLE=PASS\n'; else printf 'FORGEHX_SOLOCAST_ENDPOINT_VISIBLE=INFO:not_observed\n'; fi
if (( fail == 0 )); then printf 'FORGEHX_10_0_25_VERIFY=PASS\n'; exit 0; else printf 'FORGEHX_10_0_25_VERIFY=FAIL\n'; exit 1; fi
