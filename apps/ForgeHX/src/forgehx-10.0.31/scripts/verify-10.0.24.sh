...
printf 'FORGEHX_BASELINE_VERSION=10.0.23\n'
printf 'FORGEHX_UPDATE=GUI_CLIPPY_CLEANUP_HOTFIX\n'
...
run SOURCE python3 scripts/check-v10.0.24-source.py
run VERIFY_CLEANUP python3 scripts/test-10.0.24-verification-cleanup.py
run CLIPPY_CLEANUP_10_0_23 python3 scripts/test-10.0.23-clippy-cleanup.py
run GUI_CLIPPY_CLEANUP python3 scripts/test-10.0.24-gui-clippy-cleanup.py
run PACKAGING_VERSION python3 scripts/test-10.0.24-packaging-version.py
...
run MIC_AETHERSTREAM_BRIDGE python3 scripts/test-10.0.24-aetherstream-bridge.py
...
if (( fail == 0 )); then printf 'FORGEHX_10_0_24_VERIFY=PASS\n'; exit 0; else printf 'FORGEHX_10_0_24_VERIFY=FAIL\n'; exit 1; fi
