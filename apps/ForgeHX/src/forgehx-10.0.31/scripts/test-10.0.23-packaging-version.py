...
checks={
 'source package version':'VERSION="10.0.23"' in create,
 'build package archive':'forgehx-10.0.23.tar.gz' in build,
 'no old fixed build archive':'forgehx-10.0.17.tar.gz' not in build,
}
failed=[k for k,v in checks.items() if not v]
if failed: raise SystemExit('FAIL: '+', '.join(failed))
print('PASS: ForgeHX 10.0.23 packaging version contract')
