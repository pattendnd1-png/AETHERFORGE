...
grep -q '^version = "10.0.14"$' Cargo.toml || { echo 'workspace version 10.0.14 missing' >&2; exit 1; }
grep -q '^pkgver=10.0.14$' PKGBUILD || { echo 'PKGBUILD pkgver 10.0.14 missing' >&2; exit 1; }
...
python scripts/test-10.0.14-direct-target-id.py
...
echo 'ForgeHX 10.0.14 release identity invariants passed.'
