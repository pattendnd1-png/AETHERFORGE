...
checks={
'version':'version = "10.0.27"' in cargo and 'pkgver=10.0.27' in pkg,
...
'10.0.24 gui clippy cleanup regression':(root/'scripts/test-10.0.24-gui-clippy-cleanup.py').exists(),
'10.0.25 dsp/device-page clippy regression':(root/'scripts/test-10.0.25-dsp-devicepage-clippy-cleanup.py').exists(),
'10.0.27 live noise rejection regression':(root/'scripts/test-10.0.27-live-noise-rejection.py').exists(),
'10.0.27 always-on DSP activation regression':(root/'scripts/test-10.0.27-always-on-dsp-activation.py').exists(),
'10.0.27 routing lock regression':(root/'scripts/test-10.0.27-solocast-routing-lock.py').exists(),
'always-on stable discovered source':'ensure_always_on_with_source_name' in daemon and 'restore_always_on_dsp(&all_devices, &audio_nodes)' in daemon,
'manual apply stable discovered source':'live_microphone_source_target' in daemon and 'apply_with_source_name' in daemon,
...
if failed: raise SystemExit('FORGEHX_10_0_27_SOURCE=FAIL '+','.join(failed))
print('FORGEHX_10_0_27_SOURCE=PASS')
