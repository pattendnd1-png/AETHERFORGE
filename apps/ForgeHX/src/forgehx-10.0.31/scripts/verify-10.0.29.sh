...
printf 'FORGEHX_BASELINE_VERSION=10.0.28\n'
printf 'FORGEHX_UPDATE=LIVE_SPEAKER_REFERENCE_MONITOR_FIX\n'
...
run SOURCE python3 scripts/check-v10.0.29-source.py
run EXTRANEOUS_NOISE_MONITOR python3 scripts/test-10.0.29-extraneous-noise-monitor.py
run LIVE_SPEAKER_REFERENCE python3 scripts/test-10.0.29-speaker-reference.py .
run PACKAGING_VERSION python3 scripts/test-10.0.29-packaging-version.py
...
run SOLOCAST_ROUTING_LOCK python3 scripts/test-10.0.29-solocast-routing-lock.py
run MIC_AETHERSTREAM_BRIDGE python3 scripts/test-10.0.29-aetherstream-bridge.py
...
if command -v forgehx >/dev/null 2>&1; then
  ...
  if [[ -n "$device_id" ]] && forgehx mic dsp get "$device_id" >"$state_file" 2>/dev/null; then
    if python3 scripts/test-10.0.29-runtime-state.py "$state_file"; then
...
if (( fail == 0 )); then
  ...
  printf 'FORGEHX_10_0_29_VERIFY=PASS\n'
  printf 'FORGEHX_VERIFY_FILE=%s\n' "$OUT"
  exit 0
else
  printf 'FORGEHX_10_0_29_VERIFY=FAIL\n'
  printf 'FORGEHX_VERIFY_FILE=%s\n' "$OUT"
  exit 1
fi
