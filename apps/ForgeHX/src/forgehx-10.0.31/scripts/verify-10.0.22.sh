...
printf 'FORGEHX_BASELINE_VERSION=10.0.21\n'
printf 'FORGEHX_UPDATE=VERIFICATION_RUNTIME_CLEANUP\n'
...
run SOURCE python3 scripts/check-v10.0.22-source.py
run VERIFY_CLEANUP python3 scripts/test-10.0.22-verification-cleanup.py
run PACKAGING_VERSION python3 scripts/test-10.0.22-packaging-version.py
...
run MIC_AETHERSTREAM_BRIDGE python3 scripts/test-10.0.22-aetherstream-bridge.py
...
if (( fail == 0 )); then printf 'FORGEHX_10_0_22_VERIFY=PASS\n'; exit 0; else printf 'FORGEHX_10_0_22_VERIFY=FAIL\n'; exit 1; fi
