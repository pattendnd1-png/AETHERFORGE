...
client=(root/'crates/forgehx-aetherstream/src/lib.rs').read_text()
daemon=(root/'crates/forgehx-daemon/src/lib.rs').read_text()
gui=(root/'crates/forgehx-gui/src/output_dsp.rs').read_text()
...
joined='\n'.join([client,daemon,gui,dsp])
for token in ['set-default','set_default','pactl set-default-source','pactl set-default-sink']:
    if token in joined:
        raise SystemExit('FORGEHX_SOLOCAST_ROUTING_LOCK=FAIL mutation='+token)
