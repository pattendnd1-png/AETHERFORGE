...
printf 'FORGEHX_BASELINE_VERSION=10.0.29\n'
printf 'FORGEHX_UPDATE=VOICE_ONLY_CAPTURE_DELAY_ALIGNED_PLAYBACK_REJECTION\n'
...
run SOURCE python3 scripts/check-v10.0.30-source.py
run VOICE_ONLY_CAPTURE python3 scripts/test-10.0.30-voice-only-capture.py
run SPECTRAL_FLUX_SHADOWING python3 scripts/test-10.0.30-spectral-flux-shadowing.py .
...
run PACKAGING_VERSION python3 scripts/test-10.0.30-packaging-version.py
...
run SOLOCAST_ROUTING_LOCK python3 scripts/test-10.0.30-solocast-routing-lock.py
run MIC_AETHERSTREAM_BRIDGE python3 scripts/test-10.0.30-aetherstream-bridge.py
...
if command -v forgehx >/dev/null 2>&1; then
  ...
  if [[ -n "$device_id" ]] && forgehx mic dsp get "$device_id" >"$state_file" 2>/dev/null; then
    if python3 scripts/test-10.0.30-runtime-state.py "$state_file"; then
...
if (( fail == 0 )); then
  ...
  printf 'FORGEHX_VOICE_ONLY_CAPTURE=PASS\n'
  printf 'FORGEHX_10_0_30_VERIFY=PASS\n'
  printf 'FORGEHX_VERIFY_FILE=%s\n' "$OUT"
  exit 0
else
  printf 'FORGEHX_10_0_30_VERIFY=FAIL\n'
  printf 'FORGEHX_VERIFY_FILE=%s\n' "$OUT"
  exit 1
fi
