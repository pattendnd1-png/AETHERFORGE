...
on_exit(){
  local rc=$?
  if (( rc != 0 )); then
    if ! grep -Eq '^AETHER_BROWSER_V2_1_60_VERIFY=(PASS|FAIL)' "$VERIFY_FILE" 2>/dev/null; then
      printf 'AETHER_BROWSER_VERSION=%s\nAETHER_BROWSER_V2_1_60_VERIFY=FAIL_EARLY:%s:exit=%s\nAETHER_BROWSER_FAILURE_STAGE=%s\n' "$VERSION" "$CURRENT_STAGE" "$rc" "$CURRENT_STAGE" >> "$VERIFY_FILE"
    fi
    if ! grep -Eq '^AETHER_BROWSER_V2_1_60_INSTALL_VERIFY=(PASS|FAIL)' "$INSTALL_VERIFY" 2>/dev/null; then
      printf 'AETHER_BROWSER_VERSION=%s\nAETHER_BROWSER_V2_1_60_INSTALL_VERIFY=FAIL_EARLY:%s:exit=%s\nAETHER_BROWSER_FAILURE_STAGE=%s\n' "$VERSION" "$CURRENT_STAGE" "$rc" "$CURRENT_STAGE" >> "$INSTALL_VERIFY"
    fi
