...
## v1.0.21 — Full Downloads Gate + Project Files lane

`forgeclean-system downloads-inventory` now recursively inventories every visible filesystem entry under `~/Downloads`, including hidden files, unknown extensions, generated/build trees, directories, and symlink objects. Symlinks are never followed.

The GUI Storage view is split into **Downloads — General Files** and **Downloads — Project Files**. Project entries are grouped by detected project and tagged as Active, Build, Project Download, Release, Restored, Cold, Generated, Verify Evidence, Source, Archive, Installer, or Project File. The inventory gate is read-only and does not make a discovered path eligible for deletion.

CLI:

```bash
forgeclean-system downloads-inventory
forgeclean-system downloads-inventory --summary-only
forgeclean-system downloads-inventory --projects-only
```
ForgeClean v1.0.22 — Named Project Folders + Fast Downloads Scan

Changes
=======
1. Project files are grouped by canonical project NAME, not by every artifact filename.
   Examples:
     AetherForge-DOWNLOADS40-THEME-DIAGNOSTICS.txt -> AetherForge
     HIT-IT-AETHERFORGE-OBS-GUARD.sh               -> AetherForge
     OpenDeck-v2.0.71-VERIFY.txt                    -> OpenDeck

2. Project files are physically routed under the separate project section:
     ~/Downloads/Project Files/<ProjectName>/
   The old internal path remains compatible through:
     ~/Downloads/ForgeClean/Projects -> ~/Downloads/Project Files

3. Non-project downloads stay directly in ~/Downloads.
   The persistent organizer no longer sorts ordinary downloads into generic
   Documents / Images / Archives / Installers / Other buckets.

4. The new persistent project router only touches items confidently associated
   with a project by name. Unrelated installers, media, documents and other
   downloads remain where they were downloaded.

5. Downloads inventory is parallelized. The default worker count is 2x logical
   CPU count, bounded to 4..32 threads. Override with:
     FORGECLEAN_SCAN_THREADS=<n>

6. GUI/dashboard inventory uses the bounded parallel summary path instead of
   retaining millions of entry objects in memory.

7. Terminal output is controlled:
     forgeclean-system downloads-inventory --summary-only
       totals only, zero PROJECT spam

     forgeclean-system downloads-inventory
       totals + top 20 projects

     forgeclean-system downloads-inventory --projects-only
       totals + all project summaries

     forgeclean-system downloads-inventory --details
       explicit full per-entry output

8. Existing legacy generic ForgeClean download buckets are migrated once:
   project-related items go to their named project folder; unrelated files go
   back to ~/Downloads. ColdStorage and Restored are not touched by this migration.

Safety
======
- Symlinks are never followed by the router.
- Project source directories moved into canonical project storage receive a
  compatibility symlink at the old top-level path.
- Cross-filesystem moves are refused rather than silently copied/deleted.
- Name collisions are retained with a .duplicate-N suffix.
- The service is stopped during build/install and restarted only after the new
  binaries and unit are in place.
