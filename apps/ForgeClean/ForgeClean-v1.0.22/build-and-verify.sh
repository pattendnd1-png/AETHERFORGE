...
run_gate FORGECLEAN_V1_0_19_PROJECT_STORAGE ./tests/regression_v1_0_19_project_storage.sh || status=1
run_gate FORGECLEAN_V1_0_21_DOWNLOADS_INVENTORY ./tests/regression_v1_0_21_downloads_inventory.sh || status=1
run_gate FORGECLEAN_V1_0_22_NAMED_PROJECTS_FAST_SCAN ./tests/regression_v1_0_22_named_projects_fast_scan.sh || status=1
...
run_gate FORGECLEAN_PROJECT_STORAGE_TEST cargo test --test project_storage || status=1
run_gate FORGECLEAN_DOWNLOADS_INVENTORY_TEST cargo test --test downloads_inventory || status=1
run_gate FORGECLEAN_PROJECT_ROUTING_TEST cargo test --test project_routing || status=1
...
if [[ $status -ne 0 ]]; then
  ...
  PROJECT_STORAGE_HOME="$(mktemp -d "${OUT_DIR}/forgeclean-project-storage-e2e.XXXXXX")"
  mkdir -p \
    "$PROJECT_STORAGE_HOME/Downloads/ForgeClean/Projects/AetherBrowser/Builds/3.0.31/target/release" \
    "$PROJECT_STORAGE_HOME/Downloads/ForgeClean/Projects/AetherBrowser/Downloads"
  printf 'project-source\n' > "$PROJECT_STORAGE_HOME/Downloads/ForgeClean/Projects/AetherBrowser/Builds/3.0.31/AetherBrowser-v3.0.31-SOURCE.zip"
  printf 'generated-output\n' > "$PROJECT_STORAGE_HOME/Downloads/ForgeClean/Projects/AetherBrowser/Builds/3.0.31/target/release/aether-browser"
  printf 'widevine\n' > "$PROJECT_STORAGE_HOME/Downloads/ForgeClean/Projects/AetherBrowser/Downloads/WidevineCdm.zip"
  printf 'personal\n' > "$PROJECT_STORAGE_HOME/Downloads/vacation-photo.jpg"
  printf 'opendeck\n' > "$PROJECT_STORAGE_HOME/Downloads/OpenDeck-v2.0.71-SOURCE.zip"
  PROJECT_STORAGE_OUT="$PROJECT_STORAGE_HOME/project-storage.out"
  if HOME="$PROJECT_STORAGE_HOME" "$SYSTEM_BIN_OUT" project-storage >"$PROJECT_STORAGE_OUT" 2>&1 \
      && grep -Fxq 'FORGECLEAN_PROJECT_STORAGE=PASS' "$PROJECT_STORAGE_OUT" \
      && grep -Fxq 'ARTIFACT_PATHS=3' "$PROJECT_STORAGE_OUT" \
      && grep -Fxq 'UNROUTED_ARTIFACT_PATHS=1' "$PROJECT_STORAGE_OUT" \
      && grep -Fxq 'EXCLUDED_GENERATED_DIRS=1' "$PROJECT_STORAGE_OUT" \
      && grep -Fq $'PROJECT=AetherBrowser\t' "$PROJECT_STORAGE_OUT" \
      && grep -Fq $'PROJECT=OpenDeck\t' "$PROJECT_STORAGE_OUT"; then
    cat "$PROJECT_STORAGE_OUT" >>"$VERIFY"
    log "FORGECLEAN_PROJECT_STORAGE_E2E=PASS"
  else
    cat "$PROJECT_STORAGE_OUT" >>"$VERIFY" 2>/dev/null || true
    log "FORGECLEAN_PROJECT_STORAGE_E2E=FAIL"
    status=1
  fi
  rm -rf -- "$PROJECT_STORAGE_HOME"
  DOWNLOADS_INVENTORY_HOME="$(mktemp -d "${OUT_DIR}/forgeclean-downloads-inventory-e2e.XXXXXX")"
  mkdir -p \
    "$DOWNLOADS_INVENTORY_HOME/Downloads/misc/nested" \
    "$DOWNLOADS_INVENTORY_HOME/Downloads/ForgeClean/Projects/OpenDeck/Active/target/release" \
    "$DOWNLOADS_INVENTORY_HOME/Downloads/AetherSong/src"
  printf 'hidden\n' > "$DOWNLOADS_INVENTORY_HOME/Downloads/.hidden-unknown.zzz"
  printf 'blob\n' > "$DOWNLOADS_INVENTORY_HOME/Downloads/misc/nested/blob.odd"
  printf '[package]\nname="opendeck"\n' > "$DOWNLOADS_INVENTORY_HOME/Downloads/ForgeClean/Projects/OpenDeck/Active/Cargo.toml"
  printf 'source\n' > "$DOWNLOADS_INVENTORY_HOME/Downloads/ForgeClean/Projects/OpenDeck/Active/src.rs"
  printf 'generated\n' > "$DOWNLOADS_INVENTORY_HOME/Downloads/ForgeClean/Projects/OpenDeck/Active/target/release/opendeck"
  printf '[workspace]\n' > "$DOWNLOADS_INVENTORY_HOME/Downloads/AetherSong/Cargo.toml"
  printf 'source\n' > "$DOWNLOADS_INVENTORY_HOME/Downloads/AetherSong/src/lib.rs"
  printf 'verify\n' > "$DOWNLOADS_INVENTORY_HOME/Downloads/ForgeClean-v1.0.21-VERIFY.txt"
  ln -s 'misc/nested/blob.odd' "$DOWNLOADS_INVENTORY_HOME/Downloads/blob-link"
  DOWNLOADS_INVENTORY_OUT="$DOWNLOADS_INVENTORY_HOME/downloads-inventory.out"
  if HOME="$DOWNLOADS_INVENTORY_HOME" "$SYSTEM_BIN_OUT" downloads-inventory --details >"$DOWNLOADS_INVENTORY_OUT" 2>&1 \
      && grep -Fxq 'FORGECLEAN_DOWNLOADS_INVENTORY=PASS' "$DOWNLOADS_INVENTORY_OUT" \
      && grep -Eq '^TOTAL_ENTRIES=[1-9][0-9]*$' "$DOWNLOADS_INVENTORY_OUT" \
      && grep -Eq '^GENERAL_ENTRIES=[1-9][0-9]*$' "$DOWNLOADS_INVENTORY_OUT" \
      && grep -Eq '^PROJECT_ENTRIES=[1-9][0-9]*$' "$DOWNLOADS_INVENTORY_OUT" \
      && grep -Fq $'PROJECT=OpenDeck\t' "$DOWNLOADS_INVENTORY_OUT" \
      && grep -Fq $'PROJECT=AetherSong\t' "$DOWNLOADS_INVENTORY_OUT" \
      && grep -Fq $'ENTRY=GENERAL\tTYPE=FILE\t' "$DOWNLOADS_INVENTORY_OUT" \
      && grep -Fq $'ENTRY=PROJECT\tTYPE=FILE\tPROJECT=OpenDeck\tCLASS=GENERATED' "$DOWNLOADS_INVENTORY_OUT" \
      && grep -Fq $'PATH=.hidden-unknown.zzz\t' "$DOWNLOADS_INVENTORY_OUT" \
      && grep -Fq $'TYPE=SYMLINK\t' "$DOWNLOADS_INVENTORY_OUT"; then
    cat "$DOWNLOADS_INVENTORY_OUT" >>"$VERIFY"
    log "FORGECLEAN_DOWNLOADS_INVENTORY_E2E=PASS"
  else
    cat "$DOWNLOADS_INVENTORY_OUT" >>"$VERIFY" 2>/dev/null || true
    log "FORGECLEAN_DOWNLOADS_INVENTORY_E2E=FAIL"
    status=1
  fi
  rm -rf -- "$DOWNLOADS_INVENTORY_HOME"
