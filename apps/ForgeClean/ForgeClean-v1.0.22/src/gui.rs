...
impl ForgeCleanGui {
    ...
    fn dashboard(&mut self, ui: &mut egui::Ui) {
        ...
        ui.add_space(12.0);
        section(ui, "Full Downloads inventory", |ui| {
            ui.horizontal_wrapped(|ui| {
                ui.label(
                    egui::RichText::new(format_bytes(
                        self.snapshot.downloads_inventory.total.allocated_bytes,
                    ))
                    .size(24.0)
                    .strong()
                    .color(accent()),
                ...
                ui.label(
                    egui::RichText::new(format!(
                        "allocated · {} total entries · {} general · {} project entries",
                        self.snapshot.downloads_inventory.total.entry_paths,
                        self.snapshot.downloads_inventory.general.entry_paths,
                        self.snapshot.downloads_inventory.project_files.entry_paths
                    ))
                    .color(text_muted()),
                );
            });
            ui.label(
                egui::RichText::new("Recursive inventory includes hidden, unknown, generated and symlink paths; inventory does not grant delete permission.")
                    .color(text_muted()),
            );
        });
        ui.add_space(10.0);
        section(ui, "Project download storage", |ui| {
            ui.horizontal_wrapped(|ui| {
                ui.label(
                    egui::RichText::new(format_bytes(
                        self.snapshot.project_storage.allocated_bytes,
                    ))
                    .size(24.0)
                    .strong()
                    .color(accent()),
                ...
                ui.label(
                    egui::RichText::new(format!(
                        "allocated disk · {} artifact paths · {} projects",
                        self.snapshot.project_storage.artifact_paths,
                        self.snapshot.project_storage.projects.len()
                    ))
                    .color(text_muted()),
                );
            });
    ...
    fn storage(&mut self, ui: &mut egui::Ui) {
        page_heading(
            ui,
            "Storage",
            "Actual project-download disk use, internal cleanup mode and verified offload",
        );
        ...
        ui.add_space(10.0);
        ui.add_space(10.0);
        egui::Grid::new("project_download_storage_metrics")
            .num_columns(2)
            .spacing([10.0, 10.0])
            .show(ui, |ui| {
                metric_card(
                    ui,
                    "Project downloads",
                    &format_bytes(self.snapshot.project_storage.allocated_bytes),
                    "filesystem-allocated blocks",
                );
                metric_card(
                    ui,
                    "Apparent size",
                    &format_bytes(self.snapshot.project_storage.logical_bytes),
                ...
                metric_card(
                    ui,
                    "Artifact paths",
                    &self.snapshot.project_storage.artifact_paths.to_string(),
                    &format!(
                        "{} unique files",
                        self.snapshot.project_storage.unique_files
                    ),
                );
                metric_card(
                    ui,
                    "Unrouted project files",
                    &self
                        .snapshot
                        .project_storage
                        .unrouted_artifact_paths
                        .to_string(),
                    "recognized project artifacts still in Downloads",
                );
                ui.end_row();
                metric_card(
                    ui,
                    "Build/cache dirs excluded",
                    &self
                        .snapshot
                        .project_storage
                        .excluded_generated_dirs
                        .to_string(),
                    "target/build/cache output is not counted",
                );
                metric_card(
                    ui,
                    "Deduplicated paths",
                    &self.snapshot.project_storage.deduplicated_paths.to_string(),
                    "hardlinks counted once for disk use",
                );
                ui.end_row();
            });
        ui.add_space(10.0);
        ui.add_space(10.0);
        egui::Grid::new("full_downloads_inventory_metrics")
            .num_columns(2)
            .spacing([10.0, 10.0])
            .show(ui, |ui| {
                metric_card(
                    ui,
                    "All Downloads",
                    &format_bytes(self.snapshot.downloads_inventory.total.allocated_bytes),
                    &format!(
                        "{} entries",
                        self.snapshot.downloads_inventory.total.entry_paths
                    ),
                ...
                metric_card(
                    ui,
                    "General files",
                    &format_bytes(self.snapshot.downloads_inventory.general.allocated_bytes),
                    &format!(
                        "{} entries",
                        self.snapshot.downloads_inventory.general.entry_paths
                    ),
                ...
                metric_card(
                    ui,
                    "Inventory errors",
                    &self.snapshot.downloads_inventory.scan_errors.to_string(),
                    "unreadable entries are reported, not silently ignored",
                );
                ui.end_row();
            });
        ui.add_space(10.0);
        section(ui, "Downloads — General Files", |ui| {
            ui.label(
                egui::RichText::new("Everything in Downloads not classified as project material.")
                    .color(text_muted()),
            );
            egui::ScrollArea::vertical()
                .id_salt("downloads_inventory_general_list")
                .max_height(320.0)
                .show(ui, |ui| {
                    for entry in self
                        .snapshot
                        ...
                        .entries
                        .iter()
                        .filter(|entry| entry.project.is_none())
                    {
                        ui.horizontal_wrapped(|ui| {
                            ui.label(
                            ...
                            ui.label(
                                egui::RichText::new(entry.relative_path.display().to_string())
                                    .color(text_primary()),
                            );
                            ui.label(
                                egui::RichText::new(format!(
                            ...
                            if let Some(target) = &entry.symlink_target {
                                ui.label(
                                    egui::RichText::new(format!("→ {}", target.display()))
                                        .color(text_muted()),
                                );
                            }
                        });
        ...
        ui.add_space(10.0);
        section(ui, "Downloads — Project Files", |ui| {
            ui.label(
                egui::RichText::new("Project material is a separate inventory lane and remains protected from automatic cleanup by this gate.")
                    .color(text_muted()),
            );
            for project in &self.snapshot.downloads_inventory.projects {
                ui.horizontal_wrapped(|ui| {
                    ui.label(egui::RichText::new(&project.name).strong().color(accent()));
                    ui.label(
                        egui::RichText::new(format!(
                            "{} allocated · {} entries · {} files",
                            format_bytes(project.summary.allocated_bytes),
            ...
            ui.separator();
            egui::ScrollArea::vertical()
                .id_salt("downloads_inventory_project_list")
                .max_height(420.0)
                .show(ui, |ui| {
                    for entry in self
                        .snapshot
                        ...
                        .filter(|entry| entry.project.is_some())
                    {
                        ui.horizontal_wrapped(|ui| {
                            ui.label(
                                egui::RichText::new(entry.project.as_deref().unwrap_or("Project"))
                                    .strong()
                                    .color(accent()),
                            );
                            ui.label(
                                egui::RichText::new(
                                    entry.project_class.as_deref().unwrap_or("PROJECT_FILE"),
                                )
                                .color(text_muted()),
                            );
                            ui.label(
        ...
        ui.add_space(10.0);
        section(ui, "Project download footprint", |ui| {
            if self.snapshot.project_storage.projects.is_empty() {
                ui.label(
                    egui::RichText::new("No recognized project delivery files found.")
                        .color(text_muted()),
                );
            } else {
                egui::ScrollArea::vertical()
                    .id_salt("project_download_storage_list")
                    .max_height(320.0)
                    .show(ui, |ui| {
                        for project in &self.snapshot.project_storage.projects {
                            ui.horizontal_wrapped(|ui| {
                                ui.label(
                                ...
                                ui.label(
                                    egui::RichText::new(format!(
                                        "{} allocated · {} apparent · {} artifacts",
                                        format_bytes(project.allocated_bytes),
