# v0.1.22 On Device Profile Import
...
## v0.1.22 cache identity hardening

Cache fallback is now **same-mic only**. When the live getter scan cannot complete and a connected mic serial is known, AetherForge reads cached profiles only when the embedded `on_device.serial` matches that mic. Matching accepts the Linux-prefixed identity form used by USB/audio stacks, but an unrelated BEACN serial is rejected rather than silently loading the newest cache from another device.

The startup strip labels a cache fallback as **Same-mic cache** and shows **SERIAL VERIFIED** so the provenance is visible instead of implicit.

## v0.1.22 dependency gate

The host installer resolves dependencies once with `cargo generate-lockfile`, records the `Cargo.lock` SHA-256 in the verification log, and then runs strict Clippy, tests, and the release build with `--locked`. This prevents those later release gates from silently resolving a different dependency graph during the same qualification run.
