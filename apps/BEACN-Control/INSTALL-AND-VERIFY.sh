...
ROLLBACK_DIR="$HOME/.local/share/aetherforge-beacn-control/rollback/pre-v0.1.15"
...
echo "==> v0.1.15 adaptive BEACN interface contract"
./scripts/v0.1.15-interface-contract.sh src/main.rs src/layout.rs
echo "AETHERFORGE_BEACN_ADAPTIVE_UI_CONTRACT=PASS"

echo "==> v0.1.15 Windows BEACN layout / DragonGlass parity contract"
./scripts/v0.1.15-windows-parity-ui-contract.sh src/main.rs
echo "AETHERFORGE_BEACN_WINDOWS_PARITY_UI_CONTRACT=PASS"

echo "==> v0.1.15 Live Profiles / private software-DSP contract"
./scripts/v0.1.15-live-profile-dsp-contract.sh
echo "AETHERFORGE_BEACN_LIVE_PROFILE_DSP_CONTRACT=PASS"

echo "==> v0.1.15 Windows-style live DSP workflow contract"
./scripts/v0.1.15-windows-live-dsp-contract.sh
echo "AETHERFORGE_BEACN_WINDOWS_LIVE_DSP_CONTRACT=PASS"

echo "==> v0.1.15 private-DSP isolation contract"
./scripts/v0.1.15-private-dsp-isolation-contract.sh
echo "AETHERFORGE_BEACN_PRIVATE_DSP_ISOLATION_CONTRACT=PASS"

echo "==> v0.1.15 system-audio protection / isolation contract"
./scripts/v0.1.15-system-audio-protection-contract.sh
echo "AETHERFORGE_BEACN_SYSTEM_AUDIO_PROTECTION=PASS"

echo "==> v0.1.15 model-only hardware / private-DSP contract"
./scripts/v0.1.15-hardware-dsp-contract.sh
...
echo "==> v0.1.15 runtime audio-node contract"
./scripts/v0.1.15-runtime-audio-contract.sh "$ROOT"
echo "AETHERFORGE_BEACN_RUNTIME_AUDIO_CONTRACT=PASS"

echo "==> v0.1.15 BEACN output-profile recovery contract"
./scripts/v0.1.15-output-profile-contract.sh "$ROOT"
echo "AETHERFORGE_BEACN_OUTPUT_PROFILE_CONTRACT=PASS"

echo "==> v0.1.15 profile-repair invocation contract"
./scripts/v0.1.15-profile-repair-invocation-contract.sh "$ROOT"
echo "AETHERFORGE_BEACN_PROFILE_REPAIR_INVOCATION_CONTRACT=PASS"

echo "==> v0.1.15 graphical-session recovery contract"
./scripts/v0.1.15-graphical-session-contract.sh "$ROOT"
echo "AETHERFORGE_BEACN_GRAPHICAL_SESSION_CONTRACT=PASS"

echo "==> v0.1.15 egui 0.36 style API regression contract"
./scripts/v0.1.15-egui-style-api-contract.sh "$ROOT"
...
echo "==> v0.1.15 strict-Clippy test-style regression contract"
./scripts/v0.1.15-clippy-test-style-contract.sh "$ROOT"
...
echo "==> v0.1.15 private-audio strict-Clippy regression contract"
./scripts/v0.1.15-private-audio-clippy-contract.sh "$ROOT"
